...
**Export** opens `ExportPanel`: choose **full chat** or **last assistant response**, then one or more of **11 formats** — Markdown, Plain Text, HTML, JSON, PDF, Word (DOCX), OpenDocument Text, Excel (XLSX), OpenDocument Spreadsheet, CSV, PowerPoint (PPTX). Multi-select can download **separate files** or a **ZIP**. Server: **`POST /api/generate-office`** (rate-limited) uses `lib/office-generator.js`; **`GET /api/export/formats`** returns capability metadata. The builtin agent tool **`generate_office_file`** uses the same generator for agent-authored downloads. See **`docs/EXPORT-CHAT.md`**.
...
- **Document conversion**: Two-tier system — built-in converters handle 12 common formats without external dependencies; Docling-serve provides higher-quality conversion (OCR, complex layouts) for all 15 formats when available. Built-in: PDF (pdf-parse), DOCX (mammoth), DOC (word-extractor), XLSX/XLS/CSV (SheetJS), PPTX/PPT/ODT/ODS/ODP/RTF (officeparser). Docling-only: EPUB, LaTeX, TEX. The `/api/convert-document` endpoint tries Docling first, falls back to built-in automatically. Response includes `converter: 'docling'|'builtin'`. **Export** (see above) **generates** Office/PDF/text from chat via `office-generator.js`, separate from upload conversion. Docling auto-starts on port 5002 (not 5001 — macOS AirPlay conflict). Auto-start managed by `lib/docling-starter.js` (web) and `electron/docling-manager.js` (Electron). Config: `docling: { url, apiKey, enabled, ocr }`. Install: `uv tool install "docling-serve[ui]"`. See `docs/DOCLING-AUTO-START.md` for details.

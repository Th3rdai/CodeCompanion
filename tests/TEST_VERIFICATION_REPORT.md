...
if (dimensions.width > maxDim || dimensions.height > maxDim) {
  return {
    valid: false,
    error: `Image too large: ${w}x${h}px. Max: ${maxDim}px`,
  };
}
```
...
if (images && images.length > maxImages) {
  return res
    .status(400)
    .json({ error: `Maximum ${maxImages} images per message` });
}
```
...
```javascript
{
  processingImages && (
    <div className="text-xs text-blue-400">
      Processing {attachedFiles.filter((f) => f.isImage && !f.thumbnail).length}{" "}
      images...
    </div>
  );
}

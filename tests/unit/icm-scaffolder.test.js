...
function writeTemplateFixture(templateRoot) {
  ...
  fs.writeFileSync(
    path.join(templateRoot, "_config/brand-voice.md"),
    [
  ...
  fs.writeFileSync(
    path.join(templateRoot, "shared/README.md"),
    "Shared resources",
  );
  fs.writeFileSync(
    path.join(templateRoot, "skills/README.md"),
    "Skills resources",
  );
}
...
test("scaffoldProject creates expected project structure", () => {
  const sandboxRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "scaffolder-unit-"),
  );
  ...
  try {
    ...
    assert.ok(
      fs.existsSync(path.join(result.projectPath, "_config", "brand-voice.md")),
    );
    assert.ok(
      fs.existsSync(
        path.join(result.projectPath, "stages", "01-research", "output"),
      ),
    );
    assert.ok(
      fs.existsSync(
        path.join(result.projectPath, "stages", "01-research", "references"),
      ),
    );
    assert.ok(
      fs.existsSync(
        path.join(result.projectPath, "stages", "02-draft", "output"),
      ),
    );
  } finally {

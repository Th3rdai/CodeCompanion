...
describe("scaffoldProject with icmTemplatePath", () => {
  ...
  before(() => {
    ...
    fs.writeFileSync(
      path.join(icmFwDir, "ICM-README.md"),
      "# ICM Framework\n",
      "utf8",
    );
  ...
  it("copies Commands to .cursor/commands and ICM-fw to project root when icmTemplatePath is set", () => {
    ...
    assert.ok(
      fs.existsSync(result.projectPath),
    ...
    assert.ok(
      fs.existsSync(path.join(commandsDest, "hello.md")),
      ".cursor/commands/hello.md should exist",
    );
    assert.strictEqual(
      fs.readFileSync(path.join(commandsDest, "hello.md"), "utf8"),
      "# Hello Command\n",
    );
    assert.ok(
      fs.existsSync(path.join(commandsDest, "foo.txt")),
      ".cursor/commands/foo.txt should exist",
    );

    // ICM-fw contents at project root
    assert.ok(
      fs.existsSync(path.join(result.projectPath, "ICM-README.md")),
      "ICM-README.md should exist at root",
    );
    assert.strictEqual(
      fs.readFileSync(path.join(result.projectPath, "ICM-README.md"), "utf8"),
      "# ICM Framework\n",
    );
    assert.ok(
      fs.existsSync(path.join(result.projectPath, "sub", "nested.txt")),
      "sub/nested.txt should exist",
    );
    assert.strictEqual(
      fs.readFileSync(
        path.join(result.projectPath, "sub", "nested.txt"),
        "utf8",
      ),
      "nested",
    );
    ...
    assert.ok(
      fs.existsSync(path.join(result.projectPath, "CLAUDE.md")),
      "CLAUDE.md should exist",
    );
    assert.ok(
      fs.existsSync(path.join(result.projectPath, "CONTEXT.md")),
      "CONTEXT.md should exist",
    );
    assert.ok(
      fs.existsSync(path.join(result.projectPath, "stages")),
      "stages/ should exist",
    );
  });

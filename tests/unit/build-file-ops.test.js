...
describe("Build File Operations", { concurrency: false }, () => {
  let baseUrl;
  let projectId;
  let child;
  let tmpRoot;
  ...
  it("allows reading and writing whitelisted planning files", async () => {
    const getRes = await fetch(
      `${baseUrl}/api/build/projects/${projectId}/files/STATE.md`,
    );
  ...
  it("rejects non-whitelisted filenames with 403", async () => {
    const res = await fetch(
      `${baseUrl}/api/build/projects/${projectId}/files/secrets.env`,
    );
  ...
  it("blocks path traversal in filename segment (403)", async () => {
    ...
    const res = await fetch(
      `${baseUrl}/api/build/projects/${projectId}/files/${evil}`,
    );
    assert.equal(res.status, 403);
  });
  ...
  it("writes via temp file then rename (no stale .tmp left)", async () => {
    ...
    const putRes = await fetch(
      `${baseUrl}/api/build/projects/${projectId}/files/${target}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    assert.equal(
      tmpLeft.length,
      0,
      `unexpected tmp files: ${tmpLeft.join(", ")}`,
    );

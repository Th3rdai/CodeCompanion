...
describe('Image Processor - Base64 Extraction', () => {
  describe('extractBase64', () => {
    ...
    it('throws on invalid data URL format', () => {
      assert.throws(() => extractBase64('not-a-data-url'), /Invalid data URL format/);
      assert.throws(() => extractBase64('data:text/plain,hello'), /Invalid data URL format/);
      assert.throws(() => extractBase64('iVBORw0KGgo...'), /Invalid data URL format/);
    });

    it('throws on missing base64 content', () => {
      assert.throws(() => extractBase64('data:image/png;base64,'), /Invalid data URL format/);
    });
  });
});
...
describe('Image Processor - Hashing', () => {
  describe('hashImage', () => {
    ...
    it('samples first 10KB for large images', () => {
      ...
      assert.equal(hash1, hash2, 'Hashes should match when first 10KB is identical');
    });
  });
});
...
describe('Image Processor - Data URI Validation', () => {
  describe('validateDataURI', () => {
    ...
    it('accepts valid JPEG data URI', () => {
      assert.equal(validateDataURI('data:image/jpeg;base64,/9j/4AAQSkZJ'), true);
    });
    ...
    it('rejects non-image MIME types', () => {
      ...
      assert.equal(validateDataURI('data:application/pdf;base64,JVBERi0x'), false);
    });

    it('rejects unsupported image formats', () => {
      assert.equal(validateDataURI('data:image/svg+xml;base64,PHN2Zz4='), false);
    ...
    it('rejects data URIs with invalid base64 characters', () => {
      assert.equal(validateDataURI('data:image/png;base64,invalid!@#$%'), false);
      assert.equal(validateDataURI('data:image/png;base64,<script>alert(1)</script>'), false);
    });
...
describe('Image Processor - Filename Sanitization', () => {
  describe('sanitizeFilename', () => {
    ...
    it('removes unsafe characters', () => {
      ...
      assert.equal(sanitizeFilename('image with spaces.jpg'), 'image_with_spaces.jpg');
    });
    ...
    it('handles absolute paths by taking basename', () => {
      ...
      assert.equal(sanitizeFilename('C:\\Users\\Documents\\photo.jpg'), 'C__Users_Documents_photo.jpg');
    });

    it('handles filenames with multiple dots', () => {
      assert.equal(sanitizeFilename('file.name.with.dots.png'), 'file.name.with.dots.png');
    });
...
describe('Image Processor - Constants', () => {
  describe('ALLOWED_MIME_TYPES', () => {
    it('exports correct MIME types', () => {
      assert.deepEqual(ALLOWED_MIME_TYPES, ['image/png', 'image/jpeg', 'image/gif']);
    });

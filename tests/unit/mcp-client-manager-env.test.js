...
test('buildStdioMcpEnv: prefixed MCP_github_*__ overrides global for same key', () => {
  ...
  const prevPref = process.env.MCP_github_3rdaai_admin__GITHUB_PERSONAL_ACCESS_TOKEN;
  try {
    process.env.GITHUB_PERSONAL_ACCESS_TOKEN = 'global-pat';
    process.env.MCP_github_3rdaai_admin__GITHUB_PERSONAL_ACCESS_TOKEN = 'per-server-pat';
    ...
    if (prevPref === undefined) delete process.env.MCP_github_3rdaai_admin__GITHUB_PERSONAL_ACCESS_TOKEN;
    else process.env.MCP_github_3rdaai_admin__GITHUB_PERSONAL_ACCESS_TOKEN = prevPref;
  }
});

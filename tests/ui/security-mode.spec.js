...
const mockSecurityReportResponse = {
  ...
  data: {
    overallGrade: 'C',
    riskSummary: 'Several security vulnerabilities were found including SQL injection and hardcoded credentials.',
    topRisk: {
      category: 'injection',
      title: 'SQL Injection via string concatenation',
      explanation: 'User input is directly concatenated into SQL queries without parameterization.'
    },
    categories: {
      accessControl: { grade: 'B', summary: 'Basic access control in place', vulnerabilities: [] },
      dataProtection: {
        grade: 'D',
        ...
        vulnerabilities: [
          {
            ...
            remediationPrompt: 'Please move the hardcoded database password to an environment variable and read it with process.env.DB_PASSWORD.',
        ...
        vulnerabilities: [
          {
            ...
            codeLocation: 'Line 15: query = "SELECT * FROM users WHERE id = " + userId',
            remediation: 'Use parameterized queries instead of string concatenation.',
            remediationPrompt: 'Please replace the SQL string concatenation on line 15 with a parameterized query using prepared statements.',
            cvssEstimate: 'Critical (9.0-10.0)'
          }
        ]
      },
      authAndSession: { grade: 'B', summary: 'Session handling is adequate', vulnerabilities: [] },
      configuration: { grade: 'A', summary: 'Configuration follows best practices', vulnerabilities: [] },
      apiSecurity: { grade: 'B', summary: 'API endpoints are reasonably protected', vulnerabilities: [] }
    },
...
test.describe('Security Mode', () => {
  ...
  test('clicking Security tab shows SecurityPanel with paste input', async ({ page }) => {
    await page.click('button:has-text("Security")');
    const inputArea = page.getByPlaceholder(/paste your code here for owasp/i);
    await expect(inputArea).toBeVisible();
  });

  test('SecurityPanel has Scan for Vulnerabilities button', async ({ page }) => {
    await page.click('button:has-text("Security")');
  ...
  test('SecurityReport renders all 6 category sections', async ({ page }) => {
    await page.click('button:has-text("Security")');
    await page.getByPlaceholder(/paste your code here for owasp/i).fill('const password = "admin123"; db.query("SELECT * FROM users WHERE id = " + req.params.id);');
    ...
    await expect(page.locator('h3:has-text("Injection & Input")')).toBeVisible();
  ...
  test('SecurityReport shows vulnerability details with severity pills', async ({ page }) => {
    await page.click('button:has-text("Security")');
    await page.getByPlaceholder(/paste your code here for owasp/i).fill('const x = 1;');
    await page.getByRole('button', { name: /scan for vulnerabilities/i }).click();

...
test.describe('Prompting Builder Mode', () => {
  ...
  test('displays prompt builder input fields', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder(/what should this prompt achieve/i)).toBeVisible();
    await expect(page.getByPlaceholder(/which ai is this for/i)).toBeVisible();
  });

  test('score button is disabled when prompt text is empty', async ({ page }) => {
  ...
  test('submits prompt for scoring and displays score card', async ({ page }) => {
    // Fill in the prompt content
    await page.getByPlaceholder(/write your prompt here/i).fill('You are a helpful code reviewer. Analyze the following code and provide feedback on bugs, security, and readability.');
    await page.getByPlaceholder(/what should this prompt achieve/i).fill('Code review feedback');
    ...
    await expect(page.getByText(/score report/i).first()).toBeVisible({ timeout: 15000 });
  ...
  test('score card categories are expandable', async ({ page }) => {
    // Fill and score
    await page.getByPlaceholder(/write your prompt here/i).fill('Test prompt for scoring');
    await page.getByRole('button', { name: /score/i }).click();
    await expect(page.getByText(/score report/i).first()).toBeVisible({ timeout: 15000 });

    // Click on a category row button to expand suggestions
    const structureBtn = page.getByRole('button', { name: /structure/i }).first();
    await structureBtn.click();

    // Suggestions should be visible after expanding
    await expect(page.getByText(/add section headers/i)).toBeVisible({ timeout: 5000 });
  });
});

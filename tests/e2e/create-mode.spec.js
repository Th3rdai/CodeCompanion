...
test('Create mode renders when models endpoint fails', async ({ page }) => {
  ...
  await expect(page.getByRole('form', { name: 'Create project wizard' })).toBeVisible();
...
test('wizard creates project and opens it in file browser', async ({ page, request }) => {
  ...
  try {
    ...
    await page.getByLabel('Description').fill('Create mode end-to-end validation project');

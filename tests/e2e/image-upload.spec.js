...
test.describe("Image Upload E2E - Chat Mode", () => {
  ...
  test("should upload image via file picker", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", {
        name: /View full-size image: test-screenshot\.png/i,
      }),
    ).toBeVisible({ timeout: 20_000 });

    await expect(page.getByText(/Processing \d+ images?/i)).not.toBeVisible({
      timeout: 8000,
    });
  ...
  test("should detect non-vision model warning", async ({ page }) => {
    await page
      ...
      .selectOption({ value: "llama3:latest" });
    ...
    await expect(page.getByText(/doesn't support images/i)).toBeVisible({
      timeout: 3000,
    });
    ...
    if (await switchButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await expect(page.getByText(/doesn't support images/i)).not.toBeVisible({
        timeout: 5000,
      });
  ...
  test("should remove individual images", async ({ page }) => {
    ...
    const thumb1 = page.getByRole("button", {
      name: /View full-size image: test1\.png/i,
    });
    const thumb2 = page.getByRole("button", {
      name: /View full-size image: test2\.png/i,
    });
    await expect(thumb1).toBeVisible({ timeout: 10000 });
    await expect(thumb2).toBeVisible({ timeout: 10000 });

    const removeButton = page.getByRole("button", {
      name: /Remove test1\.png/i,
    });
  ...
  test("should open lightbox on thumbnail click", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: /View full-size image: test\.png/i })
      .click();
    ...
    const closeButton = lightbox
      ...
      .or(lightbox.locator('button:has-text("×")'));
  ...
  test("should reject unsupported file formats", async ({ page }) => {
    ...
    await expect(page.getByText(/Only PNG, JPEG, GIF allowed/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("should show privacy warning on first upload", async ({ page }) => {
    ...
    const privacyModal = page
      .getByText(/privacy.*notice/i)
      .or(page.getByText(/sensitive.*information/i));
...
test.describe("Image Upload E2E - Review Mode", () => {
  ...
  test("should upload bug screenshot with code", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", {
        name: /View full-size image: bug-screenshot\.png/i,
      }),
    ).toBeVisible({ timeout: 10000 });
...
test.describe("Image Upload E2E - Security Mode", () => {
  ...
  test("should upload error log screenshot with code", async ({ page }) => {
    const codeTextarea = page.getByPlaceholder(
      /paste your code here for owasp/i,
    );
    await codeTextarea.fill(
      'const user = req.body; db.query("SELECT * FROM users WHERE id=" + user.id);',
    );
    ...
    await expect(
      page.getByRole("button", {
        name: /View full-size image: sql-injection-error\.png/i,
      }),
    ).toBeVisible({ timeout: 10000 });
    ...
    await expect(
      page.getByRole("heading", { name: /Security Scan Report/i }),
    ).toBeVisible({
      timeout: 10000,
    });
  });
...
test.describe("Image Upload E2E - Settings", () => {
  ...
  test("should configure image support settings", async ({ page }) => {
    ...
    if (
      await imageSupportSection.isVisible({ timeout: 3000 }).catch(() => false)
    ) {

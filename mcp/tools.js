  ...
  return async ({ content, model, context }) => {
    try {
      ...
      if (!selectedModel) {
        return {
          isError: true,
          content: [{ type: 'text', text: 'Error: No Ollama models available. Make sure Ollama is running (try: ollama serve).' }]
        };
      }
...
function registerAllTools(server, deps, disabledTools = []) {
  const { getConfig, log, debug, listModels, chatComplete, checkConnection, buildFileTree, readProjectFile, listConversations } = deps;

  function register(name, description, schema, handler) {
    if (disabledTools.includes(name)) return;
    server.tool(name, description, schema, { annotations: ANNOTATIONS }, handler);
  }

  // ===== 6 Mode Tools =====

  register('codecompanion_chat',
  ...
  register('codecompanion_explain',
  ...
  register('codecompanion_find_bugs',
  ...
  register('codecompanion_refactor',
  ...
  register('codecompanion_tech_to_biz',
  ...
  register('codecompanion_biz_to_tech',
  ...
  register('codecompanion_list_models',
    ...
    async () => {
      try {
        ...
        return { isError: true, content: [{ type: 'text', text: `Error listing models: ${err.message}` }] };
      }
    }
  );
...
- MCP Server: Active`;
  ...
  register('codecompanion_browse_files',
    ...
    async ({ path: subPath, depth }) => {
      try {
        ...
        function formatTree(items, indent = '') {
          ...
          for (const item of items) {
            if (item.type === 'dir') {
              ...
              if (item.children) out += formatTree(item.children, indent + '  ');
            } else {
        ...
        return { isError: true, content: [{ type: 'text', text: `Error browsing files: ${err.message}` }] };
      }
    }
  );

  register('codecompanion_read_file',
    ...
    async ({ path: filePath }) => {
      try {
        ...
        const projectFolder = config.projectFolder;
        if (!projectFolder) {
          return { isError: true, content: [{ type: 'text', text: 'Error: No project folder configured.' }] };
        }
        ...
        return { content: [{ type: 'text', text: capResponse(header + result.content) }] };
      } catch (err) {
    ...
    async () => {
      try {
        ...
        if (conversations.length === 0) {
          return { content: [{ type: 'text', text: 'No conversations stored yet.' }] };
        }
        ...
        return { content: [{ type: 'text', text: `Saved Conversations (${conversations.length}):\n${formatted}` }] };
      } catch (err) {
        return { isError: true, content: [{ type: 'text', text: `Error: ${err.message}` }] };
      }
    }

...
function loadConfig(appRoot) {
  ...
  const defaults = {
    ...
    ollamaApiKey: "",
    /**
     * Active LLM provider: "ollama" (default) | "openrouter". Toggle, not additive —
     * exactly one is active at a time. Default keeps all existing Ollama behavior.
     */
    provider: "ollama",
    /** OpenRouter (OpenAI-compatible aggregator). Env OPENROUTER_API_KEY used when empty. */
    openrouterApiKey: "",
    ...
    chatTimeoutSec: 600,
    /**
     * Slow-model self-heal: when an auto-resolved model spends longer than this
     * (seconds) on a single chat round, auto mode demotes it for the session and
     * retries the round on a faster tool-capable model (cloud allowed). 0 = off.
     * Caps at one switch per turn; never applies to a model the user picked
     * explicitly (only auto-resolved). Vision tool rounds pass honorExplicitTimeout
     * so this cap is not raised to the 300s vision minimum in chatComplete.
     */
    ...
    mcpClients: [],
    /**
     * When true (default), external MCP tools are advertised to the agent in
     * compact form (name + key params, no long descriptions) so their schemas
     * don't inflate the system prompt on every turn. They remain fully
     * callable. Set false to restore full external tool descriptions.
     */
    ...
    agentTerminal: {
      enabled: false,
      /**
       * Allow the agent terminal to run even when the server is bound to a
       * network-exposed address (HOST=0.0.0.0 / CC_BIND_ALL=1). Default false:
       * shell execution is blocked on an exposed bind unless this is on (or the
       * CC_ALLOW_AGENT_TERMINAL=1 env var is set). Electron is always exempt.
       * Surfaced as a Settings → Agent toggle so non-technical users don't need
       * an env var; carries a LAN-exposure warning in the UI.
       */
      allowWhenExposed: false,
    ...
    /**
     * Auto-continue: re-prompt until TASK_COMPLETE, NEEDS_USER_INPUT:, or caps.
     * Agent tool loop (Chat/Experiment) and streaming modes (Explain, Spec, etc.).
     * Set enabled: false for legacy single-step behavior after tools.
     */
    autoContinue: {
      enabled: true,
      maxSteps: 15,
    ...
    if (fs.existsSync(CONFIG_FILE)) {
      ...
      const merged = {
        ...
        memory: {
          ...
          ...(saved.memory || {}),
          indexCache: {
            ...(defaults.memory.indexCache || {}),
            ...((saved.memory && saved.memory.indexCache) || {}),
          },
          pruneCheck: {
            ...(defaults.memory.pruneCheck || {}),
            ...((saved.memory && saved.memory.pruneCheck) || {}),
            thresholds: {
              ...((defaults.memory.pruneCheck &&
                defaults.memory.pruneCheck.thresholds) ||
                {}),
              ...((saved.memory &&
                saved.memory.pruneCheck &&
                saved.memory.pruneCheck.thresholds) ||
                {}),
            },
          },
        },

 ...
 */
function formatUserOllamaChatError({ status, detail, totalChars = 0 }) {
  ...
  if (
    ...
    return "Could not reach Ollama. Check that Ollama is running and Settings → General has the correct server URL.";
  }
  ...
  if (
    ...
    return "Context window or model limit exceeded. Try a shorter message, less history or tool output, or a model with a larger context.";
  }
  ...
  if (
    ...
    return clip ? `Ollama model error: ${clip}` : `Ollama returned HTTP ${status}. Check the model name and pull it if needed.`;
  }
  ...
  if (status === 500 && refMatch) {
    return `Ollama Cloud returned an opaque error (ref: ${refMatch[1]}). This usually means a missing/expired API key (Settings → General → Ollama Cloud API key), a per-model rate limit, or a transient cloud outage — not a problem with your message. Try again in a moment, or switch to a local model.`;
  }

  if (status === 500 && totalChars > 30000) {
    const tail =
      d.length > 420 ? `${d.slice(0, 420)}…` : d;
    return tail
  ...
  if (d) {
    return d.length > 900 ? `${d.slice(0, 900)}…` : d;
  }

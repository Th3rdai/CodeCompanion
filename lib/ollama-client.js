...
/**
 * Active LLM provider for a config. "ollama" (default) or "openrouter".
 * Anything other than the literal "openrouter" resolves to "ollama" so a
 * malformed/legacy config can never accidentally route to OpenRouter.
 */
function effectiveProvider(config) {
  return config && config.provider === "openrouter" ? "openrouter" : "ollama";
}

/**
 * OpenRouter API key: process.env wins over .cc-config.json (mirrors
 * {@link effectiveOllamaApiKey}) so the secret stays out of JSON.
 */
function effectiveOpenrouterApiKey(config) {
...
// Lazy-loaded so the default (Ollama-only) path never pays the require cost and
// the two modules can reference each other without a load-time cycle.
let _orClient = null;
const orClient = () => (_orClient ||= require("./openrouter-client"));
/** True when an opts bag was built by the OpenRouter branch of ollamaAuthOpts. */
const wantsOpenrouter = (opts) =>
  !!(opts && opts.__ccProvider === "openrouter");

/**
 * The single universal seam (~55 call sites). Provider-aware: for OpenRouter it
 * returns a sentinel bag ({@link wantsOpenrouter}) the dispatch guards detect;
 * for Ollama it keeps the original {apiKey} shape unchanged.
 */
...
async function listModels(ollamaUrl, opts = {}) {
  if (wantsOpenrouter(opts)) {
    return orClient().listModels(opts);
  }
...
/** Clear listModels cache (e.g. after config changes in tests). Also clears the OpenRouter client's own catalog cache. */
function invalidateListModelsCache() {
  _listModelsCache = { key: "", at: 0, models: null };
  // Clear the OR cache too — only if the module is already loaded (don't force a
  // require just to clear an empty cache on Ollama-only installs).
  if (_orClient && typeof _orClient.invalidateListModelsCache === "function") {
    _orClient.invalidateListModelsCache();
  }
}

async function checkConnection(ollamaUrl, opts = {}) {
  if (wantsOpenrouter(opts)) {
    return orClient().checkConnection(opts);
  }
  const apiKey = opts.apiKey || "";
  try {
...
function chatStream(
  ...
  // Dispatch FIRST: the entire Ollama path below (URL, headers, body) is
  // meaningless for OpenRouter. NON-async so callers' `await` works for both
  // branches (both return a Promise<Response-like>).
  if (wantsOpenrouter(ollamaOptions)) {
    return orClient().chatStream(model, messages, images, ollamaOptions);
  }
...
async function chatStructured(
  ...
  if (wantsOpenrouter(ollamaOptions)) {
    return orClient().chatStructured(
      model,
      messages,
      jsonSchema,
      timeoutMs,

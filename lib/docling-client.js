...
async function checkConnection(doclingUrl, apiKey) {
  try {
    ...
    const msg =
      err.name === "AbortError" ? "Connection timed out (5s)" : err.message;
...
async function convertDocument(
  ...
  try {
    ...
    log(
      "INFO",
      `Converting ${filename} (${(fileBuffer.length / 1024).toFixed(1)}KB)`,
      { outputFormat, ocr },
    );
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Docling returned HTTP ${res.status}: ${errBody.slice(0, 200)}`,
      );
    }
    ...
    log(
      "INFO",
      `Converted ${filename}: ${status} (${processingTime.toFixed(1)}s, ${(markdown.length / 1024).toFixed(1)}KB output)`,
    );
 ...
 */
async function convertDocumentAsync(
  doclingUrl,
  apiKey,
  fileBuffer,
  filename,
  ...
  if (!res.ok) {
    const errBody = await res.text().catch(() => "");
    throw new Error(
      `Async conversion failed: HTTP ${res.status}: ${errBody.slice(0, 200)}`,
    );
  }
...
async function pollTaskStatus(doclingUrl, apiKey, taskId) {
  ...
  const res = await fetch(`${doclingUrl}/v1/status/poll/${taskId}`, {
    headers,
  });
...
function effectiveDoclingApiKey(config) {
  ...
  return (
    (config?.docling?.apiKey && String(config.docling.apiKey).trim()) || ""
  );
}

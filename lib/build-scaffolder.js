...
function buildPlanningConfigJson() {
  return JSON.stringify({
    ...
    workflow: { research: true, plan_check: true, verifier: true, auto_advance: false },
    planning: { commit_docs: true, search_gitignored: false },
    gates: { confirm_project: true, confirm_phases: true, execute_next_plan: true },
  }, null, 2);
}
 ...
 */
function buildProjectInstructions(name, description) {
...
function scaffoldBuildProject(options, config = {}) {
  ...
  const files = [];
  const warnings = [];
  const { name, description, outputRoot: outputRootInput, audience, tone, overwrite = false } = options || {};
  ...
  const tempDir = path.join(resolvedRoot, `.build-scaffold-${slug}-${Date.now()}`);
  ...
  try {
    ...
    try {
      ...
      if (commandsSrc && fs.existsSync(commandsSrc) && fs.statSync(commandsSrc).isDirectory()) {
            const ideTargets = [
    ...
    try {
      ...
      warnings.push(`Project created but registry update failed: ${err.message}`);
    }
...
function scaffoldPlanning(projectPath, name, description) {
  ...
  if (!description) {
    try {
      ...
      if (firstParagraph.length > 10) description = firstParagraph.slice(0, 300);
    } catch {}
  }

  fs.writeFileSync(path.join(planningDir, 'PROJECT.md'), buildProjectMd(name, description || ''));
  ...
  fs.writeFileSync(path.join(planningDir, 'REQUIREMENTS.md'), buildRequirementsMd());
  fs.writeFileSync(path.join(planningDir, 'config.json'), buildPlanningConfigJson());
}

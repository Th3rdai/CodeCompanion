...
function createMcpApiRoutes(deps) {
  ...
  router.post("/mcp/clients/test-connection", async (req, res) => {
    const { transport, command, args, env, url, headers, rejectUnauthorized } =
      req.body;
    if (!transport)
  ...
  // Mask header values for API responses (same logic as env vars)
  function maskHeaders(headersObj) {
  ...
  router.post("/mcp/clients", async (req, res) => {
    ...
    res.json({
      ...
      headers: maskHeaders(newClient.headers),
  ...
  router.put("/mcp/clients/:id", async (req, res) => {
    ...
    res.json({
      ...
      headers: maskHeaders(clients[idx].headers),
      ...(connectError ? { connectError } : {}),
    });
  });

...
function createMcpApiRoutes(deps) {
  const { getConfig, updateConfig, mcpClientManager, log, debug, requireLocalOrApiKey } = deps;
  ...
  const stats = { totalCalls: 0, callsByTool: {}, callsToday: 0, lastCallAt: null, todayDate: new Date().toDateString() };

  function recordToolCall(toolName) {
    const today = new Date().toDateString();
    if (today !== stats.todayDate) { stats.callsToday = 0; stats.todayDate = today; }
    stats.totalCalls++;
    stats.callsToday++;
  ...
  router.post('/mcp/server/toggle', (req, res) => {
    ...
    log('INFO', `MCP HTTP endpoint ${mcpServer.httpEnabled ? 'enabled' : 'disabled'}`);
    res.json({ httpEnabled: mcpServer.httpEnabled });
  });

  router.post('/mcp/server/tools', (req, res) => {
    const { disabledTools } = req.body;
    if (!Array.isArray(disabledTools)) return res.status(400).json({ error: 'disabledTools must be an array' });
    const config = getConfig();
    const mcpServer = config.mcpServer || {};
  ...
  router.get('/mcp/server/clients', (req, res) => {
    ...
    res.json({ clients: [], note: 'Stateless HTTP transport — client tracking not available' });
  });
  ...
  router.post('/mcp/clients/test-connection', async (req, res) => {
    ...
    if (!transport) return res.status(400).json({ error: 'transport is required' });

    const tempConfig = { id: '_test', name: 'test', transport, command, args, env, url };
    try {
      ...
      const status = err.message.startsWith('Invalid MCP client config:') ? 400 : 500;
  ...
  router.get('/mcp/clients', (req, res) => {
    ...
    const enriched = clients.map(c => {
      ...
      return { ...c, status: status?.status || 'disconnected', toolCount: status?.toolCount || 0, error: status?.error || null };
    });
    res.json(enriched);
  });

  router.post('/mcp/clients', (req, res) => {
    const { id, name, transport, command, args, env, url, autoConnect } = req.body;
    if (!id || !name || !transport) return res.status(400).json({ error: 'id, name, and transport are required' });
    if (id === 'builtin' || name === 'builtin') {
      return res.status(400).json({ error: '"builtin" is a reserved ID and cannot be used for MCP servers' });
    }

    const config = getConfig();
    const clients = config.mcpClients || [];
    if (clients.find(c => c.id === id)) return res.status(409).json({ error: `Client ${id} already exists` });

    let newClient;
    try {
      newClient = mcpClientManager.validateAndNormalizeConfig({
        id, name, transport, command, args, env, url, autoConnect: autoConnect || false, disabledTools: []
      });
    } catch (err) {
      const status = err.message.startsWith('Invalid MCP client config:') ? 400 : 500;
      return res.status(status).json({ error: status === 400 ? err.message : CLIENT_INTERNAL_ERROR });
    }
  ...
  router.put('/mcp/clients/:id', (req, res) => {
  ...
  router.post('/mcp/clients/:id/connect', async (req, res) => {
    const config = getConfig();
    const clientConfig = (config.mcpClients || []).find(c => c.id === req.params.id);
    if (!clientConfig) return res.status(404).json({ error: 'Client not found' });

    try {
      const tools = await mcpClientManager.connect(clientConfig);
      res.json({ status: 'connected', tools: tools.map(t => ({ name: t.name, description: t.description })) });
    } catch (err) {
  ...
  router.get('/mcp/clients/:id/tools', (req, res) => {
    ...
    res.json(conn.tools.map(t => ({ name: t.name, description: t.description, inputSchema: t.inputSchema })));
  });

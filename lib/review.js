...
async function reviewCode(ollamaUrl, model, code, opts = {}) {
  ...
  try {
    const raw = await chatStructured(ollamaUrl, model, messages, reportCardJsonSchema, timeout, images)
    ...
    const stream = await chatStream(ollamaUrl, model, fallbackMessages, images)

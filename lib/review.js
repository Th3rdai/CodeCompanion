...
const { formatBrandAssetsPrompt } = require("./brand-context");
const { formatHostTimeForPrompt } = require("./host-time");
...
async function reviewCode(ollamaUrl, model, code, opts = {}) {
  ...
  const brandPrompt = formatBrandAssetsPrompt(opts.brandAssets);
  const hostTimePrompt = formatHostTimeForPrompt();

  const messages = [
    {
      role: "system",
      content: `${hostTimePrompt}${systemPrompt}${REVIEW_ACCURACY_GUARDRAIL}${brandPrompt}`,
    },
  ...
  // +2048 response-headroom stays at the call site so the intent
  // (room for the model's reply) is visible here, not buried in the helper.
  let numCtx = opts.numCtx || 0;
  if (opts.autoAdjustContext) {
    const estimatedTokens = estimateMessageTokens(messages) + 2048;
  ...
  // Forward the provider sentinel so chatStructured/chatStream dispatch to OpenRouter.
  if (opts.__ccProvider) {
    ollamaOptions.__ccProvider = opts.__ccProvider;
    ollamaOptions.__ccOpenrouterApiKey = opts.__ccOpenrouterApiKey;
    ollamaOptions.__ccOpenrouterUrl = opts.__ccOpenrouterUrl;
  }
  ...
  try {
    ...
    const multiFileFallback =
      opts.systemPrompt === SYSTEM_PROMPTS["review-multi"]
        ? 'CRITICAL: You are reviewing MULTIPLE files. In EVERY finding, reference the specific filename using "In filename.ext, ..." format.\n\n'
        : "";
    const fallbackMessages = [
      {
        role: "system",
        content: `${hostTimePrompt}${multiFileFallback}${SYSTEM_PROMPTS["review-fallback"]}${REVIEW_ACCURACY_GUARDRAIL}${brandPrompt}`,
      },
      {
        role: "user",

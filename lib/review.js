...
async function reviewCode(ollamaUrl, model, code, opts = {}) {
  ...
  const visionContext = images.length > 0
  ...
  try {
    const raw = await chatStructured(ollamaUrl, model, messages, reportCardJsonSchema, timeout, images, ollamaOptions)
    ...
    const stream = await chatStream(ollamaUrl, model, fallbackMessages, images, ollamaOptions)

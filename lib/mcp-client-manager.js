...
class McpClientManager {
  ...
  async _doConnect(normalizedConfig) {
    ...
    if (transport === "stdio") {
      ...
      try {
        const fetchFunc =
          rejectUnauthorized === false ? createInsecureFetch() : fetch;
        ...
        this.debug(
          "SSE: Failed to fetch server session ID, generating client-side",
          {
            clientId: id,
            error: err.message,
          },
    ...
    try {
      ...
      const isGoogleWork =
        ...
        normalizedConfig.name.toLowerCase().includes("google");
      if (isGoogleWork) {

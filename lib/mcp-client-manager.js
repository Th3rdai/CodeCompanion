...
class McpClientManager {
  ...
  async connect(serverConfig) {
    ...
    // accepting MCP handshake. Retry with fresh spawn + increasing delay.
    const maxAttempts = transport === 'stdio' ? 3 : 1;
    const retryDelays = [0, 3000, 6000]; // 1st: immediate, 2nd: 3s, 3rd: 6s
    let lastErr;

    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        if (attempt > 0) {
          this.log('INFO', `Retrying stdio connection for ${normalizedConfig.name} (attempt ${attempt + 1}/${maxAttempts}, delay ${retryDelays[attempt] / 1000}s)...`);
          // Spawn fresh transport — previous one is dead after Connection closed
          clientTransport = new StdioClientTransport({
            command, args,
            env: mergeDevToolPathIntoEnv({ ...process.env, ...(env || {}) })
          });
        ...
        // Wait before connecting — gives proxy time to connect upstream
        if (retryDelays[attempt] > 0) {
          await new Promise(r => setTimeout(r, retryDelays[attempt]));
        }

        const client = new Client({ name: 'code-companion', version: '1.0.0' }, { capabilities: {} });
        ...
        this.log('INFO', `Connected to external MCP server: ${normalizedConfig.name} (${tools.length} tools)${attempt > 0 ? ` on attempt ${attempt + 1}` : ''}`);
        return tools;
      } catch (err) {
        lastErr = err;
        if (attempt < maxAttempts - 1 && transport === 'stdio' && err.message?.includes('Connection closed')) {
          this.log('INFO', `Stdio connection closed for ${normalizedConfig.name}, will retry...`);
          continue;
        }
        break;
      }

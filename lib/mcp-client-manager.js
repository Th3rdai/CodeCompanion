...
function enrichTransportError(err) {
  ...
  if (!/connection closed|32000|ECONNRESET|socket hang up/i.test(msg)) return err;
  const hint =
...
class McpClientManager {
  ...
  async connect(serverConfig) {
    ...
    if (transport === 'stdio') {
      ...
      if (command === 'npx' && args.length >= 1) {
        const pkgName = args[0].replace(/@latest$/, '').replace(/@[\d.^~]+$/, '');
        const localBin = path.join(__dirname, '..', 'node_modules', '.bin', pkgName.split('/').pop());
        if (fs.existsSync(localBin)) {
          resolvedCommand = localBin;
          resolvedArgs = args.slice(1); // drop the package name, keep remaining args
          this.debug('Stdio: using local binary', { localBin, args: resolvedArgs.join(' ') });
        }
      }
      this.debug('Stdio transport', { command: resolvedCommand, args: resolvedArgs.join(' '), envKeys: Object.keys(env || {}) });
    ...
    try {
      ...
      this.connections.set(id, {
        client: null, transport: null, config: normalizedConfig,
        tools: [], status: 'error', connectedAt: null, error: enrichTransportError(err).message
      });
  ...
  async callTool(serverId, toolName, toolArgs) {
    ...
    try {
      return await conn.client.callTool({ name: toolName, arguments: toolArgs });
    } catch (err) {
      throw enrichTransportError(err);
    }
  }

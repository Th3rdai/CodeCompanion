  ...
  async connect(serverConfig) {
    ...
    if (transport === 'stdio') {
      ...
      clientTransport = new StdioClientTransport({
        ...
        stderr: 'pipe', // capture stderr to detect proxy readiness
      });
    ...
    try {
      // For stdio transports, wait for the proxy to signal readiness via stderr
      // before the MCP SDK sends the initialize handshake.
      if (transport === 'stdio' && clientTransport._stderrStream) {
        await new Promise((resolve, reject) => {
          const timeout = setTimeout(() => {
            this.log('INFO', `Stdio proxy ${normalizedConfig.name} did not signal readiness in 15s, proceeding anyway`);
            resolve();
          }, 15000);
          let buf = '';
          clientTransport._stderrStream.on('data', (chunk) => {
            const text = chunk.toString();
            buf += text;
            // Log stderr lines from the proxy
            for (const line of text.split('\n').filter(l => l.trim())) {
              this.debug(`[${normalizedConfig.name} stderr] ${line.trim()}`);
            }
            // Detect readiness: most proxies print "running", "connected", "ready", or "listening"
            if (/running|connected|ready|listening|started|tools/i.test(buf)) {
              clearTimeout(timeout);
              this.log('INFO', `Stdio proxy ${normalizedConfig.name} signaled readiness`);
              resolve();
            }
          });
          clientTransport._stderrStream.on('error', () => { clearTimeout(timeout); resolve(); });
        });
      }

...
/**
 * Create a custom fetch function that bypasses SSL certificate validation.
 * Used when rejectUnauthorized is false (for self-signed certificates).
 *
 * Note: Node.js 18+ native fetch() uses undici under the hood, which requires
 * using the 'dispatcher' option with an undici Agent, not the 'agent' option.
 */
function createInsecureFetch() {
  ...
  return async (url, init = {}) => {
    // Only apply the insecure dispatcher to HTTPS URLs
...
class McpClientManager {
  ...
  async _doConnect(normalizedConfig) {
    const { id, transport, command, args, env, url, headers, rejectUnauthorized } =
      normalizedConfig;
    ...
    if (transport === "stdio") {
      ...
      // For SSE, try to get session ID from server first (some servers like Google Work provide it)
      try {
        const fetchFunc = rejectUnauthorized === false ? createInsecureFetch() : fetch;
        const initResponse = await fetchFunc(url, {
          method: "GET",
          headers: headers && typeof headers === "object" ? headers : {},
        });

        // Check if server provides a session ID in response headers
        const serverSessionId = initResponse.headers.get("mcp-session-id");
        if (serverSessionId) {
          this.debug("SSE: Using server-provided session ID", {
            clientId: id,
          ...
          this.debug("SSE: Using client-generated session ID", {
            clientId: id,
            sessionId
          });
        }
      } catch (err) {
        // If initial request fails, fall back to generating session ID
        this.debug("SSE: Failed to fetch server session ID, generating client-side", {
          clientId: id,
          error: err.message
        });
        sessionId = crypto.randomUUID();
      ...
      // Merge session ID header with user-provided headers
      const allHeaders = {
        "mcp-session-id": sessionId,
        ...(headers && typeof headers === "object" ? headers : {}),
      };

      // SSE uses requestInit for POST requests and eventSourceInit for the SSE stream
      opts.requestInit = { headers: allHeaders };

      // Create a custom fetch wrapper that modifies the Accept header after the SDK sets it
      // The MCP SDK's SSEClientTransport overrides Accept to only "text/event-stream",
      // but Google Work requires "application/json, text/event-stream"
      const baseFetch = rejectUnauthorized === false ? createInsecureFetch() : fetch;
      const fetchWrapper = async (url, init) => {
        // Call the base fetch
        const modifiedInit = { ...init };

        // Override the Accept header to include both content types
        if (modifiedInit.headers) {
          if (modifiedInit.headers instanceof Headers) {
            modifiedInit.headers.set('Accept', 'application/json, text/event-stream');
        ...
        return baseFetch(url, modifiedInit);
      };

      // Configure eventSourceInit with our custom fetch wrapper
      opts.eventSourceInit = {
        headers: allHeaders,
        fetch: fetchWrapper
      };

      // Set the base fetch for non-EventSource requests
      if (rejectUnauthorized === false) {
        opts.fetch = createInsecureFetch();
      }

      clientTransport = new SSEClientTransport(new URL(url), opts);
    } else if (transport === "http") {
      const opts = {};

      // Include both Accept types for servers like Google Work that require it
      // The transport will automatically extract the session ID from the initialize
      // response headers - we don't need to pre-fetch it.
      const allHeaders = {
        "Accept": "application/json, text/event-stream",
        ...(headers && typeof headers === "object" ? headers : {}),
      };
      ...
      // Add custom fetch for self-signed certificates
      if (rejectUnauthorized === false) {
        opts.fetch = createInsecureFetch();
      }

      clientTransport = new StreamableHTTPClientTransport(new URL(url), opts);
    } else {
    ...
    try {
      // Debug logging for Google Work connection
      const isGoogleWork = normalizedConfig.name && normalizedConfig.name.toLowerCase().includes('google');
      if (isGoogleWork) {
        this.log("DEBUG", `[Google Work] Starting connection to ${url}`);
        this.log("DEBUG", `[Google Work] Transport: ${transport}`);
        this.log("DEBUG", `[Google Work] Headers: ${JSON.stringify(headers)}`);
      }
      ...
      if (isGoogleWork) {
        this.log("DEBUG", `[Google Work] Connected in ${connectElapsed}ms`);
      }
      ...
      if (
        ...
        try {
          ...
          if (headers && typeof headers === "object" && Object.keys(headers).length > 0) {
            ...
            sseOpts.eventSourceInit = { headers };
          }
          // Add custom fetch for self-signed certificates
          if (rejectUnauthorized === false) {
            sseOpts.fetch = createInsecureFetch();
          }
          const sseTransport = new SSEClientTransport(new URL(url), sseOpts);
    ...
    const normalized = {
      ...
      headers:
        ...
        !Array.isArray(serverConfig.headers)
          ...
          : {},
      rejectUnauthorized:
        serverConfig.rejectUnauthorized !== undefined
          ? Boolean(serverConfig.rejectUnauthorized)
          : true, // Default to secure (reject invalid certs)

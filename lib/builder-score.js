...
function getTimeoutForModel(model) {
  ...
  if (/(?:^|[^0-9])(?:33b|34b|70b|72b|110b|405b)(?:$|[^0-9])/.test(name)) return 180000
 ...
 */
async function scoreContent(ollamaUrl, model, mode, content, metadata = {}, ollamaOpts = {}) {
  ...
  try {
    const raw = await chatStructured(ollamaUrl, model, messages, jsonSchema, timeout, [], ollamaOpts)
    ...
    const stream = await chatStream(ollamaUrl, model, fallbackMessages, [], ollamaOpts)

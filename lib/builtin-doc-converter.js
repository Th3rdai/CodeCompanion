...
async function convertXlsx(buffer, filename) {
  ...
  if (ext === '.xls') {
    return {
      markdown: '_(Legacy .xls is not supported by the built-in converter. Enable Docling in Settings or save as .xlsx.)_',
      status: 'partial',
      processingTime: (Date.now() - start) / 1000,
      errors: ['.xls requires Docling or conversion to .xlsx'],
      truncated: false,
    };
  }

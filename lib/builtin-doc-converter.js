...
async function convertPdf(buffer, _filename) {
  ...
  if (text.length < 20 && data.numpages > 0) {
    errors.push(
      "PDF appears to be scanned/image-based; install Docling for OCR support.",
    );
    markdown =
      text ||
      `_(No extractable text in ${data.numpages}-page PDF. Docling with OCR is required for scanned documents.)_`;
  } else {
    markdown = text;
  }
...
async function convertDocx(buffer, _filename) {
...
async function convertDoc(buffer, _filename) {
...
async function convertBuiltin(buffer, filename) {
  ...
  switch (ext) {
    case ".pdf":
      return convertPdf(buffer, filename);
    case ".docx":
      return convertDocx(buffer, filename);
    case ".doc":
      return convertDoc(buffer, filename);
    case ".xlsx":
    case ".csv":
    ...
    case ".odp":
    case ".rtf":
      return convertWithOfficeParser(buffer, filename);
    default:

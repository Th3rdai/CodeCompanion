...
async function runReviewSnippetPhase(p) {
  ...
  const result = await reviewCode(config.ollamaUrl, model, code, {
    ...
    // Provider bag: when OpenRouter is active this carries __ccProvider so
    // reviewCode forwards it into ollamaOptions and dispatch routes to OR.
...
async function runReviewFolderPhase(p) {
  ...
  if (model === "auto") {
    try {
      ...
      const m = mergeAutoModelMap(config.autoModelMap);
      model = m.review || m.chat || "llama3.2";
      log("WARN", "Auto-model resolution failed — using fallback", {
        error: err.message,
        fallback: model,
      });
    }

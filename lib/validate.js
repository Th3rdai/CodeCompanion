...
function scanProjectForValidation(folder) {
  ...
  function scanDir(dirPath, depth) {
    ...
    for (const entry of entries) {
      ...
      if (
        ...
        !entry.name.startsWith(".mypy") &&
        !entry.name.startsWith(".ruff") &&
        !entry.name.startsWith(".github") &&
        !entry.name.startsWith(".gitlab") &&
        !entry.name.startsWith(".mocha") &&
        !entry.name.startsWith(".stylelint") &&
        !entry.name.startsWith(".rubocop") &&
        !entry.name.startsWith(".golangci") &&
        !entry.name.startsWith(".circleci") &&
        !entry.name.startsWith(".travis") &&
        !entry.name.startsWith(".pylint") &&
        entry.name !== ".ruff.toml"
      )
        continue;
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name === ".circleci") {
          ...
          if (fs.existsSync(cfgPath))
            discovered.ci.push(path.join(relPath, "config.yml"));
        }
  ...
  if (fs.existsSync(path.join(absFolder, "Cargo.toml")))
    discovered.language = "rust";
  if (fs.existsSync(path.join(absFolder, "Gemfile")))
    discovered.language = "ruby";
 ...
 */
async function generateValidateCommand(
  ollamaUrl,
  model,
  folder,
  scanResult,
...
${
  Object.keys(scanResult.scripts).length > 0
    ? Object.entries(scanResult.scripts)
        .map(([k, v]) => `- ${k}: ${v}`)
        .join("\n")
    : "none found"
}

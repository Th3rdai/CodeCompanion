...
/**
 * Copy from template root: Commands -> all IDE command paths, ICM-fw -> project root.
...
function copyTemplateIntoProject(tempDir, config, filesList) {
  ...
  if (root && (!fs.existsSync(root) || !fs.statSync(root).isDirectory())) root = null;

  // Find Commands source: IDE_COMMANDS (primary), template path Commands (fallback)
  let commandsSrc = path.join(__dirname, '..', 'IDE_COMMANDS');
  if (!fs.existsSync(commandsSrc) || !fs.statSync(commandsSrc).isDirectory()) {
    commandsSrc = root ? path.join(root, 'Commands') : null;
  }

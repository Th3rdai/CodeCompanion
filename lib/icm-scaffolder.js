...
function buildInitialMd(options) {
  ...
  const stageList = (stages || [])
    .map((s) => `- **${s.name}**: ${s.purpose || "See CONTEXT.md"}`)
    .join("\n");
...
function copyCre8IntoProject(tempDir, options, filesList) {
  ...
  if (fs.existsSync(cre8Commands) && fs.statSync(cre8Commands).isDirectory()) {
    const ideTargets = [
      path.join(".claude", "commands"),
      path.join(".cursor", "commands"),
      path.join(".cursor", "prompts"),
...
function scaffoldProject(options, config = {}) {
  ...
  try {
    ...
    const brandAssetsMd = buildBrandScaffoldFile(config.brandAssets);

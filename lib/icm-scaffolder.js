...
/**
 * Get list of allowed writable roots. From config or default [homedir/AI_Dev].
 */
function getWritableRoots(config) {
  ...
  return [path.join(os.homedir(), 'AI_Dev')];
}
...
function scaffoldProject(options, config = {}) {
  const errors = [];
  const files = [];
  const { name, description, role, audience, tone, stages: rawStages, outputRoot: outputRootInput, overwrite = false } = options || {};
  ...
  try {
    writeRel('CLAUDE.md', buildClaudeMd(name, role, stages));

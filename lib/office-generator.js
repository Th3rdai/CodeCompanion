  ...
  return new Promise((resolve, reject) => {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('# ')) {
        doc.moveDown(0.5).font('Helvetica-Bold').fontSize(22).fillColor('#0F172A')
          .text(stripMarkdown(trimmed.slice(2))).moveDown(0.3);
      } else if (trimmed.startsWith('## ')) {
        doc.moveDown(0.4).font('Helvetica-Bold').fontSize(17).fillColor('#1E293B')
          .text(stripMarkdown(trimmed.slice(3))).moveDown(0.2);
      } else if (trimmed.startsWith('### ')) {
        doc.moveDown(0.3).font('Helvetica-Bold').fontSize(14).fillColor('#334155')
          .text(stripMarkdown(trimmed.slice(4))).moveDown(0.15);
      } else if (trimmed.startsWith('#### ')) {
        doc.moveDown(0.2).font('Helvetica-Bold').fontSize(12).fillColor('#475569')
          .text(stripMarkdown(trimmed.slice(5))).moveDown(0.1);
      }
      // Blockquote
      else if (/^>\s?/.test(trimmed)) {
        doc.font('Helvetica-Oblique').fontSize(11).fillColor('#6366F1')
      ...
      // Bullet
        ...
        doc.font('Helvetica').fontSize(11).fillColor('#1E293B')
          .text('\u2022  ' + stripMarkdown(trimmed.replace(/^\s*[-*+]\s+/, '')), { indent: 20 + indent });
      }
      // Numbered list
      else if (/^\s*\d+\.\s+/.test(trimmed)) {
        const num = trimmed.match(/^\s*(\d+)\./)[1];
        doc.font('Helvetica').fontSize(11).fillColor('#1E293B')
          .text(`${num}.  ` + stripMarkdown(trimmed.replace(/^\s*\d+\.\s+/, '')), { indent: 20 });
      }
      // Horizontal rule
      else if (/^[-*_]{3,}$/.test(trimmed)) {
        doc.moveDown(0.5);
        const y = doc.y;
        doc.moveTo(72, y).lineTo(doc.page.width - 72, y).strokeColor('#CBD5E1').lineWidth(0.5).stroke();
        doc.moveDown(0.5);
      }
      // Table row (simple rendering)
      else if (trimmed.startsWith('|') && trimmed.endsWith('|')) {
        if (/^\|[\s-:|]+\|$/.test(trimmed)) continue; // skip separator
        const cells = trimmed.slice(1, -1).split('|').map(c => stripMarkdown(c.trim()));
...
async function generateOds(content, options = {}) {
  ...
  if (tables.length === 0) {
    const rows = content.split('\n').map(l => [stripMarkdown(l)]).filter(r => r[0] !== '');
    const ws = XLSX.utils.aoa_to_sheet(rows);
    ws['!cols'] = [{ wch: 80 }];
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
  } else {
    for (const sheet of tables) {
      const cleanRows = sheet.rows.map(row => row.map(c => stripMarkdown(c)));
      const ws = XLSX.utils.aoa_to_sheet(cleanRows);
      XLSX.utils.book_append_sheet(wb, ws, sheet.name.slice(0, 31));
    }
  }
...
async function generateOdt(content, options = {}) {
  ...
  const paragraphs = clean.split('\n').filter(l => l.trim()).map(l =>
    `<text:p text:style-name="Standard">${escapeXml(l)}</text:p>`
  ).join('\n');
...
async function generateOfficeFile(content, filename, options = {}) {
  ...
  switch (ext) {
    ...
    case '.csv':  buffer = await generateCsv(content, options); break;
    case '.pdf':  buffer = await generatePdf(content, options); break;
    case '.odt':  buffer = await generateOdt(content, options); break;
    case '.ods':  buffer = await generateOds(content, options); break;
    case '.md':   buffer = generateMd(content); break;
    case '.txt':  buffer = generateTxt(content); break;
    case '.html': buffer = generateHtml(content); break;
    case '.json': buffer = generateJson(content, options); break;

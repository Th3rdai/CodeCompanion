...
async function generateDocx(content, _options = {}) {
...
async function generateXlsx(content, _options = {}) {
...
async function generateCsv(content, _options = {}) {
...
/**
 * Generate a professional compliance audit report PDF from security scan data.
 * @param {Object} securityReport - Security scan results with categories, grades, and compliance controls
 * @param {Object} options - { organizationName, scanDate, filename }
 * @returns {Promise<Buffer>} PDF buffer
 */
async function generateAuditReport(securityReport, options = {}) {
  const PDFDocument = require("pdfkit");
  ...
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({
        ...
        info: {
          ...
          Author: options.organizationName || "Code Companion",
        },
      });
    ...
    const scanDate = options.scanDate || new Date().toLocaleDateString();
    ...
    doc
      ...
      .fontSize(28)
      .fillColor("#1E293B")
      .text("Security Compliance", { align: "center" })
      .fontSize(32)
    ...
    doc
      ...
      .text(`Grade: ${overallGrade}`, { align: "center" })
      .moveDown(2);
    ...
    doc
      ...
      .fontSize(12)
      .fillColor("#64748B")
      .text(riskSummary || "No summary available", { align: "center" })
      .addPage();
    ...
    if (cleanBillOfHealth) {
      doc
        .font("Helvetica")
        .fontSize(11)
        .fillColor("#10B981")
        .text("✓ No security vulnerabilities detected", { indent: 20 })
        .moveDown();
    } else {
      doc
        .font("Helvetica")
        .fontSize(11)
        .fillColor("#1E293B")
        .text(`Overall Security Grade: ${overallGrade}`, { indent: 20 })
        .moveDown(0.3);

      // Count vulnerabilities by severity
      const allVulns = [];
      Object.values(categories || {}).forEach((cat) => {
        if (cat.vulnerabilities) allVulns.push(...cat.vulnerabilities);
      });
      ...
      doc
        .text(`Total Vulnerabilities: ${allVulns.length}`, { indent: 20 })
        .moveDown(0.2);

      if (severityCounts.critical > 0) {
        doc
          .fillColor("#DC2626")
          .text(`  • Critical: ${severityCounts.critical}`, { indent: 40 });
      }
      if (severityCounts.high > 0) {
        doc
          .fillColor("#F59E0B")
          .text(`  • High: ${severityCounts.high}`, { indent: 40 });
      }
      if (severityCounts.medium > 0) {
        doc
          .fillColor("#8B5CF6")
          .text(`  • Medium: ${severityCounts.medium}`, { indent: 40 });
      }
      if (severityCounts.low > 0) {
        doc
          .fillColor("#3B82F6")
          .text(`  • Low: ${severityCounts.low}`, { indent: 40 });
      }
    }
    ...
    if (
      ...
      frameworks.forEach((fw) => {
        const controls = complianceControls[fw.key] || [];
        ...
        doc
          ...
          .fontSize(14)
          .fillColor("#334155")
          .text(`${fw.label} (${controls.length} controls)`)
          .moveDown(0.2);
        ...
        doc
          ...
          .fontSize(9)
          .fillColor("#1E293B")
          .text(controls.join(", "), { indent: 20, width: 450 })
          .moveDown(0.8);
      });
    ...
    if (!cleanBillOfHealth && categories) {
      doc.addPage();
      doc
        .font("Helvetica-Bold")
        .fontSize(20)
      ...
      const categoryLabels = {
        ...
        injection: "Injection",
        authAndSession: "Authentication & Session Management",
      ...
      categoryKeys.forEach((key) => {
        ...
        if (!cat || !cat.vulnerabilities || cat.vulnerabilities.length === 0)
          return;
        ...
        doc
          ...
          .fontSize(16)
          .fillColor("#1E293B")
          .text(`${categoryLabels[key]} (Grade: ${cat.grade})`)
          .moveDown(0.3);

        cat.vulnerabilities.forEach((vuln, idx) => {
          ...
          doc
            ...
            .fillColor(sevColor)
            .text(
              `${idx + 1}. ${vuln.title} [${vuln.severity.toUpperCase()}]`,
              {
                indent: 20,
              },
            )
          ...
          doc
            ...
            .fontSize(10)
            .fillColor("#1E293B")
            .text(vuln.description, { indent: 30, width: 480 })
            .moveDown(0.2);
          ...
          if (vuln.impact) {
            doc
              ...
              .fontSize(9)
              .fillColor("#475569")
              .text("Impact:", { indent: 30 })
              .font("Helvetica")
              .text(vuln.impact, { indent: 50, width: 460 })
              .moveDown(0.2);
          }
          ...
          if (vuln.remediation) {
            doc
              ...
              .fontSize(9)
              .fillColor("#10B981")
              .text("Remediation:", { indent: 30 })
              .font("Helvetica")
              .text(vuln.remediation, { indent: 50, width: 460 })
              .moveDown(0.5);
          }
        });
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc
        ...
        .fillColor("#94A3B8")
        .text(
          `Generated by Code Companion | Page ${i + 1} of ${pageCount}`,
          72,
          doc.page.height - 50,
...
async function generateOds(content, _options = {}) {
...
async function generateOdt(content, _options = {}) {
  const JSZip = require("jszip");
  const zip = new JSZip();

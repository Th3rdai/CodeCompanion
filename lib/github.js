 ...
 * then falls back to legacy `config.githubToken`.
...
function resolveToken(config, owner) {
  ...
  if (owner && tokens.length) {
    ...
    // Match by label first (unique per token), then username
    const match = tokens.find(t => (t.label || '').toLowerCase() === ownerLower)
      || tokens.find(t => (t.username || '').toLowerCase() === ownerLower);
    if (match) return match.token;
  }
  // Return first token in array, or legacy single token
  if (tokens.length) return tokens[0].token;
...
function getAllTokens(config) {
  const tokens = [...(config.githubTokens || [])];
  // If legacy token exists and isn't already in the array, include it
  if (config.githubToken && !tokens.some(t => t.token === config.githubToken)) {
    tokens.unshift({ label: 'default', token: config.githubToken, username: '', avatar: '' });
  }
  return tokens;
}
...
async function validateTokenCached(token) {
  ...
  if (result.valid) {
    tokenValidationCache.set(key, { result, expires: Date.now() + TOKEN_VALIDATION_CACHE_TTL_MS });
  }
  return result;
}
...
function getGitStatus(repoPath) {
  ...
  let behind = 0;
  try {
    const upstreamCounts = runGit(repoPath, ['rev-list', '--left-right', '--count', '@{upstream}...HEAD']);
...
function createBranch(repoPath, branchName, checkout = true) {
  ...
  if (!/^[A-Za-z0-9._/-]+$/.test(clean) || clean.startsWith('/') || clean.endsWith('/')) {
...
function getMergePreview(repoPath, sourceBranch, targetRef = 'HEAD') {
  ...
  const preview = runGit(repoPath, ['merge-tree', mergeBase, targetRef, sourceBranch], { timeout: 30000 });
...
function resolveConflictFile(repoPath, filePath, strategy) {
  ensureGitRepo(repoPath);
  const strategyArg = strategy === 'theirs' ? '--theirs' : strategy === 'ours' ? '--ours' : null;
...
function initAndPush(localPath, remoteUrl, token, options = {}) {
  ...
  try {
    ...
    execSync(`git config user.name "${gitUser}"`, { cwd: localPath, stdio: 'pipe' });
    execSync(`git config user.email "${gitEmail}"`, { cwd: localPath, stdio: 'pipe' });
    ...
    try {
      ...
      try {
        ...
        execSync(`git commit --allow-empty -m "${commitMessage.replace(/"/g, '\\"')}"`, { cwd: localPath, stdio: 'pipe' });
      }
    } catch {
      // Has staged changes — commit them
      execSync(`git commit -m "${commitMessage.replace(/"/g, '\\"')}"`, { cwd: localPath, stdio: 'pipe' });
    }
    ...
    try {
      ...
      execSync(`git remote add origin "${authedUrl}"`, { cwd: localPath, stdio: 'pipe' });
    }

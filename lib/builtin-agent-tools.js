...
const BUILTIN_TOOLS = [
  ...
  },
  {
    ...
    description: 'Generate a Microsoft Office or CSV file from markdown content and save it to disk. Supported formats: DOCX (Word), XLSX (Excel), PPTX (PowerPoint), CSV. Args: {"content": "# Report Title\\n\\nBody text with **bold**, *italic*, bullet lists, numbered lists, and markdown tables.", "filename": "report.docx", "savePath": "/Users/james/Desktop/report.docx"}. For XLSX/CSV: include markdown tables (| col1 | col2 |). For PPTX: use # headings to split slides. All markdown formatting is converted to native Office formatting — no raw markdown in output. Returns the file path and size.',
...
async function generateOfficeFileTool(args, config, log) {
  ...
  if (!['.docx', '.xlsx', '.pptx', '.csv'].includes(ext)) {
    return {
      success: false,
      result: { content: [{ type: 'text', text: `Unsupported format: ${ext}. Use .docx, .xlsx, .pptx, or .csv` }] },
    };
  }

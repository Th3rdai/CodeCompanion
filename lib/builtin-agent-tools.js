...
const BUILTIN_TOOLS = [
  ...
  },
  {
    ...
    description: 'Generate a Microsoft Office file (DOCX, XLSX, or PPTX) from markdown content and save it to disk. Args: {"content": "# Report Title\\n\\nBody text with **bold**, *italic*, bullet lists, numbered lists, and markdown tables.", "filename": "report.docx", "savePath": "/Users/james/Desktop/report.docx"}. For XLSX: include markdown tables (| col1 | col2 |). For PPTX: use # headings to split slides. Returns the file path and size.',
...
async function generateOfficeFileTool(args, config, log) {
  ...
  if (!['.docx', '.xlsx', '.pptx'].includes(ext)) {
    return {
      success: false,
      result: { content: [{ type: 'text', text: `Unsupported format: ${ext}. Use .docx, .xlsx, or .pptx` }] },
    };
  }

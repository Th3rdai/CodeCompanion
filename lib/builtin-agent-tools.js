...
function validateCommand(command, args, config) {
  ...
  const invokeShell =
    allowShellChaining && METACHAR_PATTERN.test(joinedLine);
  ...
  if (invokeShell) {
    ...
    if (win) {
      if (!(allowlist.includes("cmd") || allowlist.includes("cmd.exe"))) {
        return {
          allowed: false,
          reason:
            'Shell chaining (&& ; | redirections) requires "cmd" in the agent terminal allowlist on Windows.',
          action:
            'ACTION: Tell the user to add `cmd` to Settings → Agent Terminal → Allowlist, or disable chaining and rewrite as separate tool calls.',
        };
      }
    } else if (!(allowlist.includes("bash") || allowlist.includes("sh"))) {
      return {
        allowed: false,
        reason:
          'Shell chaining (&& ; | redirections) requires "bash" or "sh" in the agent terminal allowlist.',
        action:
          'ACTION: Tell the user to add `bash` (or `sh`) to Settings → Agent Terminal → Allowlist, or disable "Allow shell chaining" and split the command.',
      };
    }
    return {
      allowed: true,

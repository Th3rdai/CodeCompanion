...
function checkCmdRateLimit(clientKey, limit = 20, windowMs = 60000) {
  const now = Date.now();
  ...
  if (entry.count > limit) {
    return {
      allowed: false,
      reason: `Terminal rate limit exceeded (${limit} commands per minute). Wait and try again.`,
    };
  }
  return { allowed: true };
}
...
setInterval(() => {
  ...
  for (const [key, entry] of _cmdRateMap) {
    if (now - entry.windowStart > 120000) _cmdRateMap.delete(key);
  }
}, 300000).unref();
...
const BUILTIN_TOOLS = [
  ...
  },
  {
    name: "search_files",
    description:
      'Search file contents for text/regex (grep-like; parallel-safe, no terminal). Returns file:line: snippets. Args: {"query","path"?,"regex"?,"caseSensitive"?,"maxResults"?}.',
    requiresTerminal: false,
    parallelSafe: true,
  },
  {
    name: "find_files",
    description:
      'Find files by glob pattern (parallel-safe, no terminal). Args: {"pattern":"**/*.test.js","path"?,"maxResults"?}. Globs: * (not /), ** (any dirs), ? (one char).',
    requiresTerminal: false,
    parallelSafe: true,
  },
  {
  ...
  },
  {
    name: "edit_file",
    description:
      "Targeted edit of an existing text file (.backup kept). Replace mode " +
      '{"path","oldText","newText","replaceAll"?} (oldText must be exact + unique) ' +
      'or append mode {"path","append":"..."}. Use instead of rewriting whole files; for brand-new files use write_file.',
    requiresTerminal: false,
    parallelSafe: false, // filesystem mutation
  },
  {
    name: "move_file",
    description:
      'Move/rename one file within the project. Args: {"from","to"}. Backs up an existing destination. Files only (not directories).',
    requiresTerminal: false,
    parallelSafe: false, // filesystem mutation
  },
  {
    name: "delete_file",
    description:
      'Delete one file within the project; keeps a <file>.backup. Args: {"path"}. Files only; only delete files the user asked you to remove.',
    requiresTerminal: false,
    parallelSafe: false, // filesystem mutation
  },
  {
  ...
  },
  {
    name: "fetch_url",
    description:
      'Fetch a public URL as text, no browser session (SSRF-guarded). Returns status, title, HTML-stripped text. Args: {"url","maxChars"?,"timeoutMs"?}. http(s) only; for JS/clicking use browse_url.',
    requiresBrowser: true,
    parallelSafe: true,
  },
  {
...
function getBuiltinTools(config) {
  ...
  if (terminalAllowed) {
    ...
    const allowExposedTerminal =
      process.env.CC_ALLOW_AGENT_TERMINAL === "1" ||
      config?.agentTerminal?.allowWhenExposed === true;
    if (isExposedBind && !allowExposedTerminal) {
...
FILES — read, search, edit, write (all work even when the terminal tool is off; all stay within the project folder, never use ".." to escape):
- FIND: builtin.search_files({"query":"...","path":"src"}) searches file CONTENTS (grep-like); builtin.find_files({"pattern":"**/*.jsx"}) finds by name. Prefer these over grep/rg/find via run_terminal_cmd — parallel-safe, no terminal needed.
- READ: builtin.read_file({"path":"src/App.jsx"}) (prefer over cat/head; read-only, 500KB cap). builtin.list_dir({"path":"."}) to see structure. For PDF/binary docs use view_pdf_pages or generate_office_file with sourcePath, not read_file.
- EDIT an existing file: builtin.edit_file — {"path","oldText","newText","replaceAll"?} for a targeted replace (oldText must be exact + unique, or set replaceAll), or {"path","append":"..."} to append a line. Prefer this over re-sending the whole file. Do NOT use shell redirection (>/>>) or $(...) substitution (blocked + a dead-end); embed dynamic values like the date (given above) as literals.
- CREATE a file: builtin.write_file({"path","content"}) — use for new files and for project Markdown/source (README, CHANGELOG, docs/*.md, .claude|.cursor command files, planning notes), NOT generate_office_file. Auto-.backup on overwrite.
- MOVE/RENAME: builtin.move_file({"from","to"}). DELETE: builtin.delete_file({"path"}) (keeps a .backup; only when the user asked).
`;
...
async function executeBuiltinTool(
  ...
  const allowExposedTerminal =
    process.env.CC_ALLOW_AGENT_TERMINAL === "1" ||
    config?.agentTerminal?.allowWhenExposed === true;
  if (isExposedBind && !isElectron && !allowExposedTerminal) {
    return {
      success: false,
      result: {
        content: [
          {
            type: "text",
            text: 'Agent terminal blocked: the server is reachable on your network. Enable Settings → Agent → "Allow agent terminal on a network-exposed server" (or set CC_ALLOW_AGENT_TERMINAL=1) to allow shell commands.',
          },
        ],
      },
    };
  ...
  if (clientKey) {
    const rateCheck = checkCmdRateLimit(
      clientKey,
      config.agentTerminal?.cmdRateLimit ?? 20,
      config.agentTerminal?.cmdRateWindowMs ?? 60000,
    );

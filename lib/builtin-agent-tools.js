 ...
 * Provides file I/O (read_file, write_file, list_dir), terminal, office/PDF,
 * validate, planner, browser, and app-skill tools.
 */
...
const { readProjectFile, isTextFile } = require("./file-browser");
...
// Detects commands that are likely to be long-running (dev servers, watchers)
// Returns { isLongRunning: boolean, suggestion?: string }
...
// Shell metacharacters: when agentTerminal.allowShellChaining is true (default),
// a matching full command line runs via bash -lc (Unix) or cmd (Windows) so chaining
// behaves as the user expects. When chaining is off or the line has no metachars,
// spawn uses a single executable + argv; argv entries must not contain these patterns:
// - `;`           command sequencing
// - `|`           pipe
// - backtick       command substitution (legacy)
// - `$(...)`       command substitution
// - `>&`, `>>`, `<<`, `2>&1` style redirection / fd duplication
// - `>` / `<`     common stdout/stdin redirection (spacing-delimited)
// - `&&`, `||`     conditional chaining
const METACHAR_PATTERN =
  /[;|`]|\$\(|>&|>>|<<|&&|\|\||(?:^|\s)>(?:\s|$)|(?:^|\s)<(?:\s|$)/;

/**
 * True when Settings → Agent Terminal allowlist includes a lone `*` entry
 * (after trim). Any command basename is then allowed; blocklist and
 * metacharacter rules still apply. Not glob syntax — only the exact token `*`.
 */
function agentTerminalAllowlistPermitsAll(terminal) {
  const list = terminal?.allowlist;
  if (!Array.isArray(list)) return false;
  return list.some((c) => String(c ?? "").trim() === "*");
}

/**
 * Whether `basename` (already normalized: lower case, no path) is allowed by
 * agentTerminal.allowlist. Supports:
 * - exact token (e.g. `npm`)
 * - simple globs: only `*` and `?` (e.g. `git*`, `n?pm`); no `[]`, `{}`, or ranges
 * - lone `*` entry → handled by {@link agentTerminalAllowlistPermitsAll} first
 */
function terminalAllowlistMatchesBasename(terminal, basename) {
  const norm = String(basename || "")
    .trim()
    .toLowerCase();
  if (!norm) return false;
  if (agentTerminalAllowlistPermitsAll(terminal)) return true;
  const list = (terminal?.allowlist || [])
    .map((c) =>
      String(c ?? "")
        .trim()
  ...
  for (const entry of list) {
    ...
    reSrc += "$";
    try {
      if (new RegExp(reSrc, "i").test(norm)) return true;
    } catch {
...
/**
 * Resolved spawn shape + audit/confirm labeling after validateCommand succeeds.
 */
function terminalSpawnFromValidation(validation, command, cmdArgs) {
  const logLine = [command, ...cmdArgs].join(" ").trim();
  if (!validation.invokeShell) {
    return {
      useWinJoinedLine: false,
      spawnFile: command,
      spawnArgs: cmdArgs,
      spawnShell: process.platform === "win32",
      logLine,
  ...
  const line = validation.joinedLine;
  if (validation.win) {
    return {
      useWinJoinedLine: true,
      spawnFile: null,
      spawnArgs: null,
      spawnShell: true,
      logLine: line,
...
/** Basename looks like source / data, not a typical argv0 executable (avoids ENOENT spam). */
function sourceFileAsExecutableReason(basename) {
  const b = String(basename || "").trim();
  if (!b.includes(".")) return null;
  if (/\.(exe|cmd|bat|com|msi|ps1|dll|dylib|so|app)$/i.test(b)) return null;
  if (
    /\.(swift|rs|java|kt|kts|c|cc|cpp|cxx|h|hpp|cs|ts|tsx|jsx|vue|svelte|py|rb|php|pl|go|sql|md|json|yaml|yml|toml|gradle|properties|js|mjs|cjs)$/i.test(
      b,
    )
  ) {
    return `"${basename}" looks like a source or document file, not a shell executable. Use an interpreter (swift, bash, python3, node, npm, etc.) or a real binary name; put file paths in args.`;
  }
  return null;
}
...
function validateCommand(command, args, config) {
  ...
  const allowAll = agentTerminalAllowlistPermitsAll(terminal);
  const shellInvokerAllowed = (win) =>
    allowAll ||
    (win
      ? terminalAllowlistMatchesBasename(terminal, "cmd")
      : terminalAllowlistMatchesBasename(terminal, "bash") ||
        terminalAllowlistMatchesBasename(terminal, "sh"));
  ...
  const joinedLine = [command, ...args].join(" ").trim();

  const allowShellChaining = terminal.allowShellChaining !== false;
  const invokeShell = allowShellChaining && METACHAR_PATTERN.test(joinedLine);
  ...
  if (/[\n\r{}]/.test(cmdStr)) {
    return {
      allowed: false,
      reason:
        "Command contains newlines or braces — likely malformed JSON in the command field. Use a plain executable name and put arguments in args only.",
      action:
        'ACTION: Emit TOOL_CALL with JSON like {"command":"find","args":[".","-name","*.png"]}. Do not paste a whole JSON object into command.',
    };
  }
  ...
  const srcReason = sourceFileAsExecutableReason(
  ...
  if (srcReason) {
    return {
      ...
      reason: srcReason,
      action:
        "ACTION: Use an allowlisted interpreter as command and pass the file path in args, or run a build tool (swift build, npm test, etc.). Do not retry with the same argv0.",
    };
  }

  // Allowlist check (empty = deny all; lone `*` = allow any basename, see agentTerminalAllowlistPermitsAll)
  const allowlist = (terminal.allowlist || [])
    .map((c) =>
      String(c ?? "")
        .trim()
        .toLowerCase(),
  ...
  if (!allowAll && allowlist.length === 0) {
    return {
      ...
      action:
        "ACTION: Tell the user the allowlist is empty and they need to add commands in Settings → Agent Terminal → Allowlist (or add * to allow all commands — blocklist still applies). Do not retry.",
    };
  }

  const fullCmdForBlocklist = joinedLine;

  // Blocklist check on entire line (token-boundary match — see commandContainsBlockedToken)
  const blocklist = terminal.blocklist || [];
  for (const blocked of blocklist) {
    if (commandContainsBlockedToken(fullCmdForBlocklist, blocked)) {
      const blockedLc = String(blocked).toLowerCase();
      const rmRfExtra =
        blockedLc === "rm -rf"
          ? " Safer options: use swift package clean or xcodebuild clean instead of rm -rf .build, or delete the build folder outside the agent (Finder / terminal outside Code Companion)."
          : "";
      return {
        allowed: false,
        reason: `Command blocked by security policy: "${blocked}"`,
        action: `ACTION: This command matches the user's blocklist entry "${blocked}" — it is intentionally rejected for safety. Do not retry. Tell the user the operation was blocked by their security policy and propose a safer alternative.${rmRfExtra}`,
      };
    }
  }

  // Shell chaining: delegate to bash/cmd so && | ; redirects work as the user intends.
  if (invokeShell) {
    const win = process.platform === "win32";
    if (!shellInvokerAllowed(win)) {
      return {
        allowed: false,
        reason: win
          ? 'Shell chaining (&& ; | redirections) requires "cmd" (or a matching allowlist pattern such as `cmd*`) in the agent terminal allowlist on Windows.'
          : 'Shell chaining (&& ; | redirections) requires "bash" or "sh" (or a matching pattern such as `bash*`) in the agent terminal allowlist.',
        action: win
          ? "ACTION: Tell the user to add `cmd` or a pattern that matches it (e.g. `cmd*`) to Settings → Agent Terminal → Allowlist, or disable chaining and rewrite as separate tool calls."
          : 'ACTION: Tell the user to add `bash` or `sh` (or a pattern like `bash*`) to Settings → Agent Terminal → Allowlist, or disable "Allow shell chaining" and split the command.',
      };
    }
    return {
      allowed: true,
  ...
  if (!terminalAllowlistMatchesBasename(terminal, basename)) {
    return {
      allowed: false,
      reason: `Command "${basename}" is not in the allowlist. Allowed entries: ${allowlist.join(", ")}`,
      action: `ACTION: Tell the user verbatim "Add '${basename}' to Settings → Agent Terminal → Allowlist (or a pattern like git*) if you want this command to run." Do NOT retry "${basename}" or close variants. Either propose a different approach using an allowed command or pattern, or stop and wait for the user.`,
    };
  }

  // No shell redirect: disallow metacharacters inside argv (posix spawn semantics)
  for (const arg of args) {
    if (METACHAR_PATTERN.test(arg)) {
      return {
        ...
        action:
          "ACTION: Arguments must not contain shell metacharacters when not using a shell line. Use Settings → Agent Terminal → Allow shell chaining (runs the full line via bash/cmd) and add `bash` (macOS/Linux) or `cmd` (Windows) to the allowlist; or pass `cwd` instead of `cd ... &&`; split into separate tool calls; do NOT retry the same argv shape.",
      };
    }
  }

  return { allowed: true, invokeShell: false };
}
...
function validateCwd(cwd, projectFolder) {
  ...
  try {
    ...
    if (
      ...
      return {
        valid: false,
        reason: `Working directory "${cwd}" is outside the project folder (${resolvedProject}). Set Settings → Project folder to this repository or a parent directory so agent paths and terminal CWD stay within that root.`,
      };
    }
...
const BUILTIN_TOOLS = [
  {
    ...
    description:
      'Run a terminal command under the configured project/File Browser cwd. Uses one executable plus args by default; when "Allow shell chaining" is enabled (Settings → Agent Terminal), a line that contains shell metacharacters runs via bash -lc (Unix) or cmd (Windows) — add bash/sh or cmd to the allowlist for those lines. Args: {"command": "npm", "args": ["test"], "cwd": "(optional subdirectory)", "timeoutMs": 60000}. Returns stdout/stderr and exit code. ' +
  ...
  },
  {
    name: "list_dir",
    description:
      'List a project subdirectory as a compact, depth-limited tree. Prefer this over shelling `ls`/`find` via run_terminal_cmd to discover structure — it returns a bounded tree (cheaper, survives context trimming, and works even when the terminal tool is off). Args: {"path": "<dir relative to the File Browser folder; default \\".\\">", "depth": <1-4, default 2>}. Read-only; scoped to the project folder; skips node_modules/.git/build output.',
  ...
  },
  {
    name: "read_file",
    description:
      'Read a text file under the active File Browser folder (Settings project folder is the outer permission boundary). Prefer this over `cat`/`head` via run_terminal_cmd — parallel-safe, read-only, 500KB limit with truncation for larger files. Args: {"path": "src/app.js"}. Path is relative to the File Browser folder unless absolute within the project tree. For PDFs or Office docs use view_pdf_pages or generate_office_file with sourcePath.',
    requiresTerminal: false,
    parallelSafe: true,
  },
  {
    name: "search_files",
    description:
      'Search file CONTENTS across the project for text or a regex — like grep/ripgrep, but parallel-safe and without needing the terminal. Returns matching `file:line: snippet` results. Args: {"query": "TODO", "path": "src" (optional subdir), "regex": false, "caseSensitive": false, "maxResults": 100}. Prefer this over `grep`/`rg` via run_terminal_cmd to find where a symbol or string lives. Read-only; scoped to the File Browser folder; skips node_modules/.git/build dirs and binary files.',
    requiresTerminal: false,
    parallelSafe: true,
  },
  {
    name: "find_files",
    description:
      'Find files by name/glob pattern — like `find`/glob, parallel-safe, no terminal needed. Args: {"pattern": "**/*.test.js", "path": "src" (optional subdir), "maxResults": 300}. Glob syntax: `*` (any chars except /), `**` (any directories), `?` (one char). "*.md" matches that filename anywhere; "**/*.md" matches by full path. Returns paths relative to the File Browser folder. Skips node_modules/.git/build dirs.',
    requiresTerminal: false,
    parallelSafe: true,
  },
  {
  ...
  },
  {
    name: "edit_file",
    description:
      "Make a TARGETED edit to an existing text file without rewriting the whole thing. Two modes: " +
      'REPLACE — {"path": "src/app.js", "oldText": "<exact text incl. whitespace>", "newText": "<replacement>", "replaceAll": false}; oldText must match exactly and be unique (or pass replaceAll). ' +
      'APPEND — {"path": "journal/2026-05-25.md", "append": "\\n- new line"}; appends to the end (creates the file if missing). ' +
      "Use this instead of shell redirection (>/>>) or re-sending an entire file via write_file. Creates a .backup first. For brand-new files use write_file.",
    requiresTerminal: false,
    parallelSafe: false, // filesystem mutation
  },
  {
    name: "move_file",
    description:
      'Move or rename a single file within the project. Args: {"from": "old/name.js", "to": "new/name.js"} (relative to the File Browser folder). Creates parent dirs as needed; backs up the destination if it already exists. Files only (not directories); both paths must stay under the project folder.',
    requiresTerminal: false,
    parallelSafe: false, // filesystem mutation
  },
  {
    name: "delete_file",
    description:
      'Delete a single file within the project. Args: {"path": "tmp/scratch.txt"}. Keeps a recoverable <file>.backup copy. Files only (not directories); path must stay under the project folder. Only delete files the user asked you to remove.',
    requiresTerminal: false,
    parallelSafe: false, // filesystem mutation
  },
  {
  ...
  },
  {
    name: "fetch_url",
    description:
      "Fetch a public web page or API as TEXT (no browser session) — lighter than browse_url for reading docs, articles, or JSON. " +
      "Returns HTTP status, page title, and extracted text (HTML stripped), truncated to maxChars. " +
      'Args: {"url": "https://example.com", "maxChars": 10000, "timeoutMs": 20000}. ' +
      "Public http(s) only — localhost/private IPs are blocked (use run_terminal_cmd for local checks). For pages that need clicking or JS interaction, use browse_url instead.",
    requiresBrowser: true,
    parallelSafe: true,
  },
  {
    ...
    description:
      ...
      'Optional: "filename", "images" (array, max 10, single-file only). Pass "sourcePath" as a file or a folder under the File Browser tree (folder = multi-file review like Review mode; same limits as UI: text files, size caps). Example file: TOOL_CALL: builtin.review_run({"model": "auto", "sourcePath": "src/app.js"}). Example folder: TOOL_CALL: builtin.review_run({"model": "auto", "sourcePath": "src/components"})',
  ...
  },
  {
    name: "pentest_scan",
    description:
      "OWASP-style security assessment (same pipeline as Security mode). " +
      'Requires "model" and either "code" or "sourcePath". ' +
      "Optional filename, images (single-file only — omit images when sourcePath is a folder). " +
      'sourcePath may be a file or a folder under the File Browser tree (folder scan matches Security mode caps; use pentest_scan_folder with explicit "folder" if you prefer). ' +
      'Example file: TOOL_CALL: builtin.pentest_scan({"model": "auto", "sourcePath": "src/app.js"}). Example folder: TOOL_CALL: builtin.pentest_scan({"model": "auto", "sourcePath": "src/components"})',
...
FILE READING (builtin.read_file):
- Use this tool to read source files, configs, logs, and docs before editing or answering questions about code.
- Prefer this over \`cat\`/\`head\` via run_terminal_cmd — read-only, parallel-safe, and bounded (500KB max; larger files are truncated with a notice).
- Pair with builtin.list_dir to discover structure, then read_file on specific paths.
- Path is relative to the File Browser folder; do not use ".." to escape outside it.
- Example: TOOL_CALL: builtin.read_file({"path": "src/App.jsx"})
- For PDF/binary documents, use view_pdf_pages or generate_office_file with sourcePath instead of read_file.
...
- Do NOT create Markdown files through terminal redirection when write_file fits; otherwise shell chaining may run redirections only if chaining is enabled and bash/cmd are allowlisted — prefer builtin.write_file for project docs.
- To APPEND a line to a file (journal/log/changelog), use builtin.edit_file with {"append": "..."} — do NOT use shell redirection (\`>\`/\`>>\`) or command substitution (\`$(...)\`) via run_terminal_cmd (those are blocked unless shell chaining is enabled and are a common dead-end). Embed dynamic values like the current date as literal strings (the date/time is provided to you in this prompt) rather than shelling \`$(date)\`.
...
CODE NAVIGATION & EDITING (these work even when the terminal tool is off):
- Find WHERE something is: builtin.search_files({"query": "...", "path": "src"}) for contents (grep-like), builtin.find_files({"pattern": "**/*.jsx"}) for filenames. Prefer these over \`grep\`/\`rg\`/\`find\` via run_terminal_cmd — they're parallel-safe and don't need the terminal.
- READ before you edit: builtin.read_file({"path": "..."}) (prefer over \`cat\`/\`head\`). builtin.list_dir({"path": "."}) to see structure.
- EDIT an existing file: builtin.edit_file — {"path","oldText","newText"} for a targeted replace (oldText must be exact and unique; add context or "replaceAll": true), or {"path","append":"..."} to append. Prefer this over rewriting the whole file with write_file; it's cheaper and safer on large files.
- CREATE a new file: builtin.write_file. RENAME/MOVE: builtin.move_file({"from","to"}). DELETE: builtin.delete_file({"path"}) (keeps a .backup). All stay within the project folder.
`;
...
- Prefer read-only commands first (git status, ls) before write operations; use builtin.read_file and builtin.list_dir instead of cat/find when exploring code.
- By default, chained shell lines (&&, ||, ;, |, redirects, substitutions) run as one line via bash -lc on macOS/Linux or cmd on Windows when "Allow shell chaining" is on (General → Agent Terminal). You must have \`bash\` or \`sh\` on the allowlist (or \`cmd\` on Windows) for those lines to run; the allowlist still restricts simple (non-chained) commands to a single binary + argv.
- When shell chaining is OFF, run_terminal_cmd uses a single executable plus argv only — no pipes or \`cd x && …\`; use the \`cwd\` argument for directory instead. Stdout and stderr are both returned — you do not need \`2>&1\`.
...
- For multi-statement Python: write a short script to a file with builtin.write_file, then run it with "python3 path/to/script.py". Multi-statement \`python3 -c\` may route through a shell when chaining is on (\`bash\` must be allowlisted); a script file avoids fragile quoting and keeps intent obvious.
...
- builtin.review_run — Same AI review as Review mode. Pass model + inline code, or sourcePath to a **file** or **folder** (relative paths resolve from the File Browser folder; must stay under the Settings project folder). Folder reviews scan nested text files with the same limits as Review mode folder review.
- builtin.pentest_scan — OWASP-style security scan (not remediation). Model + inline code, or sourcePath to a **file** or **folder** (folder = same multi-file scan as Security mode; no images on folder). For an explicit folder arg, use pentest_scan_folder.
...
- builtin.fetch_url — Fetch a public page/API as TEXT (no browser session) — use this first for reading docs/articles/JSON; it's lighter than browse_url. Public http(s) only.
  Example: TOOL_CALL: builtin.fetch_url({"url": "https://example.com/docs"})

- builtin.browse_url — Open a URL, load the page, return title/text/screenshot. Starts (or replaces) the browser session. Use when you need to click/type/scroll or render a JS-heavy page (otherwise prefer fetch_url).
...
async function executeBuiltinTool(
  ...
  if (toolName === "list_dir") {
    return listDirTool(args, config);
  }

  if (toolName === "read_file") {
    return readFileTool(args, config, log);
  }

  if (toolName === "search_files") {
    return searchFilesTool(args, config);
  }

  if (toolName === "find_files") {
    return findFilesTool(args, config);
  }

  if (toolName === "edit_file") {
    return editFileTool(args, config, log);
  }

  if (toolName === "move_file") {
    return moveFileTool(args, config, log);
  }

  if (toolName === "delete_file") {
    return deleteFileTool(args, config, log);
  }

  if (toolName === "fetch_url") {
    return fetchUrlTool(args, config, log);
  }
...
async function runTerminalCmd(args, config, log, context) {
  ...
  const normalizeArgList = (raw) => {
    ...
    if (typeof raw === "string") {
      ...
      try {
        ...
        /* keep raw string as one argv token */
      }
      return [raw];
    }
  ...
  let command = args.command;
  let cmdArgs = normalizeArgList(args.args);
  if (!command && typeof args.input === "string" && args.input.trim()) {
  ...
  const spawnPlan = terminalSpawnFromValidation(validation, command, cmdArgs);
  ...
  if (terminal.confirmBeforeRun) {
    ...
    try {
      const decision = await context.confirmCallback({
        command: spawnPlan.confirmCommand,
        args: spawnPlan.confirmArgs,
        cwd: cwdCheck.resolved,
      });
  ...
  log(
    "INFO",
    `${LOG_PREFIX} Executing: ${spawnPlan.logLine}${isBackground ? " (background)" : ""}`,
    {
      cwd: cwdCheck.resolved,
  ...
  return new Promise((resolve) => {
    ...
    const proc = spawnPlan.useWinJoinedLine
      ? spawn(spawnPlan.confirmCommand, {
          ...spawnOpts,
          shell: true,
        })
      : spawn(spawnPlan.spawnFile, spawnPlan.spawnArgs, {
          ...spawnOpts,
          shell: spawnPlan.spawnShell,
        });
    ...
    proc.on("error", (err) => {
      ...
      log("ERROR", `${LOG_PREFIX} Spawn error: ${err.message}`, {
        command: spawnPlan.logLine,
        duration,
      });
      auditTerminalEvent({
        event: "spawn-error",
        command: spawnPlan.auditCommand,
        args: spawnPlan.auditArgs,
        cwd: cwdCheck.resolved,
        durationMs: duration,
        error: err.message,
      });
      resolve({
        ...
        result: {
          content: [
            {
              type: "text",
              text: `Error spawning command "${spawnPlan.logLine}": ${err.message}`,
            },
          ],
        },
      });
    ...
    proc.on("close", (code) => {
      ...
      log(
        "INFO",
        `${LOG_PREFIX} Finished: ${spawnPlan.logLine} exit=${code} duration=${duration}ms output=${(cleanOutput.length / 1024).toFixed(1)}KB${killed ? " (killed)" : ""}`,
      );
      auditTerminalEvent({
        event: "executed",
        command: spawnPlan.auditCommand,
...
function runTerminalCmdBackground({
  ...
  return new Promise((resolve) => {
    ...
    let proc;
    try {
      proc = spawnPlan.useWinJoinedLine
        ? spawn(spawnPlan.confirmCommand, { ...spawnOpts, shell: true })
        : spawn(spawnPlan.spawnFile, spawnPlan.spawnArgs, {
            ...spawnOpts,
            shell: spawnPlan.spawnShell,
          });
      ...
      return resolve({
        ...
        result: {
          content: [
            {
              type: "text",
              text: `Error spawning background command "${spawnPlan.logLine}": ${err.message}`,
            },
          ],
        },
      });
    ...
    proc.on("close", (code) => {
      ...
      log(
        "INFO",
        `${LOG_PREFIX} Background finished: ${spawnPlan.logLine} pid=${proc.pid} exit=${code} duration=${duration}ms output=${(entry.output.length / 1024).toFixed(1)}KB${entry.killed ? " (killed)" : ""}`,
      );
    ...
    const startupTimer = setTimeout(() => {
      ...
      const cleanOutput = stripAnsi(entry.output);
      const cmdLine = [spawnPlan.confirmCommand, ...spawnPlan.confirmArgs].join(
        " ",
      );
      resolve({
        success: true,
...
// ── List directory tool (compact, read-only tree) ───────────────────────
const _LIST_DIR_IGNORE = new Set([
  "node_modules",
  ".git",
  "dist",
  "build",
...
/**
 * Return a compact, depth-limited directory tree so the agent can discover
 * structure without shelling `ls`/`find` (whose large output gets capped and
 * forces re-listing). Read-only; resolved under the File Browser folder and
 * bounded by the project folder. Symlinked dirs are not traversed (Dirent type),
 * so there's no escape or loop risk.
 */
async function listDirTool(args, config) {
  ...
  const root = getAgentInteractionRoot(config);
  if (!root) {
    return {
      success: false,
      result: {
        content: [
          {
            type: "text",
            text: "Error: Could not resolve File Browser / project folder.",
          },
        ],
      },
    };
  ...
  if (target !== boundaryNorm && !target.startsWith(boundaryNorm + path.sep)) {
    return {
      success: false,
      result: {
        content: [
  ...
  try {
    ...
    return {
      success: false,
      result: { content: [{ type: "text", text: `Error: Not found: ${rel}` }] },
    };
  }
  if (!st.isDirectory()) {
    return {
      success: false,
      result: {
        content: [{ type: "text", text: `Error: Not a directory: ${rel}` }],
      },
    };
  }
  ...
  const walk = (dir, prefix, depth) => {
    ...
    for (const e of entries) {
      if (_LIST_DIR_IGNORE.has(e.name)) continue;
  ...
  walk(target, "", 1);
  const header = `Directory tree of "${rel}" (depth ${maxDepth}${
    truncated
      ? `, truncated at ${MAX_ENTRIES} entries — list a subdirectory for more`
      : ""
  }):`;
  return {
    success: true,
...
// ── Read file tool ────────────────────────────────
async function readFileTool(args, config, log) {
  const filePath =
    args?.path ??
    args?.filepath ??
    args?.file_path ??
        ...
        content: [
          {
            type: "text",
            text:
              'Error: builtin.read_file needs "path" (file path relative to the File Browser folder). ' +
              'Example: TOOL_CALL: builtin.read_file({"path": "src/app.js"})',
          },
        ],
      },
    };
  ...
  const readRoot = getAgentInteractionRoot(config);
  if (!readRoot) {
    return {
      success: false,
      result: {
  ...
  const resolved = path.resolve(readRoot, filePath.trim());
  const boundaryNorm = path.resolve(boundaryRoot);
  const readRootNorm = path.resolve(readRoot);
  ...
  if (
    resolved !== readRootNorm &&
    !resolved.startsWith(readRootNorm + path.sep)
  ) {
    return {
      success: false,
      result: {
  ...
  if (!fs.existsSync(resolved)) {
    return {
      ...
      result: {
        content: [
          { type: "text", text: `Error: File not found: ${filePath.trim()}` },
        ],
      },
    };
  }
    ...
    return {
      ...
      result: {
        content: [
          {
            type: "text",
            text: `Error: Path is not a file: ${filePath.trim()}`,
          },
        ],
      },
    };
    ...
    return {
      ...
      result: {
        content: [
          {
            type: "text",
            text: `Error: "${baseName}" looks like a binary/media file. Use builtin.view_pdf_pages for PDF visuals or builtin.generate_office_file with sourcePath to extract document text.`,
          },
        ],
      },
    };
  ...
  try {
    const relPath = path.relative(readRootNorm, resolved);
    const result = readProjectFile(readRootNorm, relPath);
    const header = `File: ${result.name} (${result.lines} lines, ${result.size} bytes${result.truncated ? ", TRUNCATED" : ""})\n\n`;
    log("INFO", `[READ_FILE] Read ${resolved} (${result.size} bytes)`);
    return {
      success: true,
      result: {
...
async function writeFileTool(args, config, log) {
  // Tolerate the common arg-name variations models emit so a near-miss call
  // (e.g. {filename, text}) doesn't hard-fail — mirrors run_terminal_cmd's
  // acceptance of `args.input` as a `command` alias.
  const filePath =
    args.path ?? args.filepath ?? args.file_path ?? args.filename ?? args.file;
  const content = args.content ?? args.text ?? args.body ?? args.contents;
        ...
        content: [
          {
            type: "text",
            text:
              'Error: builtin.write_file needs "path" (file path relative to the File Browser folder) and "content" (the full file text). ' +
              'Retry with exactly those keys, e.g. TOOL_CALL: builtin.write_file({"path": "docs/notes.md", "content": "..."}).',
          },
        ],
      },
    };
...
// ── Shared path resolver for write-class file tools ───────────────
// Mirrors the boundary checks used by read_file/write_file: permission boundary
// is config.projectFolder; relative paths resolve from the File Browser root.
// Lexical resolution (path.resolve) + startsWith catches ".." escapes.
// Returns { ok:true, resolved, root, boundary } or { ok:false, error }.
function resolveAgentWritePath(config, userPath) {
  const boundaryRoot = config.projectFolder;
  if (!boundaryRoot) return { ok: false, error: "No project folder set." };
  ...
  if (!root) {
    return {
      ok: false,
      error: "Could not resolve File Browser / project folder.",
    };
  }
  const resolved = path.resolve(root, userPath.trim());
  const boundaryNorm = path.resolve(boundaryRoot);
  ...
  if (
    ...
    return { ok: false, error: "Path is outside the project folder." };
  }
  if (resolved !== rootNorm && !resolved.startsWith(rootNorm + path.sep)) {
    return {
      ok: false,
      error:
        'Path escapes the active File Browser folder. Use a relative path without ".." to parent directories, or open the parent folder in the File Browser.',
    };
  }
  return { ok: true, resolved, root: rootNorm, boundary: boundaryNorm };
}

/** Wrap text in the MCP-compatible result envelope these tools return. */
function _agentText(success, text) {
  return { success, result: { content: [{ type: "text", text }] } };
}

// ── Search file contents (grep-like) ──────────────
async function searchFilesTool(args, config) {
  const query = args?.query ?? args?.pattern ?? args?.q ?? args?.search;
  if (!query || typeof query !== "string" || !query.trim()) {
    return _agentText(
      false,
      'Error: builtin.search_files needs "query" (text or regex to find). ' +
        'Example: TOOL_CALL: builtin.search_files({"query": "TODO", "path": "src"})',
    );
  }
  const subdir =
    typeof args?.path === "string" && args.path.trim() ? args.path.trim() : ".";
  const r = resolveAgentWritePath(config, subdir);
  if (!r.ok) return _agentText(false, `Error: ${r.error}`);
  let st;
  try {
    st = fs.statSync(r.resolved);
  } catch {
    return _agentText(false, `Error: Not found: ${subdir}`);
  }
  if (!st.isDirectory())
    return _agentText(false, `Error: Not a directory: ${subdir}`);

  const useRegex = args?.regex === true || args?.useRegex === true;
  const caseSensitive = args?.caseSensitive === true;
  const maxResults = Math.min(
    500,
    Math.max(1, Number(args?.maxResults) || 100),
  );
  ...
  if (useRegex) {
    try {
      ...
      return _agentText(false, `Error: invalid regex: ${e.message}`);
    }
  } else {
  ...
  const walk = (dir) => {
    ...
    for (const e of entries) {
      ...
      if (!e.isFile() || !isTextFile(e.name)) continue;
      ...
      const rel = path.relative(r.root, full).split(path.sep).join("/");
  ...
  const label = useRegex ? `/${query}/` : `"${query}"`;
  if (matches.length === 0) {
    return _agentText(
      true,
      `No matches for ${label} in "${subdir}" (${filesScanned} text files scanned).`,
    );
  }
  const header = `${matches.length} match(es) for ${label} in "${subdir}"${
    truncated
      ? ` (truncated at ${maxResults} — narrow the query or path for more)`
      : ""
  }:`;
  return _agentText(true, `${header}\n${matches.join("\n")}`);
}

// ── Find files by glob pattern ─────────────────────
// Minimal glob → RegExp: ** = any path segments, * = any chars except "/",
// ? = single char except "/". Anchored full-match.
...
async function findFilesTool(args, config) {
  const pattern = args?.pattern ?? args?.glob ?? args?.name;
  if (!pattern || typeof pattern !== "string" || !pattern.trim()) {
    return _agentText(
      false,
      'Error: builtin.find_files needs "pattern" (a glob like "**/*.test.js" or "*.md"). ' +
        'Example: TOOL_CALL: builtin.find_files({"pattern": "**/*.jsx", "path": "src"})',
    );
  }
  const subdir =
    typeof args?.path === "string" && args.path.trim() ? args.path.trim() : ".";
  ...
  let re;
  try {
    re = _globToRegExp(pattern.trim());
  } catch (e) {
    return _agentText(false, `Error: invalid pattern: ${e.message}`);
  }
  const maxResults = Math.min(
    1000,
    Math.max(1, Number(args?.maxResults) || 300),
  );
  const out = [];
  let truncated = false;
  const walk = (dir) => {
    ...
    for (const e of entries) {
      ...
      const relToSearch = path
        .relative(r.resolved, full)
        .split(path.sep)
        .join("/");
      if (re.test(relToSearch) || re.test(e.name)) {
        out.push(path.relative(r.root, full).split(path.sep).join("/"));
  ...
  if (out.length === 0)
    return _agentText(true, `No files match "${pattern}" under "${subdir}".`);
  const header = `${out.length} file(s) matching "${pattern}" under "${subdir}"${
    truncated ? ` (truncated at ${maxResults})` : ""
  }:`;
  return _agentText(true, `${header}\n${out.join("\n")}`);
}

// ── Targeted edit / append ─────────────────────────
async function editFileTool(args, config, log) {
  const filePath =
    args?.path ??
    args?.filepath ??
    args?.file_path ??
    args?.filename ??
    args?.file;
  const r = resolveAgentWritePath(config, filePath);
  if (!r.ok) {
    return _agentText(
      false,
      `Error: ${r.error}\nbuiltin.edit_file needs "path" plus either {"oldText","newText"} to replace, or {"append":"..."} to append.`,
    );
  }
  const resolved = r.resolved;
  const rel = path.relative(r.root, resolved);

  const appendText = args?.append ?? args?.append_text ?? args?.appendText;
  const isAppend = appendText != null;

  try {
    if (isAppend) {
      const existed = fs.existsSync(resolved);
      if (existed && !fs.statSync(resolved).isFile()) {
        return _agentText(false, "Error: Path is not a file.");
      }
      let toWrite = String(appendText);
      if (existed) {
        fs.copyFileSync(resolved, resolved + ".backup");
        const cur = fs.readFileSync(resolved, "utf8");
        // Ensure a separating newline if the file doesn't already end with one.
        if (cur.length && !cur.endsWith("\n") && !toWrite.startsWith("\n")) {
          toWrite = "\n" + toWrite;
        }
        fs.appendFileSync(resolved, toWrite, "utf8");
      } else {
        fs.mkdirSync(path.dirname(resolved), { recursive: true });
        fs.writeFileSync(resolved, toWrite, "utf8");
      }
      const bytes = Buffer.byteLength(toWrite);
      const sizeKb = (fs.statSync(resolved).size / 1024).toFixed(1);
      log("INFO", `[EDIT_FILE] Appended ${bytes} bytes to ${resolved}`);
      return _agentText(
        true,
        `Appended ${bytes} bytes to ${rel} (now ${sizeKb} KB).${existed ? ` Backup: ${rel}.backup` : " (new file)"}`,
      );
    }
    ...
    if (oldText == null || newText == null) {
      return _agentText(
        false,
        'Error: builtin.edit_file replace mode needs "oldText" and "newText". To append instead, pass "append". ' +
          'Example: TOOL_CALL: builtin.edit_file({"path": "src/app.js", "oldText": "foo()", "newText": "bar()"})',
      );
    }
    if (!fs.existsSync(resolved)) {
      return _agentText(
        false,
        `Error: File not found: ${rel} (edit_file replace needs an existing file; use builtin.write_file to create one).`,
      );
    }
    if (!fs.statSync(resolved).isFile())
      return _agentText(false, "Error: Path is not a file.");
    ...
    if (!isTextFile(baseName)) {
      return _agentText(
        false,
        `Error: "${baseName}" looks like a binary/media file — edit_file only edits text files.`,
      );
    }
    const oldStr = String(oldText);
    if (oldStr === "")
      return _agentText(false, 'Error: "oldText" cannot be empty.');
    ...
    if (count === 0) {
      return _agentText(
        false,
        `Error: "oldText" not found in ${rel}. The match must be exact (including whitespace/indentation). Read the file first with builtin.read_file.`,
      );
    }
    if (count > 1 && !replaceAll) {
      return _agentText(
        false,
        `Error: "oldText" matches ${count} places in ${rel}. Add surrounding context to make it unique, or pass "replaceAll": true.`,
      );
    }
    fs.copyFileSync(resolved, resolved + ".backup");
    // Literal replacement (no $-pattern interpretation): indexOf/slice for one
    // match, split/join for replaceAll.
    let updated;
    if (replaceAll) {
      updated = original.split(oldStr).join(String(newText));
    } else {
      const at = original.indexOf(oldStr);
      updated =
        original.slice(0, at) +
        String(newText) +
        original.slice(at + oldStr.length);
    }
    fs.writeFileSync(resolved, updated, "utf8");
    const replaced = replaceAll ? count : 1;
    log(
      "INFO",
      `[EDIT_FILE] Replaced ${replaced} occurrence(s) in ${resolved}`,
    );
    return _agentText(
      true,
      `Replaced ${replaced} occurrence(s) in ${rel}. Backup: ${rel}.backup`,
    );
  } catch (err) {
    log("ERROR", `[EDIT_FILE] Failed: ${err.message}`);
    return _agentText(false, `Error editing file: ${err.message}`);
  }
}

// ── Move / rename a file ───────────────────────────
async function moveFileTool(args, config, log) {
  const from =
    args?.from ?? args?.source ?? args?.src ?? args?.oldPath ?? args?.path;
  const to = args?.to ?? args?.dest ?? args?.destination ?? args?.newPath;
  if (!from || !to) {
    return _agentText(
      false,
      'Error: builtin.move_file needs "from" and "to" (paths relative to the File Browser folder). ' +
        'Example: TOOL_CALL: builtin.move_file({"from": "a.txt", "to": "sub/b.txt"})',
    );
  }
  const rf = resolveAgentWritePath(config, from);
  if (!rf.ok) return _agentText(false, `Error (from): ${rf.error}`);
  const rt = resolveAgentWritePath(config, to);
  if (!rt.ok) return _agentText(false, `Error (to): ${rt.error}`);
  try {
    if (!fs.existsSync(rf.resolved))
      return _agentText(false, `Error: source not found: ${from}`);
    if (!fs.statSync(rf.resolved).isFile())
      return _agentText(
        false,
        "Error: move_file only moves regular files (not directories).",
      );
    if (fs.existsSync(rt.resolved)) {
      if (fs.statSync(rt.resolved).isDirectory())
        return _agentText(
          false,
          'Error: destination is a directory — include the target filename in "to".',
        );
      fs.copyFileSync(rt.resolved, rt.resolved + ".backup");
    }
    fs.mkdirSync(path.dirname(rt.resolved), { recursive: true });
    try {
      fs.renameSync(rf.resolved, rt.resolved);
    } catch (err) {
      // Cross-device rename (EXDEV) — fall back to copy + unlink.
      if (err.code === "EXDEV") {
        fs.copyFileSync(rf.resolved, rt.resolved);
        fs.unlinkSync(rf.resolved);
      } else {
        throw err;
      }
    }
    log("INFO", `[MOVE_FILE] ${rf.resolved} -> ${rt.resolved}`);
    return _agentText(
      true,
      `Moved ${path.relative(rf.root, rf.resolved)} -> ${path.relative(rt.root, rt.resolved)}.`,
    );
  } catch (err) {
    log("ERROR", `[MOVE_FILE] Failed: ${err.message}`);
    return _agentText(false, `Error moving file: ${err.message}`);
  }
}

// ── Delete a file (keeps a .backup) ────────────────
async function deleteFileTool(args, config, log) {
  const filePath =
    args?.path ??
    args?.file ??
    args?.filepath ??
  ...
  if (!r.ok) {
    return _agentText(
      false,
      `Error: ${r.error}\nbuiltin.delete_file needs "path" (a file under the File Browser folder).`,
    );
  }
  try {
    if (!fs.existsSync(r.resolved))
      return _agentText(false, `Error: file not found: ${filePath}`);
    if (!fs.statSync(r.resolved).isFile()) {
      return _agentText(
        false,
        "Error: delete_file only deletes regular files (not directories). Use the terminal for directory removal if the user asked for it.",
      );
    }
    // Keep a recoverable copy (consistent with write_file's .backup).
    fs.copyFileSync(r.resolved, r.resolved + ".backup");
    fs.unlinkSync(r.resolved);
    log("INFO", `[DELETE_FILE] Deleted ${r.resolved} (backup kept)`);
    return _agentText(
      true,
      `Deleted ${path.relative(r.root, r.resolved)}. A recoverable copy was kept at ${path.basename(r.resolved)}.backup`,
    );
  } catch (err) {
    log("ERROR", `[DELETE_FILE] Failed: ${err.message}`);
    return _agentText(false, `Error deleting file: ${err.message}`);
  }
}

// ── Fetch a public URL as text (no browser session) ─
async function fetchUrlTool(args, config, log) {
  if (!config.agentBrowser?.enabled) {
    return _agentText(
      false,
      "Error: web access is disabled. Enable Settings → General → Agent Browser to allow builtin.fetch_url and the browser_* tools.",
    );
  }
  ...
  if (!url || typeof url !== "string" || !url.trim()) {
    return _agentText(
      false,
      'Error: builtin.fetch_url needs "url". Example: TOOL_CALL: builtin.fetch_url({"url": "https://example.com"})',
    );
  }
  const check = validateBrowseUrl(url.trim());
  if (!check.valid) return _agentText(false, `Error: ${check.reason}`);
  const maxChars = Math.min(
    50000,
    Math.max(500, Number(args?.maxChars) || 10000),
  );
  const timeoutMs = Math.min(
    60000,
    Math.max(1000, Number(args?.timeoutMs) || 20000),
  );
  ...
  try {
    const resp = await fetch(url.trim(), {
      ...
      redirect: "follow",
      headers: {
        "User-Agent": "CodeCompanion-Agent/1.0",
    ...
    log(
      "INFO",
      `${BROWSER_LOG_PREFIX} [FETCH_URL] ${resp.status} ${url} (${text.length} chars${truncated ? ", truncated" : ""})`,
    );
    const header = `Fetched ${url} — HTTP ${resp.status}${title ? `\nTitle: ${title}` : ""} (${ctype.split(";")[0] || "?"})${truncated ? `, truncated to ${maxChars} chars` : ""}:`;
    return _agentText(true, `${header}\n\n${text || "(empty body)"}`);
  } catch (err) {
    clearTimeout(timer);
    const msg =
      err.name === "AbortError"
        ...
        : err.message;
    log("ERROR", `${BROWSER_LOG_PREFIX} [FETCH_URL] Failed: ${msg}`);
    return _agentText(false, `Error fetching URL: ${msg}`);
  }
}

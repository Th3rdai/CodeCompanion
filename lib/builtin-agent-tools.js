...
- To APPEND to a file (e.g. add a journal/log line), do NOT use shell redirection (\`>\`/\`>>\`) or command substitution (\`$(...)\`) via run_terminal_cmd — those are blocked unless shell chaining is enabled and are a common dead-end. Instead read the current contents first (e.g. \`cat <file>\` via run_terminal_cmd, or use what's already shown in context), then call builtin.write_file with the full updated text. Embed dynamic values like the current date as literal strings (the date/time is provided to you in this prompt) rather than shelling \`$(date)\`.

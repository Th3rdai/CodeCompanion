...
function validateProjectFilePath(userPath, projectFolder) {
  ...
  try {
    ...
    if (resolvedFile !== resolvedProject && !resolvedFile.startsWith(resolvedProject + path.sep)) {
      return { valid: false, reason: 'File path must be under the project folder' };
    }
...
const BUILTIN_TOOLS = [
  ...
  {
    name: 'write_file',
    description: 'Write or overwrite a file in the project folder. Automatically creates a .backup before overwriting. Args: {"path": "restart.sh", "content": "#!/bin/bash\\n..."}. Path is relative to project folder. Returns success/failure and file size.',
...
function getBuiltinTools(config) {
  ...
  if (terminalAllowed) {
    const bindAddr = process.env.HOST || (process.env.CC_BIND_ALL === '1' ? '0.0.0.0' : '127.0.0.1');
  ...
  return BUILTIN_TOOLS
    .filter(t => !t.requiresTerminal || terminalAllowed)
    .map(t => ({
      serverId: 'builtin',
      name: t.name,
...
async function writeFileTool(args, config, log) {
  ...
  if (!filePath || content == null) {
    return { success: false, result: { content: [{ type: 'text', text: 'Error: "path" and "content" are required.' }] } };
  }

  const projectFolder = config.projectFolder;
  if (!projectFolder) {
    return { success: false, result: { content: [{ type: 'text', text: 'Error: No project folder set.' }] } };
  }
  ...
  if (!resolved.startsWith(path.resolve(projectFolder))) {
    return { success: false, result: { content: [{ type: 'text', text: 'Error: Path is outside the project folder.' }] } };
  }
  ...
  try {
    ...
    log('INFO', `[WRITE_FILE] Saved ${resolved} (${(stats.size / 1024).toFixed(1)}KB)`);
    ...
    return { success: false, result: { content: [{ type: 'text', text: `Error writing file: ${err.message}` }] } };
  }
}
...
async function generateOfficeFileTool(args, config, log) {
  ...
  const { canConvertBuiltin, convertBuiltin, BUILTIN_SUPPORTED } = require('./builtin-doc-converter');

  let content = args.content || args.input;

  if (args.sourcePath) {
    const pathCheck = validateProjectFilePath(args.sourcePath, config.projectFolder);
    if (!pathCheck.valid) {
      return {
        success: false,
        result: { content: [{ type: 'text', text: `Error (sourcePath): ${pathCheck.reason}` }] },
      };
    }
    ...
    if (!canConvertBuiltin(srcName)) {
      ...
      return {
        success: false,
        result: { content: [{ type: 'text', text: `Built-in conversion does not support ${path.extname(srcName)}. Supported extensions: ${exts}.` }] },
      };
    }
    try {
      ...
      if (conv.errors?.length) {
        log('INFO', `[OFFICE] sourcePath conversion notes for ${srcName}: ${conv.errors.join('; ')}`);
      }
    } catch (err) {
      log('ERROR', `[OFFICE] sourcePath read failed: ${err.message}`);
      return {
        success: false,
        result: { content: [{ type: 'text', text: `Error reading or converting source file: ${err.message}` }] },
      };
    }
  }

  if (!content || !String(content).trim()) {
    log('ERROR', '[OFFICE] No content (provide "content" or a readable "sourcePath")', { argKeys: Object.keys(args) });
    return {
      success: false,
      result: { content: [{ type: 'text', text: 'Error: Missing "content", or "sourcePath" did not produce text.' }] },
    };
  }
  ...
  if (!filename) {
    ...
    const firstLine = content.split('\n')[0].replace(/^#+\s*/, '').replace(/[^a-zA-Z0-9 ]/g, '').trim();
    const slug = firstLine.split(/\s+/).slice(0, 3).join('-').toLowerCase() || 'document';
  ...
  if (!SUPPORTED_FORMATS.has(ext)) {
    return {
      success: false,
      result: { content: [{ type: 'text', text: `Unsupported format: ${ext}. Supported: ${[...SUPPORTED_FORMATS].join(', ')}` }] },
    };
  }

  try {
    log('INFO', `[OFFICE] Generating ${ext}: ${filename} (${content.length} chars)`);
    ...
    const outputPath = args.savePath
      || path.join(fs.existsSync(desktop) ? desktop : (config.projectFolder || require('os').homedir()), filename);
    ...
    log('INFO', `[OFFICE] Saved ${outputPath} (${(result.size / 1024).toFixed(1)}KB)`);
    ...
    return {
      success: false,
      result: { content: [{ type: 'text', text: `Office file generation failed: ${err.message}` }] },
    };
  }
}

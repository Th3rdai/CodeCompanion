...
const BUILTIN_TOOLS = [
  ...
  },
  {
    ...
    description: 'Generate a document file from markdown content and save it to disk. Supported formats: DOCX (Word), XLSX (Excel), PPTX (PowerPoint), CSV, PDF, ODT (OpenDocument Text), ODS (OpenDocument Spreadsheet), HTML, TXT, MD, JSON. Args: {"content": "# Report\\n\\nBody with **bold**, *italic*, lists, and markdown tables.", "filename": "report.docx", "savePath": "/Users/james/Desktop/report.docx"}. For XLSX/CSV/ODS: include markdown tables (| col1 | col2 |). For PPTX: use # headings to split slides. All markdown is converted to native formatting. Returns file path and size.',
...
- When the user asks you to create, export, convert, or save a file in ANY of these formats: DOCX, XLSX, PPTX, CSV, PDF, ODT, ODS, HTML, TXT, MD, JSON — you MUST use this tool. Do NOT suggest manual alternatives.
...
- For spreadsheets (XLSX/ODS/CSV): structure data as markdown tables with | column | headers |.
...
- If converting an existing file to another format, read the file content first, then generate the new format.
...
async function generateOfficeFileTool(args, config, log) {
  ...
  // Accept content from args.content or args.input (fallback from parser)
  const content = args.content || args.input;
  if (!content) {
    log('ERROR', '[OFFICE] No content provided', { argKeys: Object.keys(args) });
    return {
      success: false,
      result: { content: [{ type: 'text', text: 'Error: Missing "content" argument.' }] },
    };
  }

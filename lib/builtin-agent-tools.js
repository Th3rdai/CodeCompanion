...
/**
 * Safety preamble for builtin tools (PDF, office, write_file, optional terminal).
...
async function viewPdfPagesTool(args, config, log) {
  ...
  if (!args.sourcePath) {
    return {
      success: false,
      result: { content: [{ type: "text", text: 'Error: "sourcePath" is required.' }] },
    };
  }
  ...
  if (path.extname(pathCheck.resolved).toLowerCase() !== ".pdf") {
    return {
      success: false,
      result: { content: [{ type: "text", text: "Error: view_pdf_pages only supports PDF files." }] },
    };
  }
  ...
  if (Array.isArray(args.pages) && args.pages.length > 0) {
    const nums = args.pages.map(Number).filter((n) => Number.isFinite(n) && n >= 1);
    if (nums.length === 0) {
      return {
        success: false,
        result: { content: [{ type: "text", text: 'Error: "pages" must be an array of positive integers.' }] },
      };
    }
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      const proc = spawn(
        "pdftoppm",
        ["-r", "120", "-png", "-f", String(firstPage), "-l", String(lastPage), pathCheck.resolved, prefix],
        { env: spawnEnv },
      );
      let stderr = "";
      proc.stderr.on("data", (d) => (stderr += d.toString()));
      proc.on("close", (code) => {
        if (code !== 0) reject(new Error(`pdftoppm failed (exit ${code}): ${stderr.slice(0, 200)}`));
        else resolve();
      });
      proc.on("error", (err) => reject(new Error(`pdftoppm not found: ${err.message}. Install poppler: brew install poppler`)));
    });
    ...
    if (files.length === 0) {
      return {
        success: false,
        result: { content: [{ type: "text", text: "No pages rendered. The PDF may be empty or the page range invalid." }] },
      };
    }

    log("INFO", `[PDF-PAGES] Rendered ${files.length} page(s) from ${path.basename(pathCheck.resolved)} (pages ${firstPage}-${firstPage + files.length - 1})`);
    ...
    return {
      success: false,
      result: { content: [{ type: "text", text: `PDF page rendering failed: ${err.message}` }] },
    };
  } finally {

 ...
 * @param {string} [input.projectFolder]            used to resolve relative paths
...
function enforceExperimentScope({
  ...
  if (tool === "write_file") {
    ...
    const baseFolder = projectFolder || allowed[0];
    const absolute = path.isAbsolute(userPath)
      ? path.resolve(userPath)
...
- If the user expects the file in a specific folder, include that folder in "path". Relative paths are resolved under the configured project folder.
...
- NEVER report "undefined error", "unknown error", "the terminal failed", or similar vague text to the user. Every deny response from run_terminal_cmd starts with literal text "Command denied:" followed by the exact reason and may include an "ACTION:" line with the next step. When you summarize a deny to the user, quote (or paraphrase) the actual reason — do not invent a vague one. If the deny was an allowlist miss, tell the user the exact command name to add (e.g. "Add 'chmod' to Settings → Agent Terminal → Allowlist"); if it was a metacharacter / shell-feature deny, tell them you cannot use that shell feature here and what you tried instead.

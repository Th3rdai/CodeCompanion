    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (depth >= maxDepth) {
          ...
          result.push({ name: entry.name, path: relativePath, type: 'dir', children: [], truncated: true });
        } else {
  ...
  function walk(dirPath, depth) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          const stat = fs.statSync(fullPath);
          if (stat.size > maxFileSize) { skipped++; continue; }
          if (totalSize + stat.size > maxTotalSize) { skipped++; continue; }

...
${toolList}${terminalSessionLine}`;
   ...
   * Explicit risky builtins: run_terminal_cmd, generate_office_file
   *
  ...
  segmentToolCalls(toolCalls) {
    ...
    const RISKY_BUILTINS = new Set(["run_terminal_cmd", "generate_office_file"]);

    const isRisky = (call) => {
      if (call.serverId === "builtin" && RISKY_BUILTINS.has(call.toolName))
        return true;

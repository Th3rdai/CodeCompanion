...
/**
 * Gemma/Gemini hallucination wrappers around tool calls — the model emits
 *   <tool_code>builtin.run_terminal_cmd({...})</tool_code>
 * or fenced
 *   ```tool_code\nbuiltin.run_terminal_cmd({...})\n```
 * instead of `TOOL_CALL: builtin.run_terminal_cmd({...})`. Inner content is a
 * valid `server.tool({...})` call but the missing `TOOL_CALL:` prefix makes
 * the primary parser miss it, so the round ends with a tool-less narration.
 */
const TOOL_CODE_TAG_BLOCK = /<tool_code>\s*([\s\S]*?)\s*<\/tool_code>/gi;
const TOOL_CODE_FENCE_BLOCK = /```tool_code\s*\n([\s\S]*?)```/gi;
...
// 300s default — image generation against Gemini Pro / 4K can legitimately
// run 60–90s+ with grounding enabled, and the 180s ceiling was killing the
// stdio transport mid-call (see 2026-05-03 timeout incident). Override via
// MCP_IMAGE_TOOL_TIMEOUT_MS env var.
const MCP_IMAGE_TOOL_TIMEOUT_MS = (() => {
  const raw = process.env.MCP_IMAGE_TOOL_TIMEOUT_MS;
  if (raw === undefined || raw === "") return 300000;
  const n = Number(raw);
  return Number.isFinite(n) ? Math.max(50, n) : 300000;
})();

/** When set on the handler instance, MCP is blocked and only these builtins run. */
const EXPERIMENT_ALLOWED_BUILTINS = new Set([
  // Read-only discovery (no scope check needed — enforceExperimentScope only
  // gates write_file/run_terminal_cmd; reads anywhere in the project are fine).
  "read_file",
  "list_dir",
  "search_files",
  "find_files",
  "view_pdf_pages",
  // Mutations / commands (scope-enforced for write_file + run_terminal_cmd).
  // edit_file/move_file/delete_file are intentionally NOT here: they bypass
  // enforceExperimentScope, so experiment changes go through scope-checked
  // write_file instead.
  "write_file",
  "run_terminal_cmd",
  "kill_process",
...
class ToolCallHandler {
  ...
  parseToolCalls(text) {
    ...
    // Fallback: <tool_code>server.tool({...})</tool_code> or fenced
    // ```tool_code\nserver.tool({...})\n``` (Gemma/Gemini hallucination wrappers).
    // Inside the wrapper the call is well-formed; we just need to peel the
    // wrapper off and run the same balanced-args extraction the primary parser
    // uses, so JSON with parens/newlines is preserved correctly.
    if (calls.length === 0) {
      const wrapperPatterns = [
        new RegExp(TOOL_CODE_TAG_BLOCK.source, "gi"),
        new RegExp(TOOL_CODE_FENCE_BLOCK.source, "gi"),
      ];
      for (const wrapRe of wrapperPatterns) {
        let wm;
        while ((wm = wrapRe.exec(text)) !== null) {
          const inner = (wm[1] || "").trim();
          if (!inner) continue;
          // Inner should look like `server.tool(...)` — locate the header.
          const headerMatch = inner.match(/^(\S+?)\.(\S+?)\(/);
          if (!headerMatch) continue;
          try {
            const argsStart = headerMatch[0].length;
            const { args: rawArgs } = this._extractBalancedArgs(
              inner,
              argsStart,
            );
            const args = this._parseArgs(rawArgs);
            calls.push({
              serverId: headerMatch[1],
              toolName: headerMatch[2],
              args,
            });
            this.debug("Parsed <tool_code> wrapped call", {
              tool: `${headerMatch[1]}.${headerMatch[2]}`,
            });
          } catch (e) {
            this.debug("Failed to parse <tool_code> wrapped args", {
              error: e.message,
              preview: inner.slice(0, 160),
            });
          }
        }
      }
    ...
    // MiniMax variant: bare CALL-SYNTAX inside the wrapper, possibly multiple
    // calls per block. Observed in production logs from minimax-m2:cloud:
    //   <minimax:tool_call>
    //     builtin.run_terminal_cmd({"command": "cat", "args": ["CHANGELOG.md"]})
    //     builtin.run_terminal_cmd({"command": "cat", "args": ["docs/TESTING.md"]})
    //   </minimax:tool_call>
    // Brace/quote-aware walk so JSON args with strings, escapes, and nested
    ...
    if (calls.length === 0 && text.includes("<minimax:tool_call>")) {
      ...
      while ((blockMatch = blockRegex.exec(text)) !== null) {
        ...
        if (block.includes("<invoke")) continue; // handled by parameter-style branch
        const callOpening = /([\w-]+)\.([\w-]+)\(/g;
        let callMatch;
        while ((callMatch = callOpening.exec(block)) !== null) {
          const serverId = callMatch[1];
          const toolName = callMatch[2];
          ...
          if (end < 0) continue;
          const jsonText = block.slice(start, end).trim();
          try {
            const args = jsonText ? JSON.parse(jsonText) : {};
            calls.push({ serverId, toolName, args });
            this.debug(
              "Parsed minimax wrapped-call tool call (bare server.tool form)",
              { tool: `${serverId}.${toolName}` },
            );
          } catch (e) {
            this.debug("Failed to parse minimax wrapped-call JSON args", {
              raw: jsonText.slice(0, 200),
              error: e.message,
            });
          }
        }
        // If this block produced calls, advance to the next block — don't fall
        // through to the legacy header-line parser below.
    ...
    this.log("INFO", "MCP tool call started", {
      ...
      normalizedModel:
        argsWereNormalized && isGoogleAiStudioServer(serverId)
          ...
          : null,
      ...(serverId === "pci-assistant" && { argValues: normalizedArgs }),
    });
  ...
  getToolsPromptAndFlags() {
    ...
    // When compactMcpTools is on (default), external MCP tools are listed in
    // compact form (name + required/enum params, no prose description) so their
    // schemas don't inflate the prompt on every turn — they remain fully
    // callable. Builtin tools always stay detailed. Set compactMcpTools=false
    // to restore full external descriptions.
    const compactMcpTools = config.compactMcpTools !== false;
    ...
    const compactedServerLine =
      compactedServerNames.size > 0
        ? `\n\nNOTE: External MCP servers are listed in compact form (tool names + key params, no long descriptions) to keep the prompt lean: ${Array.from(compactedServerNames).join(", ")}. Tool names remain callable with TOOL_CALL.`
        : "";
    ...
    const terminalSessionLine = hasTerminalTool
      ? "\n\nAGENT TERMINAL: builtin.run_terminal_cmd executes shell commands directly on the user's machine in their project folder. Use TOOL_CALL when a shell command is the right way to help.\n" +
        ...
        'To discover project structure, prefer TOOL_CALL: builtin.list_dir({"path": "src", "depth": 2}) over `ls`/`find` — it returns a compact tree and is cheaper than shelling out.\n' +
        'To read source files, prefer TOOL_CALL: builtin.read_file({"path": "src/app.js"}) over `cat` — parallel-safe and size-bounded.\n' +
        'GIT/GITHUB: You have full git capabilities via the terminal. Examples: TOOL_CALL: builtin.run_terminal_cmd({"command": "git", "args": ["status"]}), TOOL_CALL: builtin.run_terminal_cmd({"command": "git", "args": ["log", "--oneline", "-5"]}), TOOL_CALL: builtin.run_terminal_cmd({"command": "git", "args": ["diff"]}).\n' +
...
- "I can only call one tool at a time and then wait"
- "say continue and I'll run the next tool"
- "I need your permission before calling tools"
Those statements are FALSE. You have tools. Use them.

IMPORTANT: When a tool is genuinely needed, call it using this exact format — but never lead your first reply to a user message with a bare TOOL_CALL. Start with a brief user-facing sentence first (acknowledge, answer from context, or say what you'll check), then call tools if still needed:

TOOL_CALL: server_id.tool_name({"arg1": "value"})

You may call multiple tools in one response once you've opened with user-facing prose. After calling a tool, STOP — do NOT write anything after the TOOL_CALL line. Do NOT predict, guess, or fabricate tool results. The system will execute the tool and provide the actual results in the next message. Do not ask permission to use tools when action is clearly required — but always greet or acknowledge the user before the first TOOL_CALL in a turn.
If the user asked for a concrete machine action and you finish your first reply with prose only (no TOOL_CALL) while tools are still required, you may be prompted to run the tool on the next round.

MULTI-STEP WORK: Keep emitting TOOL_CALL until the user's request is fully done. Do not stop after one file or command unless you output TASK_COMPLETE on its own line. If you need one user decision, start with NEEDS_USER_INPUT:.

DEFAULT RESPONSE MODE:
${
  config.chatRequireExplicitFileWrites === true
    ? `- Prefer answering directly in chat when the user asked for explanation, summary, review, or analysis.
- Do NOT create/save files by default.
- Only use file-writing tools when the user explicitly asks for a file artifact (e.g. "save as markdown", "export PDF", "create a DOCX"), or when the task explicitly requires modifying project files.`
    : `- Prefer answering directly in chat when a short in-chat answer is enough (explanation, summary, quick review).
- Use file-writing tools (\`builtin.write_file\`, \`builtin.generate_office_file\`) when saving or updating the project helps the user (fixes, new files, exports, reports). Avoid creating files the user does not need; prefer in-chat code blocks for tiny throwaway snippets.`
}
...
${hasOfficeTool ? '- For PDFs, DOCX, PPTX, and similar files under the project folder: call `TOOL_CALL: builtin.generate_office_file` with `"sourcePath"` only when you genuinely need file extraction/conversion to answer the request. If you already have enough text/context to answer in chat, answer directly and do not create files.\n' : ""}
...
${toolList}${compactedServerLine}${terminalSessionLine}${crawlWebResearchLine}${browserSessionLine}${googleWorkspaceLine}`;
  ...
  segmentToolCalls(toolCalls) {
    const safePatterns =
      /^(read_|view_|list_|get_|fetch_|find_|search_|validate_|check_|scan_|analyze_|score_|explain_|chat_)/i;
    const riskyPatterns =

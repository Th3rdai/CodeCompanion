...
// Fallback: minimax native format — "minimax:tool_call\n{...}" (minimax-m2 sometimes ignores TOOL_CALL: instructions)
const MINIMAX_TOOL_CALL = /minimax:tool_call\s*\n\s*(\{[\s\S]*?\})/g;
const MAX_ROUNDS = 5;
...
class ToolCallHandler {
  ...
  parseToolCalls(text) {
    ...
    // Fallback: minimax native format — "minimax:tool_call\n{...}"
    // Infer builtin tool from JSON shape: path+content → write_file, command → run_terminal_cmd
    if (calls.length === 0) {
      const mmRegex = new RegExp(MINIMAX_TOOL_CALL.source, "g");
      while ((match = mmRegex.exec(text)) !== null) {
        try {
          const args = this._parseArgs(match[1]);
          let toolName;
          ...
          if (toolName) {
            calls.push({ serverId: "builtin", toolName, args });
            this.debug("Parsed minimax:tool_call", { tool: toolName });
          }
        } catch (e) {
  ...
  buildToolsPrompt() {
    ...
    const terminalSessionLine = hasTerminalTool
      ? "\n\nAGENT TERMINAL: builtin.run_terminal_cmd is available for this session (project folder + allowlist). Use TOOL_CALL when a shell command is the right way to help."
      : "";

...
${toolList}${terminalSessionLine}`;
  ...
  /**
   * Split tool calls into parallel and serial queues based on safety patterns.
   * Safe patterns (read, view, list, get, fetch, search, validate, check, scan, analyze, score, explain, chat) → parallel
   * Risky patterns (write, create, delete, remove, update, modify, patch, execute, run, generate) → serial
   * Unknown patterns → serial (conservative default)
   * @param {Array} toolCalls - Array of { serverId, toolName, args } objects
   * @returns {{ parallelBatch: Array, serialQueue: Array }} - Two execution batches with originalIndex attached
   */
  batchToolCalls(toolCalls) {
    const safePatterns = /^(read_|view_|list_|get_|fetch_|search_|validate_|check_|scan_|analyze_|score_|explain_|chat_)/;
    const riskyPatterns = /^(write_|create_|delete_|remove_|update_|modify_|patch_|execute_|run_|generate_)/;

    // Explicit safelist for builtin tools: only run_terminal_cmd and generate_office_file are risky
    const riskySafeList = new Set(["run_terminal_cmd", "generate_office_file"]);
    ...
    toolCalls.forEach((call, idx) => {
      ...
      // Builtin tools: check explicit safelist
      if (call.serverId === "builtin") {
        if (riskySafeList.has(call.toolName)) {
      ...
      // MCP tools: use pattern matching (conservative default = serial)
      if (safePatterns.test(call.toolName)) {
        parallelBatch.push(entry);
      } else if (riskyPatterns.test(call.toolName)) {
        serialQueue.push(entry);
      } else {
        // Unknown pattern → serial (safe default)
        serialQueue.push(entry);
      }
    });

    return { parallelBatch, serialQueue };
  }
}

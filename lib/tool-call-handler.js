...
- You do NOT receive HTTP status codes (413, 404, 500, "payload too large", etc.) from this chat. NEVER invent them. If you mention "413" or "conversion service failed" without the user pasting an exact error message, you are hallucinating — STOP and use a tool instead.
- Binary gibberish, unreadable text, or a PDF filename in the message does NOT mean a conversion failed. Do NOT fabricate plausible errors. Use the appropriate tool to read or extract text from project files.
${hasOfficeTool ? '- For PDFs, DOCX, PPTX, and similar files under the project folder: call `TOOL_CALL: builtin.generate_office_file` with `"sourcePath"` set to the project-relative path (e.g. `"reports/guide.pdf"`) so the server extracts text — do NOT claim Docling or "the service" returned 413.\n' : ""}

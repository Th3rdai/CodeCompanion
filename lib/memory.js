...
const { chatStructured, embed, effectiveOllamaApiKey } = require('./ollama-client');
...
async function extractAndStore(ollamaUrl, chatModel, embeddingModel, conversation, config = null) {
...
Return JSON only. Be concise. Skip generic facts like "user writes code".`;
  ...
  if (extracted.summary && typeof extracted.summary === 'string' && extracted.summary.trim().length > 5) {
    try {
      const emb = await embed(ollamaUrl, extracted.summary, embeddingModel, embOpts);
      addMemory({
        type: 'summary',
...
async function buildMemoryContext(ollamaUrl, embeddingModel, messages, config) {
  ...
  try {
    ...
    try {
      ...
      queryEmbedding = await embed(ollamaUrl, lastUserMsg.content, embeddingModel, embKey ? { apiKey: embKey } : {});
    } finally {

const fs = require("fs");
const path = require("path");
const { randomUUID } = require("crypto");
const {
  chatStructured,
  embed,
  effectiveOllamaApiKey,
} = require("./ollama-client");
const {
  AGENT_AUTONOMY_MEMORY_MARKER,
  AGENT_AUTONOMY_MEMORY_CONTENT,
} = require("./agent-autonomy");

let _memoryDir = null;
let _memories = [];
let _memoryVersion = 0;

const NULL_SOURCE_KEY = "__NULL_SOURCE__";
const DEBUG_TRUE = new Set(["1", "true", "yes", "on"]);
const _indexCache = {
  builtForVersion: -1,
  builtAtMs: 0,
  embedded: [],
  bySource: new Map(),
  byType: new Map(),
  byTypeSource: new Map(),
};

const MEMORIES_FILE = "memories.json";

/** Single source of truth for the embedding model fallback when none is configured. */
const DEFAULT_EMBEDDING_MODEL = "nomic-embed-text";
const MS_PER_DAY = 24 * 60 * 60 * 1000;
// Node.js setInterval has a max value of 2^31-1 ms (~24.8 days).
// Values exceeding this limit get clamped to 1ms, causing rapid firing.
// See: https://developer.mozilla.org/en-US/docs/Web/API/setTimeout#maximum_delay_value
const MAX_SAFE_INTERVAL_DAYS = 24; // Safe limit to avoid overflow
const DEFAULT_PRUNE_CHECK_INTERVAL_DAYS = 24;
const DEFAULT_PRUNE_CHECK_SAMPLE_SIZE = 50;
const DEFAULT_PRUNE_CHECK_THRESHOLDS = Object.freeze({
  nearDuplicateSimilarity: 0.86,
  lowSignalScoreThreshold: 0.6,
  minContentLength: 24,
  staleDays: 120,
});

function _nowMs() {
  return Number(process.hrtime.bigint()) / 1e6;
}

function _isTimingEnabled(config) {
  const debugRaw = String(process.env.DEBUG || "")
    .trim()
    .toLowerCase();
  if (DEBUG_TRUE.has(debugRaw)) return true;
  return !!config?.memory?.indexCache?.logTimings;
}

function _logTiming(config, label, startedMs, fields = {}) {
  if (!_isTimingEnabled(config)) return;
  const elapsedMs = Math.max(0, _nowMs() - startedMs);
  const suffix = Object.entries(fields)
    .filter(([, value]) => value !== undefined && value !== null)
    .map(([key, value]) => `${key}=${value}`)
    .join(" ");
  const details = suffix ? ` ${suffix}` : "";
  console.log(`[Memory][Perf] ${label} ${elapsedMs.toFixed(1)}ms${details}`);
}

function _sourceKey(source) {
  return source || NULL_SOURCE_KEY;
}

function _typeSourceKey(type, source) {
  return `${type || ""}::${_sourceKey(source)}`;
}

function _pushToIndex(map, key, memory) {
  const arr = map.get(key);
  if (arr) {
    arr.push(memory);
    return;
  }
  map.set(key, [memory]);
}

function _markMemoryCollectionChanged() {
  _memoryVersion += 1;
  _indexCache.builtForVersion = -1;
  _indexCache.builtAtMs = 0;
  _indexCache.embedded = [];
  _indexCache.bySource.clear();
  _indexCache.byType.clear();
  _indexCache.byTypeSource.clear();
}

function _isIndexCacheEnabled(options = {}) {
  return options.indexCacheEnabled === true;
}

function _ensureIndexCache(options = {}) {
  if (!_isIndexCacheEnabled(options)) return null;
  if (_indexCache.builtForVersion === _memoryVersion) return _indexCache;

  const embedded = [];
  const bySource = new Map();
  const byType = new Map();
  const byTypeSource = new Map();

  for (const memory of _memories) {
    if (!memory || !Array.isArray(memory.embedding)) continue;
    embedded.push(memory);
    _pushToIndex(bySource, _sourceKey(memory.source), memory);
    _pushToIndex(byType, memory.type || "", memory);
    _pushToIndex(
      byTypeSource,
      _typeSourceKey(memory.type, memory.source),
      memory,
    );
  }

  _indexCache.embedded = embedded;
  _indexCache.bySource = bySource;
  _indexCache.byType = byType;
  _indexCache.byTypeSource = byTypeSource;
  _indexCache.builtForVersion = _memoryVersion;
  _indexCache.builtAtMs = Date.now();
  return _indexCache;
}

function _getIndexedPool(options = {}) {
  const cache = _ensureIndexCache(options);
  if (!cache) return { pool: null, usedIndex: false };

  const { conversationId, scopeToConversation, types } = options;
  const hasTypes = Array.isArray(types) && types.length > 0;

  if (scopeToConversation && conversationId) {
    if (!hasTypes) {
      return {
        pool: cache.bySource.get(_sourceKey(conversationId)) || [],
        usedIndex: true,
      };
    }
    const scoped = [];
    for (const type of types) {
      const bucket = cache.byTypeSource.get(
        _typeSourceKey(type, conversationId),
      );
      if (bucket && bucket.length > 0) scoped.push(...bucket);
    }
    return { pool: scoped, usedIndex: true };
  }

  if (hasTypes) {
    const typed = [];
    for (const type of types) {
      const bucket = cache.byType.get(type);
      if (bucket && bucket.length > 0) typed.push(...bucket);
    }
    return { pool: typed, usedIndex: true };
  }

  return { pool: cache.embedded, usedIndex: true };
}

/** Pick the user's configured embedding model, or the default if unset/blank. */
function resolveEmbeddingModel(config) {
  const m = config && config.memory && config.memory.embeddingModel;
  return typeof m === "string" && m.trim() ? m.trim() : DEFAULT_EMBEDDING_MODEL;
}

/**
 * Derive a stable, normalized project key from a folder path or project name.
 * e.g. "/Users/james/Projects/CodeCompanion" → "codecompanion"
 *      "My Cool App"                          → "my-cool-app"
 * Returns null if nothing usable is provided.
 */
function deriveProjectKey(folderPathOrName) {
  if (!folderPathOrName || typeof folderPathOrName !== "string") return null;
  const base = path.basename(folderPathOrName.trim());
  const key = base
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  return key || null;
}

function initMemory(dataRoot) {
  const startedMs = _nowMs();
  // Drop any pending debounced write from a prior init (re-init / test isolation)
  // so it can't fire against the new/old directory after the swap.
  try {
    if (_persistTimer) {
      clearTimeout(_persistTimer);
      _persistTimer = null;
    }
    _persistDirty = false;
    _extractionWatermark.clear();
    _memoryDir = path.join(dataRoot, "memory");
    if (!fs.existsSync(_memoryDir)) {
      fs.mkdirSync(_memoryDir, { recursive: true });
    }
    _loadFromDisk();
    _migrateGlobalScope();
  } finally {
    _logTiming(null, "initMemory", startedMs, { memories: _memories.length });
  }
}

function _hasAgentAutonomyMemory() {
  return _memories.some(
    (m) =>
      m.type === "fact" &&
      m.pinned === true &&
      String(m.content || "").includes(AGENT_AUTONOMY_MEMORY_MARKER),
  );
}

function _pinnedGlobalFactCandidates(embeddingModel) {
  return _memories
    .filter(
      (m) =>
        m.pinned === true &&
        m.type === "fact" &&
        !m.source &&
        Array.isArray(m.embedding) &&
        (!embeddingModel || m.embeddingModel === embeddingModel),
    )
    .map((m) => ({
      id: m.id,
      type: m.type,
      content: m.content,
      projectKey: m.projectKey || null,
      score: 1,
    }));
}

/**
 * Seed a pinned global fact so autonomy preference survives across chats.
 * Safe to call repeatedly; skips when the marker memory already exists.
 */
async function ensureAgentAutonomyMemory(config = {}) {
  if (!_memoryDir) return { created: false, reason: "not_initialized" };
  if (_hasAgentAutonomyMemory()) return { created: false, reason: "exists" };

  const ollamaUrl = config.ollamaUrl;
  if (!ollamaUrl) return { created: false, reason: "no_ollama_url" };

  const embeddingModel = resolveEmbeddingModel(config);
  const embKey = effectiveOllamaApiKey(config);
  let embedding = null;
  try {
    embedding = await embed(
      ollamaUrl,
      AGENT_AUTONOMY_MEMORY_CONTENT,
      embeddingModel,
      {
        ...(embKey ? { apiKey: embKey } : {}),
        timeoutMs: 30000,
      },
    );
  } catch (err) {
    const msg = err && err.message != null ? String(err.message) : String(err);
    console.warn("[Memory] Agent autonomy seed embed failed:", msg);
    return { created: false, reason: "embed_failed", error: msg };
  }
  if (!embedding) return { created: false, reason: "empty_embedding" };

  addMemory({
    type: "fact",
    content: AGENT_AUTONOMY_MEMORY_CONTENT,
    source: null,
    embedding,
    embeddingModel,
    confidence: 1,
    pinned: true,
  });
  return { created: true };
}

/**
 * One-time migration:
 * 1. Facts/patterns/projects previously stored with a conversationId source get
 *    source cleared so they become globally searchable.
 * 2. Project memories get a projectKey recovered from their content string
 *    ("Project: CodeCompanion — ...") if they don't already have one.
 * 3. Pattern memories without a projectKey stay null (treated as agent-level patterns).
 */
function _migrateGlobalScope() {
  const GLOBAL_TYPES = ["fact", "pattern", "project"];
  // Regex to extract project name from "Project: <Name> — Stack: ..."
  const PROJECT_RE = /^Project:\s*([^—\n]+)/i;
  let changed = 0;
  for (const m of _memories) {
    if (!GLOBAL_TYPES.includes(m.type)) continue;
    // Clear legacy conversationId source
    if (m.source !== null) {
      m.source = null;
      changed++;
    }
    // Recover projectKey for project memories that lack one
    if (m.type === "project" && !m.projectKey) {
      const match = m.content && m.content.match(PROJECT_RE);
      if (match) {
        m.projectKey = deriveProjectKey(match[1].trim());
        changed++;
      }
    }
  }
  if (changed > 0) {
    _markMemoryCollectionChanged();
    console.log(
      `[Memory] Migrated ${changed} memory fields to global/project scope.`,
    );
    _persistToDisk();
  }
}

/**
 * Rebuild memories.json, optionally pruning orphaned conversation summaries.
 * @param {{ validSources?: string[]|Set<string> }} [options]
 *   validSources: ids of conversations that still exist. Summary memories whose
 *   `source` is not in this set are orphaned (their conversation was deleted)
 *   and removed. Pinned memories are never removed. Omit to just rebuild.
 * @returns {{ before:number, after:number, removed:number }}
 */
function compactMemories(options = {}) {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");
  const before = _memories.length;
  const { validSources } = options;
  if (validSources) {
    const valid =
      validSources instanceof Set ? validSources : new Set(validSources);
    _memories = _memories.filter((m) => {
      if (m.pinned) return true; // never drop pinned
      // Only summaries are conversation-scoped; orphan = source not in valid set.
      if (m.type === "summary" && m.source && !valid.has(m.source))
        return false;
      return true;
    });
  }
  if (_memories.length !== before) _markMemoryCollectionChanged();
  _persistToDisk();
  const after = _memories.length;
  return { before, after, removed: before - after };
}

function addMemory({
  type,
  content,
  source,
  projectKey,
  embedding,
  embeddingModel,
  confidence,
  topics,
  pinned,
}) {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");

  const now = new Date().toISOString();
  const resolvedType = type || "fact";
  const memory = {
    id: randomUUID(),
    type: resolvedType,
    content,
    source: source || null,
    projectKey: projectKey || null,
    createdAt: now,
    updatedAt: now,
    embedding: embedding || null,
    embeddingModel: embeddingModel || "",
    confidence: typeof confidence === "number" ? confidence : 0.5,
    pinned: pinned === true,
  };
  // Topics are keyword tags for a conversation, only meaningful on summaries.
  // Persist as a normalized string[] when provided on a summary record.
  if (resolvedType === "summary" && Array.isArray(topics)) {
    memory.topics = topics
      .filter((t) => typeof t === "string" && t.trim())
      .map((t) => t.trim());
  }
  _memories.push(memory);
  _markMemoryCollectionChanged();
  _persistToDisk();
  return memory;
}

function getMemories(filter) {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");

  let results = _memories;
  if (filter && filter.type) {
    results = results.filter((m) => m.type === filter.type);
  }
  return results.slice();
}

function getMemory(id) {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");
  return _memories.find((m) => m.id === id) || null;
}

function updateMemory(id, updates) {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");

  const memory = _memories.find((m) => m.id === id);
  if (!memory) return null;

  // Only allow safe fields to be updated
  const allowedFields = [
    "type",
    "content",
    "source",
    "projectKey",
    "embedding",
    "embeddingModel",
    "confidence",
    "topics",
    "pinned",
    "active",
  ];
  for (const key of allowedFields) {
    if (updates[key] !== undefined) {
      memory[key] = updates[key];
    }
  }
  memory.updatedAt = new Date().toISOString();
  _markMemoryCollectionChanged();
  _persistToDisk();
  return memory;
}

function deleteMemory(id) {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");

  const idx = _memories.findIndex((m) => m.id === id);
  if (idx === -1) return false;

  _memories.splice(idx, 1);
  _markMemoryCollectionChanged();
  _persistToDisk();
  return true;
}

/**
 * Hard-delete every memory whose `source` matches the given conversation id.
 * Returns the number of records removed. Used to cascade history deletes.
 */
function deleteMemoriesBySource(source) {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");
  if (!source || typeof source !== "string") return 0;

  const before = _memories.length;
  _memories = _memories.filter((m) => m.source !== source);
  const removed = before - _memories.length;
  if (removed > 0) {
    _markMemoryCollectionChanged();
    _persistToDisk();
  }
  return removed;
}

/**
 * @param {number[]} queryEmbedding
 * @param {number} [topK]
 * @param {number} [threshold]
 * @param {{
 *   conversationId?: string|null,
 *   scopeToConversation?: boolean,
 *   projectKey?: string|null,
 *   types?: string[],
 *   indexCacheEnabled?: boolean,
 *   timingConfig?: object,
 * }} [options]
 *   - `scopeToConversation`: restrict to memories from one conversation (for summaries)
 *   - `projectKey`: restrict to memories tagged with this project (for project/pattern recall)
 *   - `types`: only include memories of these types
 */
function searchMemories(
  queryEmbedding,
  topK = 5,
  threshold = 0.3,
  options = {},
) {
  const startedMs = _nowMs();
  let candidateCount = 0;
  let resultCount = 0;
  let usedIndex = false;
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");
  if (!queryEmbedding || !Array.isArray(queryEmbedding)) {
    _logTiming(options.timingConfig, "searchMemories", startedMs, {
      candidates: 0,
      results: 0,
      indexed: false,
    });
    return [];
  }

  const {
    conversationId,
    scopeToConversation,
    projectKey,
    types,
    embeddingModel,
  } = options;
  const indexed = _getIndexedPool(options);
  let pool =
    indexed.pool ||
    _memories.filter((m) => m.embedding && Array.isArray(m.embedding));
  usedIndex = indexed.usedIndex;

  // Skip records embedded with a different model — cosine similarity across
  // different embedding models is meaningless. Opt-in: only filters when the
  // caller passes the active embeddingModel. Stale records become recallable
  // again after POST /api/memory/reembed re-embeds them with the current model.
  if (embeddingModel) {
    pool = pool.filter((m) => m.embeddingModel === embeddingModel);
  }
  if (scopeToConversation && conversationId) {
    pool = pool.filter((m) => m.source === conversationId);
  }
  if (projectKey) {
    pool = pool.filter((m) => m.projectKey === projectKey);
  }
  if (Array.isArray(types) && types.length > 0) {
    pool = pool.filter((m) => types.includes(m.type));
  }

  candidateCount = pool.length;
  const scored = pool
    .map((m) => ({
      ...m,
      score: cosineSimilarity(queryEmbedding, m.embedding),
    }))
    .filter((m) => m.score >= threshold)
    .sort((a, b) => b.score - a.score)
    .slice(0, topK);

  resultCount = scored.length;
  _logTiming(options.timingConfig, "searchMemories", startedMs, {
    candidates: candidateCount,
    results: resultCount,
    indexed: usedIndex,
  });
  return scored;
}

function getStats() {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");

  const byType = {};
  for (const m of _memories) {
    byType[m.type] = (byType[m.type] || 0) + 1;
  }

  let storageBytes = 0;
  const filePath = path.join(_memoryDir, MEMORIES_FILE);
  if (fs.existsSync(filePath)) {
    try {
      storageBytes = fs.statSync(filePath).size;
    } catch {
      // ignore
    }
  }

  return {
    total: _memories.length,
    totalIncludingDeleted: _memories.length,
    byType,
    storageBytes,
  };
}

function _toFiniteNumber(value, fallback, min, max) {
  const n = Number(value);
  if (!Number.isFinite(n)) return fallback;
  if (Number.isFinite(min) && n < min) return min;
  if (Number.isFinite(max) && n > max) return max;
  return n;
}

function _normalizeMemoryContent(content) {
  return String(content || "")
    .replace(/\s+/g, " ")
    .trim()
    .toLowerCase();
}

function _bucketForAgeDays(ageDays) {
  if (ageDays <= 7) return "0-7";
  if (ageDays <= 30) return "8-30";
  if (ageDays <= 90) return "31-90";
  if (ageDays <= 180) return "91-180";
  return "181+";
}

function _percentile(values, ratio) {
  if (!Array.isArray(values) || values.length === 0) return 0;
  const sorted = values.slice().sort((a, b) => a - b);
  const idx = Math.min(
    sorted.length - 1,
    Math.max(0, Math.floor((sorted.length - 1) * ratio)),
  );
  return sorted[idx];
}

function _scoreLowSignalCandidate(memory, opts, nowMs) {
  const reasons = [];
  const createdMs = Number(new Date(memory.createdAt || 0)) || nowMs;
  const updatedMs = Number(new Date(memory.updatedAt || 0)) || createdMs;
  const ageDays = Math.max(
    0,
    Math.floor((nowMs - Math.max(createdMs, updatedMs)) / MS_PER_DAY),
  );
  const confidence = _toFiniteNumber(memory.confidence, 0.5, 0, 1);
  const content = String(memory.content || "").trim();
  const normalized = _normalizeMemoryContent(content);
  const contentLength = normalized.length;
  const tokens = _tokenize(normalized);
  const uniqueTokenRatio =
    tokens.length > 0 ? new Set(tokens).size / tokens.length : 0;
  let score = 0;

  if (contentLength < opts.minContentLength) {
    const contentPenalty = Math.min(
      1,
      (opts.minContentLength - contentLength) /
        Math.max(1, opts.minContentLength),
    );
    score += 0.45 * contentPenalty;
    reasons.push("short_content");
  }

  if (confidence < 0.5) {
    score += (0.3 * (0.5 - confidence)) / 0.5;
    reasons.push("low_confidence");
  }

  if (ageDays > opts.staleDays) {
    score += Math.min(
      0.2,
      ((ageDays - opts.staleDays) / Math.max(1, opts.staleDays)) * 0.2,
    );
    reasons.push("stale");
  }

  if (tokens.length <= 3 || uniqueTokenRatio < 0.6) {
    score += 0.2;
    reasons.push("low_token_diversity");
  }

  if (
    /^(ok|thanks|noted|got it|done|cool|yes|no|maybe|sure)[.!]?$/i.test(content)
  ) {
    score += 0.35;
    reasons.push("generic_content");
  }

  return {
    score: Number(Math.min(1, score).toFixed(4)),
    reasons,
    ageDays,
    confidence,
    contentLength,
  };
}

/**
 * Analyze memory quality with a non-destructive, read-only dry run.
 * This function never mutates in-memory records and never writes to disk.
 */
function analyzeMemoryQualityDryRun(opts = {}) {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");

  const nowMs = Number.isFinite(Number(opts.nowMs))
    ? Number(opts.nowMs)
    : Date.now();
  const sampleSize = Math.floor(
    _toFiniteNumber(opts.sampleSize, DEFAULT_PRUNE_CHECK_SAMPLE_SIZE, 1, 500),
  );
  const nearDuplicateSimilarity = _toFiniteNumber(
    opts.nearDuplicateSimilarity,
    DEFAULT_PRUNE_CHECK_THRESHOLDS.nearDuplicateSimilarity,
    0.5,
    1,
  );
  const lowSignalScoreThreshold = _toFiniteNumber(
    opts.lowSignalScoreThreshold,
    DEFAULT_PRUNE_CHECK_THRESHOLDS.lowSignalScoreThreshold,
    0,
    1,
  );
  const minContentLength = Math.floor(
    _toFiniteNumber(
      opts.minContentLength,
      DEFAULT_PRUNE_CHECK_THRESHOLDS.minContentLength,
      1,
      500,
    ),
  );
  const staleDays = Math.floor(
    _toFiniteNumber(
      opts.staleDays,
      DEFAULT_PRUNE_CHECK_THRESHOLDS.staleDays,
      1,
      3650,
    ),
  );

  const localOpts = {
    nearDuplicateSimilarity,
    lowSignalScoreThreshold,
    minContentLength,
    staleDays,
    sampleSize,
  };
  const snapshot = _memories.slice();
  const byType = {};
  const byConfidence = {
    "0.00-0.24": 0,
    "0.25-0.49": 0,
    "0.50-0.74": 0,
    "0.75-1.00": 0,
  };
  const byAgeDays = { "0-7": 0, "8-30": 0, "31-90": 0, "91-180": 0, "181+": 0 };
  const duplicateBuckets = new Map();
  const nearDuplicateBuckets = new Map();
  const lowSignalScores = [];
  const lowSignalCandidates = [];

  for (const memory of snapshot) {
    const type = memory.type || "unknown";
    byType[type] = (byType[type] || 0) + 1;

    const confidence = _toFiniteNumber(memory.confidence, 0.5, 0, 1);
    if (confidence < 0.25) byConfidence["0.00-0.24"] += 1;
    else if (confidence < 0.5) byConfidence["0.25-0.49"] += 1;
    else if (confidence < 0.75) byConfidence["0.50-0.74"] += 1;
    else byConfidence["0.75-1.00"] += 1;

    const updatedMs = Number(
      new Date(memory.updatedAt || memory.createdAt || 0),
    );
    const ageDays = Math.max(
      0,
      Math.floor(
        (nowMs - (Number.isFinite(updatedMs) ? updatedMs : nowMs)) / MS_PER_DAY,
      ),
    );
    byAgeDays[_bucketForAgeDays(ageDays)] += 1;

    const normalized = _normalizeMemoryContent(memory.content);
    const dedupeKey = `${type}::${memory.projectKey || ""}::${memory.source || ""}::${normalized}`;
    if (!duplicateBuckets.has(dedupeKey)) duplicateBuckets.set(dedupeKey, []);
    duplicateBuckets.get(dedupeKey).push(memory);

    const nearKey = `${type}::${memory.projectKey || ""}`;
    if (!nearDuplicateBuckets.has(nearKey))
      nearDuplicateBuckets.set(nearKey, []);
    nearDuplicateBuckets.get(nearKey).push(memory);

    const signal = _scoreLowSignalCandidate(memory, localOpts, nowMs);
    lowSignalScores.push(signal.score);
    if (signal.score >= lowSignalScoreThreshold) {
      lowSignalCandidates.push({
        id: memory.id,
        type,
        score: signal.score,
        reasons: signal.reasons,
        confidence: Number(signal.confidence.toFixed(4)),
        ageDays: signal.ageDays,
        contentPreview: String(memory.content || "").slice(0, 160),
      });
    }
  }

  const exactDuplicateGroups = [...duplicateBuckets.values()]
    .filter((group) => group.length > 1)
    .map((group) =>
      group
        .slice()
        .sort((a, b) => String(a.id || "").localeCompare(String(b.id || ""))),
    )
    .sort((a, b) => String(a[0].id || "").localeCompare(String(b[0].id || "")))
    .map((group) => ({
      type: group[0].type || "unknown",
      source: group[0].source || null,
      projectKey: group[0].projectKey || null,
      normalizedContent: _normalizeMemoryContent(group[0].content),
      members: group.map((m) => ({
        id: m.id,
        confidence: _toFiniteNumber(m.confidence, 0.5, 0, 1),
        pinned: m.pinned === true,
        createdAt: m.createdAt || null,
      })),
      removableCount: Math.max(0, group.length - 1),
    }));
  const exactDuplicateRemovableCount = exactDuplicateGroups.reduce(
    (sum, group) => sum + group.removableCount,
    0,
  );

  const nearDuplicatePairs = [];
  for (const bucket of nearDuplicateBuckets.values()) {
    const sortedBucket = bucket
      .slice()
      .sort((a, b) => String(a.id || "").localeCompare(String(b.id || "")));
    for (let i = 0; i < sortedBucket.length; i++) {
      for (let j = i + 1; j < sortedBucket.length; j++) {
        const a = sortedBucket[i];
        const b = sortedBucket[j];
        const normA = _normalizeMemoryContent(a.content);
        const normB = _normalizeMemoryContent(b.content);
        if (!normA || !normB || normA === normB) continue;
        const tokensA = new Set(_tokenize(normA));
        const tokensB = new Set(_tokenize(normB));
        const union = new Set([...tokensA, ...tokensB]);
        if (union.size === 0) continue;
        let intersectionCount = 0;
        for (const token of tokensA) {
          if (tokensB.has(token)) intersectionCount += 1;
        }
        const jaccard = intersectionCount / union.size;
        const lengthRatio =
          Math.min(normA.length, normB.length) /
          Math.max(normA.length, normB.length);
        const similarity = Number(
          (jaccard * 0.7 + lengthRatio * 0.3).toFixed(4),
        );
        if (similarity >= nearDuplicateSimilarity) {
          nearDuplicatePairs.push({
            type: a.type || "unknown",
            projectKey: a.projectKey || null,
            similarity,
            ids: [a.id, b.id],
          });
        }
      }
    }
  }
  nearDuplicatePairs.sort(
    (a, b) =>
      b.similarity - a.similarity ||
      a.ids[0].localeCompare(b.ids[0]) ||
      a.ids[1].localeCompare(b.ids[1]),
  );

  lowSignalCandidates.sort(
    (a, b) => b.score - a.score || a.id.localeCompare(b.id),
  );

  const suggestedNearDuplicateSimilarity = Number(
    Math.max(
      nearDuplicateSimilarity,
      _percentile(
        nearDuplicatePairs.map((p) => p.similarity),
        0.75,
      ),
    ).toFixed(4),
  );
  const suggestedLowSignalScoreThreshold = Number(
    Math.max(
      lowSignalScoreThreshold,
      _percentile(lowSignalScores, 0.8),
    ).toFixed(4),
  );

  return {
    readOnly: true,
    generatedAt: new Date(nowMs).toISOString(),
    totalCount: snapshot.length,
    distributions: {
      byType,
      byConfidence,
      byAgeDays,
    },
    exactDuplicates: {
      totalGroups: exactDuplicateGroups.length,
      removableCount: exactDuplicateRemovableCount,
      groups: exactDuplicateGroups.slice(0, sampleSize),
    },
    nearDuplicates: {
      similarityThreshold: nearDuplicateSimilarity,
      totalCandidatePairs: nearDuplicatePairs.length,
      pairs: nearDuplicatePairs.slice(0, sampleSize),
    },
    lowSignal: {
      scoreThreshold: lowSignalScoreThreshold,
      totalCandidates: lowSignalCandidates.length,
      candidates: lowSignalCandidates.slice(0, sampleSize),
    },
    suggestedThresholds: {
      nearDuplicateSimilarity: suggestedNearDuplicateSimilarity,
      lowSignalScoreThreshold: suggestedLowSignalScoreThreshold,
      minContentLength,
      staleDays,
      sampleSize,
    },
  };
}

function _buildMemoryPruneLogSummary(analysis) {
  return {
    totalCount: analysis.totalCount,
    exactDuplicateGroups: analysis.exactDuplicates.totalGroups,
    exactDuplicateRemovable: analysis.exactDuplicates.removableCount,
    nearDuplicatePairs: analysis.nearDuplicates.totalCandidatePairs,
    lowSignalCandidates: analysis.lowSignal.totalCandidates,
  };
}

function startMemoryPruneCheckScheduler({
  getConfig,
  log = () => {},
  setIntervalFn = setInterval,
  clearIntervalFn = clearInterval,
} = {}) {
  if (typeof getConfig !== "function") {
    throw new Error("startMemoryPruneCheckScheduler requires getConfig()");
  }
  const pruneCheck = getConfig()?.memory?.pruneCheck || {};
  if (pruneCheck.enabled === false) {
    return {
      enabled: false,
      intervalMs: 0,
      stop() {},
      runNow() {
        return null;
      },
    };
  }

  const intervalDays = _toFiniteNumber(
    pruneCheck.intervalDays,
    DEFAULT_PRUNE_CHECK_INTERVAL_DAYS,
    1,
    MAX_SAFE_INTERVAL_DAYS, // Enforce max to prevent setInterval overflow
  );
  let intervalMs = Math.floor(intervalDays * MS_PER_DAY);

  // Sanity check: Ensure interval doesn't exceed Node.js setInterval limit (2^31-1 ms).
  // A delay above this silently collapses to 1ms in Node, turning this dry-run
  // check into a CPU-pegging hot loop that floods the log (observed in prod with
  // intervalDays=30 → ~125 runs/sec). MAX_SAFE_INTERVAL_DAYS normally keeps us
  // under the limit, but clamp here too so no future default/config can reopen it.
  const MAX_SETINTERVAL_MS = 2147483647; // 2^31-1, ~24.8 days
  if (intervalMs > MAX_SETINTERVAL_MS) {
    log(
      "WARN",
      "Memory prune check interval exceeds Node.js setInterval limit",
      {
        requestedDays: intervalDays,
        requestedMs: intervalMs,
        clampedMs: MAX_SETINTERVAL_MS,
        clampedDays: (MAX_SETINTERVAL_MS / MS_PER_DAY).toFixed(2),
      },
    );
    intervalMs = MAX_SETINTERVAL_MS;
  }

  const runNow = (reason = "manual") => {
    const cfg = getConfig()?.memory?.pruneCheck || {};
    if (cfg.enabled === false) return null;
    const thresholds = cfg.thresholds || {};
    const analysis = analyzeMemoryQualityDryRun({
      sampleSize: cfg.sampleSize,
      nearDuplicateSimilarity: thresholds.nearDuplicateSimilarity,
      lowSignalScoreThreshold: thresholds.lowSignalScoreThreshold,
      minContentLength: thresholds.minContentLength,
      staleDays: thresholds.staleDays,
    });
    const summary = _buildMemoryPruneLogSummary(analysis);
    log("INFO", "Memory prune dry-run summary", { reason, ...summary });
    return { analysis, summary };
  };

  const timer = setIntervalFn(() => {
    try {
      runNow("interval");
    } catch (err) {
      log("WARN", "Memory prune dry-run check failed", { error: err.message });
    }
  }, intervalMs);
  if (timer && typeof timer.unref === "function") timer.unref();

  return {
    enabled: true,
    intervalMs,
    stop() {
      clearIntervalFn(timer);
    },
    runNow,
  };
}

function cosineSimilarity(a, b) {
  if (!a || !b || a.length !== b.length || a.length === 0) return 0;

  let dot = 0;
  let magA = 0;
  let magB = 0;
  for (let i = 0; i < a.length; i++) {
    dot += a[i] * b[i];
    magA += a[i] * a[i];
    magB += b[i] * b[i];
  }
  magA = Math.sqrt(magA);
  magB = Math.sqrt(magB);

  if (magA === 0 || magB === 0) return 0;
  return dot / (magA * magB);
}

function _loadFromDisk() {
  const startedMs = _nowMs();
  const filePath = path.join(_memoryDir, MEMORIES_FILE);
  if (!fs.existsSync(filePath)) {
    _memories = [];
    _markMemoryCollectionChanged();
    _logTiming(null, "loadMemoryFromDisk", startedMs, { loaded: 0 });
    return;
  }
  try {
    const parsed = JSON.parse(fs.readFileSync(filePath, "utf-8"));
    if (!Array.isArray(parsed)) {
      _memories = [];
      _markMemoryCollectionChanged();
      _logTiming(null, "loadMemoryFromDisk", startedMs, { loaded: 0 });
      return;
    }
    // Migration: drop legacy soft-delete tombstones (active === false) and
    // strip the `active` field from kept records. If anything was removed,
    // compact the file in place so the next start sees the slim version.
    const before = parsed.length;
    _memories = parsed
      .filter((m) => m && m.active !== false)
      .map(({ active: _active, ...rest }) => rest);
    _markMemoryCollectionChanged();
    if (_memories.length !== before) {
      _persistToDisk();
    }
    _logTiming(null, "loadMemoryFromDisk", startedMs, {
      loaded: _memories.length,
    });
  } catch {
    _memories = [];
    _markMemoryCollectionChanged();
    _logTiming(null, "loadMemoryFromDisk", startedMs, { loaded: 0 });
  }
}

// Debounced persistence (MEMORYFIX P1b): bursts of writes — e.g. extraction
// adding many records in a loop, or auto-prune — coalesce into a single disk
// write. flushMemoryToDisk() forces any pending write synchronously and is
// called on process shutdown (server.js SIGINT/SIGTERM). A non-graceful crash
// within the debounce window can lose the last write; acceptable for local
// memory, and the window is small.
let _persistTimer = null;
let _persistDirty = false;
const PERSIST_DEBOUNCE_MS = 250;

/** Write current in-memory state to disk synchronously (atomic tmp+rename). */
function _writeToDiskNow() {
  const filePath = path.join(_memoryDir, MEMORIES_FILE);
  const tmp = filePath + ".tmp." + process.pid;
  fs.writeFileSync(tmp, JSON.stringify(_memories, null, 2));
  fs.renameSync(tmp, filePath);
  _persistDirty = false;
}

/** Mark state dirty and schedule a coalesced disk write (debounced). */
function _persistToDisk() {
  _persistDirty = true;
  if (_persistTimer) return;
  _persistTimer = setTimeout(() => {
    _persistTimer = null;
    if (!_persistDirty || !_memoryDir) return;
    try {
      _writeToDiskNow();
    } catch (err) {
      console.warn("[Memory] debounced persist failed:", err.message);
    }
  }, PERSIST_DEBOUNCE_MS);
  // A pending memory flush should not keep the process alive on its own.
  if (typeof _persistTimer.unref === "function") _persistTimer.unref();
}

/**
 * Force any pending debounced write to disk synchronously. Called on process
 * shutdown so a deferred write isn't lost. No-op (returns false) if memory was
 * never initialized or nothing is pending.
 * @returns {boolean} true if a flush was performed
 */
function flushMemoryToDisk() {
  if (!_memoryDir) return false;
  if (_persistTimer) {
    clearTimeout(_persistTimer);
    _persistTimer = null;
  }
  if (!_persistDirty) return false;
  try {
    _writeToDiskNow();
    return true;
  } catch (err) {
    console.warn("[Memory] flushMemoryToDisk failed:", err.message);
    return false;
  }
}

/**
 * Recompute embeddings for every stored memory using the given embedding model.
 * Mutates records in place and persists once at the end (not per record).
 * `embedFn` is injectable so this is unit-testable without a live Ollama.
 * @returns {Promise<{total:number, reembedded:number, failed:number, embeddingModel:string}>}
 */
async function reembedAllMemories(
  ollamaUrl,
  embeddingModel,
  opts = {},
  embedFn = embed,
) {
  if (!_memoryDir)
    throw new Error("Memory not initialized. Call initMemory(dataRoot) first.");

  let reembedded = 0;
  let failed = 0;
  for (const m of _memories) {
    if (!m.content || typeof m.content !== "string") {
      failed++;
      continue;
    }
    try {
      const emb = await embedFn(ollamaUrl, m.content, embeddingModel, opts);
      if (emb && Array.isArray(emb) && emb.length > 0) {
        m.embedding = emb;
        m.embeddingModel = embeddingModel;
        m.updatedAt = new Date().toISOString();
        reembedded++;
      } else {
        failed++;
      }
    } catch (err) {
      console.warn("[Memory] Re-embed failed for a memory:", err.message);
      failed++;
    }
  }
  if (reembedded > 0) {
    _markMemoryCollectionChanged();
    _persistToDisk();
  }
  return { total: _memories.length, reembedded, failed, embeddingModel };
}

// ── Phase 2: Extraction Pipeline ──────────────────────

const EXTRACTION_SCHEMA = {
  type: "object",
  properties: {
    facts: {
      type: "array",
      items: { type: "string" },
      description: "Factual statements about the user",
    },
    project: {
      type: "object",
      properties: {
        name: { type: "string" },
        stack: { type: "string" },
        description: { type: "string" },
      },
      nullable: true,
    },
    patterns: {
      type: "array",
      items: { type: "string" },
      description: "Code patterns or recurring issues",
    },
    summary: {
      type: "string",
      description: "2-3 sentence conversation summary",
    },
    topics: { type: "array", items: { type: "string" } },
  },
  required: ["facts", "patterns", "summary", "topics"],
};

// Per-conversation extraction watermark (MEMORYFIX P3): highest message count
// already extracted, keyed by conversationId. In-memory only — resets on
// restart, costing at most one redundant extraction per conversation afterward.
const _extractionWatermark = new Map();

/** True if the conversation has grown since its last successful extraction. */
function extractionHasNewMessages(conversationId, messageCount) {
  if (!conversationId) return true;
  return messageCount > (_extractionWatermark.get(conversationId) || 0);
}

/** Record the message count extracted for a conversation. */
function markExtractionWatermark(conversationId, messageCount) {
  if (conversationId) _extractionWatermark.set(conversationId, messageCount);
}

async function extractAndStore(
  ollamaUrl,
  chatModel,
  embeddingModel,
  conversation,
  config = null,
  projectKey = null,
) {
  const apiKey = effectiveOllamaApiKey(config || {});
  const embOpts = apiKey ? { apiKey } : {};
  const structOpts = apiKey ? { apiKey } : {};
  const msgs = conversation.messages || [];
  const mode = conversation.mode || "chat";
  const conversationId = conversation.id || null;

  // Incremental (MEMORYFIX P3): skip when no new messages have been added since
  // the last successful extraction for this conversation — avoids redundant LLM
  // + embedding calls on rapid saves. Dedup handles any window overlap.
  if (!extractionHasNewMessages(conversationId, msgs.length)) return;

  // Format last 20 messages
  const recent = msgs.slice(-20);
  const formatted = recent
    .map((m) => {
      const role = m.role === "user" ? "User" : "Assistant";
      return `${role}: ${m.content}`;
    })
    .join("\n");

  const projectContext = projectKey
    ? `Active project: ${projectKey}`
    : "No specific project context.";

  const extractionPrompt = `Analyze this conversation and extract memories. Return ONLY new, specific information — not general programming knowledge.

Mode: ${mode}
${projectContext}
Messages:
${formatted}

Extract:
1. "facts": Facts about THIS USER as a person — their preferences, communication style, skill level, tools they use across all projects. These are AGENT-LEVEL memories recalled in every future conversation. Do NOT include project-specific details here.
2. "project": If a specific project was discussed: { name, stack, description }. null if no project mentioned.
3. "patterns": Code patterns, bugs, or architectural decisions specific to the ACTIVE PROJECT. Leave empty if no project context.
4. "summary": 2-3 sentence summary of key takeaways from this conversation.
5. "topics": Keyword tags for this conversation.

Return JSON only. Be concise. Skip generic facts like "user writes code".`;

  let extracted;
  try {
    extracted = await chatStructured(
      ollamaUrl,
      chatModel,
      [{ role: "user", content: extractionPrompt }],
      EXTRACTION_SCHEMA,
      60000,
      [],
      structOpts,
    );
  } catch (err) {
    const msg = err && err.message != null ? String(err.message) : "";
    if (err && err.name === "AbortError") return;
    if (/\baborted\b/i.test(msg)) return;
    console.warn("[Memory] Extraction failed (chatStructured):", msg || err);
    return;
  }

  if (!extracted) return;

  const maxMemories =
    (config && config.memory && config.memory.maxMemories) || 500;

  // Process facts — stored globally (source: null) so they persist across conversations
  if (Array.isArray(extracted.facts)) {
    for (const fact of extracted.facts) {
      if (!fact || typeof fact !== "string" || fact.trim().length < 5) continue;
      try {
        const emb = await embed(ollamaUrl, fact, embeddingModel, embOpts);
        await _deduplicateAndAdd(
          "fact",
          fact,
          null, // global scope — not tied to one conversation
          emb,
          embeddingModel,
          0.7,
          maxMemories,
        );
      } catch (err) {
        console.warn("[Memory] Failed to embed fact:", err.message);
      }
    }
  }

  // Process patterns — scoped to the active project so they don't bleed into other codebases
  if (Array.isArray(extracted.patterns)) {
    for (const pattern of extracted.patterns) {
      if (!pattern || typeof pattern !== "string" || pattern.trim().length < 5)
        continue;
      try {
        const emb = await embed(ollamaUrl, pattern, embeddingModel, embOpts);
        await _deduplicateAndAdd(
          "pattern",
          pattern,
          null,
          emb,
          embeddingModel,
          0.6,
          maxMemories,
          projectKey, // project-scoped — only recalled when this project is active
        );
      } catch (err) {
        console.warn("[Memory] Failed to embed pattern:", err.message);
      }
    }
  }

  // Resolve projectKey: prefer caller-supplied, fall back to extracted project name
  const resolvedProjectKey =
    projectKey ||
    (extracted.project?.name ? deriveProjectKey(extracted.project.name) : null);

  // Process project — scoped to its own key so different projects stay isolated
  if (extracted.project && extracted.project.name) {
    const projContent = `Project: ${extracted.project.name} — Stack: ${extracted.project.stack || "unknown"} — ${extracted.project.description || ""}`;
    try {
      const emb = await embed(ollamaUrl, projContent, embeddingModel, embOpts);
      await _deduplicateAndAdd(
        "project",
        projContent,
        null,
        emb,
        embeddingModel,
        0.8,
        maxMemories,
        resolvedProjectKey,
      );
    } catch (err) {
      console.warn("[Memory] Failed to embed project:", err.message);
    }
  }

  // Process summary
  if (
    extracted.summary &&
    typeof extracted.summary === "string" &&
    extracted.summary.trim().length > 5
  ) {
    try {
      const emb = await embed(
        ollamaUrl,
        extracted.summary,
        embeddingModel,
        embOpts,
      );
      addMemory({
        type: "summary",
        content: extracted.summary,
        source: conversationId,
        embedding: emb,
        embeddingModel,
        confidence: 0.5,
        topics: Array.isArray(extracted.topics) ? extracted.topics : [],
      });
      _autoPrune(maxMemories);
    } catch (err) {
      console.warn("[Memory] Failed to embed summary:", err.message);
    }
  }

  // Incremental (MEMORYFIX P3): record that this conversation has been extracted
  // up to its current message count, so the next debounced run skips when no new
  // messages were added. Only reached on a successful extraction pass.
  markExtractionWatermark(conversationId, msgs.length);
}

/**
 * Normalized identity name for a project memory ("Project: <name> — Stack: ...").
 * Lets us keep at most one identity memory per project regardless of how the
 * stack guess or projectKey drifts between extraction passes. Returns null for
 * non-identity content.
 */
function _projectIdentityName(content) {
  const m = String(content || "").match(/^Project:\s*(.+?)\s+[—-]\s+Stack:/i);
  if (!m) return null;
  const norm = m[1].toLowerCase().replace(/[^a-z0-9]/g, "");
  return norm || null;
}

function _deduplicateAndAdd(
  type,
  content,
  source,
  embedding,
  embeddingModel,
  confidence,
  maxMemories,
  projectKey = null,
) {
  // Project-identity memories ("Project: <name> — Stack: ...") must stay unique
  // per project. The cosine + same-projectKey dedup below misses them: stack
  // guesses vary across sessions (similarity dips under 0.92) and the projectKey
  // drifts (folder- vs name-derived), so a fresh duplicate accumulated every
  // session. Match by normalized project name instead and refresh the existing
  // entry in place (consolidating its projectKey to the current one).
  if (type === "project") {
    const idName = _projectIdentityName(content);
    if (idName) {
      const existing = _memories.find(
        (m) =>
          m.type === "project" && _projectIdentityName(m.content) === idName,
      );
      if (existing) {
        existing.updatedAt = new Date().toISOString();
        existing.confidence = Math.min(
          1.0,
          (existing.confidence || 0.8) + 0.05,
        );
        existing.content = content;
        existing.source = source;
        if (projectKey) existing.projectKey = projectKey;
        if (embedding) {
          existing.embedding = embedding;
          existing.embeddingModel = embeddingModel;
        }
        _markMemoryCollectionChanged();
        _persistToDisk();
        return;
      }
    }
  }

  // Check for duplicates among same type (and same project, if applicable)
  const sameType = _memories.filter(
    (m) =>
      m.type === type && m.embedding && m.projectKey === (projectKey || null),
  );
  for (const existing of sameType) {
    const sim = cosineSimilarity(embedding, existing.embedding);
    if (sim > 0.92) {
      // Update existing — bump confidence, refresh content and timestamp
      existing.updatedAt = new Date().toISOString();
      existing.confidence = Math.min(1.0, existing.confidence + 0.05);
      existing.content = content;
      existing.source = source;
      existing.projectKey = projectKey || null;
      _markMemoryCollectionChanged();
      _persistToDisk();
      return;
    }
  }

  // No duplicate — add as new
  addMemory({
    type,
    content,
    source,
    projectKey,
    embedding,
    embeddingModel,
    confidence,
  });
  _autoPrune(maxMemories);
}

function _autoPrune(maxMemories) {
  if (_memories.length <= maxMemories) return;

  const overflow = _memories.length - maxMemories;
  // Pinned memories are never auto-pruned — only unpinned records are candidates.
  const prunable = _memories.filter((m) => !m.pinned);
  if (prunable.length === 0) return;

  // Sort prunable by confidence (asc), then createdAt (asc) — oldest low-confidence first
  const sorted = prunable.sort((a, b) => {
    if (a.confidence !== b.confidence) return a.confidence - b.confidence;
    return new Date(a.createdAt) - new Date(b.createdAt);
  });

  const toRemove = new Set(
    sorted.slice(0, Math.min(overflow, prunable.length)).map((m) => m.id),
  );
  if (toRemove.size === 0) return;
  _memories = _memories.filter((m) => !toRemove.has(m.id));
  _markMemoryCollectionChanged();
  _persistToDisk();
}

// ── Phase 2 hybrid recall (MEMORYFIX): multi-turn query + BM25 + RRF ──
// All opt-in via config.memory.hybridRecall; the default path is unchanged.

const RRF_K = 60;
const HYBRID_CANDIDATE_K = 25;
const MULTI_TURN_DEFAULT = 3;

/**
 * Combine the last N user messages into one query string, weighting the most
 * recent higher (it is repeated once → ~2x weight in both the embedding text
 * and the BM25 terms). Returns "" when there are no user messages.
 */
function buildMultiTurnQuery(messages, maxTurns = MULTI_TURN_DEFAULT) {
  if (!Array.isArray(messages)) return "";
  const userTexts = messages
    .filter(
      (m) =>
        m &&
        m.role === "user" &&
        typeof m.content === "string" &&
        m.content.trim(),
    )
    .map((m) => m.content.trim());
  if (userTexts.length === 0) return "";
  const recent = userTexts.slice(-Math.max(1, maxTurns));
  const latest = recent[recent.length - 1];
  return [...recent, latest].join("\n");
}

const _STOPWORDS = new Set(
  "the a an and or but if then else of to in on for with is are was were be been it this that these those as at by from".split(
    " ",
  ),
);

function _tokenize(text) {
  return String(text || "")
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .filter((t) => t.length >= 2 && !_STOPWORDS.has(t));
}

/**
 * BM25 relevance scores for each doc against the query text.
 * @param {string} queryText
 * @param {{id:string, content:string}[]} docs
 * @returns {Map<string, number>} id → score (0 when no term overlap)
 */
function bm25Scores(queryText, docs, k1 = 1.5, b = 0.75) {
  const scores = new Map();
  if (!Array.isArray(docs) || docs.length === 0) return scores;
  const docTokens = docs.map((d) => _tokenize(d.content));
  const docLens = docTokens.map((t) => t.length);
  const avgdl = docLens.reduce((s, n) => s + n, 0) / docs.length || 1;
  const queryTerms = [...new Set(_tokenize(queryText))];
  if (queryTerms.length === 0) {
    for (const d of docs) scores.set(d.id, 0);
    return scores;
  }
  const N = docs.length;
  const df = new Map();
  for (const term of queryTerms) {
    let n = 0;
    for (const toks of docTokens) if (toks.includes(term)) n++;
    df.set(term, n);
  }
  for (let i = 0; i < docs.length; i++) {
    const tf = new Map();
    for (const t of docTokens[i]) tf.set(t, (tf.get(t) || 0) + 1);
    let score = 0;
    for (const term of queryTerms) {
      const f = tf.get(term) || 0;
      if (f === 0) continue;
      const n = df.get(term) || 0;
      const idf = Math.log(1 + (N - n + 0.5) / (n + 0.5));
      const denom = f + k1 * (1 - b + (b * docLens[i]) / avgdl);
      score += idf * ((f * (k1 + 1)) / denom);
    }
    scores.set(docs[i].id, score);
  }
  return scores;
}

/**
 * Reciprocal Rank Fusion of multiple ranked id-lists (best→worst order).
 * @param {string[][]} rankings
 * @returns {Map<string, number>} id → fused score
 */
function rrfFuse(rankings, k = RRF_K) {
  const fused = new Map();
  for (const ranking of rankings) {
    if (!Array.isArray(ranking)) continue;
    for (let rank = 0; rank < ranking.length; rank++) {
      const id = ranking[rank];
      fused.set(id, (fused.get(id) || 0) + 1 / (k + rank + 1));
    }
  }
  return fused;
}

// ── Phase 3: Retrieval & Injection ──────────────────────

async function buildMemoryContext(
  ollamaUrl,
  embeddingModel,
  messages,
  config,
  conversationId = null,
  projectKey = null,
) {
  const startedMs = _nowMs();
  let queryLength = 0;
  let candidateCount = 0;
  let injectedCount = 0;
  const empty = { prompt: "", memories: [] };

  // Hybrid recall (multi-turn query + BM25/RRF fusion) is opt-in (MEMORYFIX P2).
  const hybrid = !!(config.memory && config.memory.hybridRecall);

  // Query text: the last user message by default, or the last few user messages
  // combined (recent-weighted) when hybrid recall is enabled.
  const lastUserMsg = [...messages].reverse().find((m) => m.role === "user");
  if (!lastUserMsg || !lastUserMsg.content) return empty;
  const queryText = hybrid
    ? buildMultiTurnQuery(
        messages,
        config.memory?.recallTurns || MULTI_TURN_DEFAULT,
      )
    : lastUserMsg.content;
  queryLength = queryText ? queryText.length : 0;
  if (!queryText) return empty;

  try {
    const embKey = effectiveOllamaApiKey(config);
    const queryEmbedding = await embed(ollamaUrl, queryText, embeddingModel, {
      ...(embKey ? { apiKey: embKey } : {}),
      timeoutMs: 12000,
    });

    if (!queryEmbedding) return empty;

    const maxChars = (config.memory?.maxContextTokens || 500) * 4;

    // User-tunable threshold (Settings → Memory → Recall Threshold). Default 0.6.
    const GLOBAL_THRESHOLD =
      typeof config.memory?.recallThreshold === "number"
        ? config.memory.recallThreshold
        : 0.6;
    // Summaries are already scoped to one conversation so a looser match is fine.
    const LOCAL_THRESHOLD = Math.max(0.3, GLOBAL_THRESHOLD - 0.25);

    // Candidate gather. Default path keeps the cosine thresholds and small K;
    // hybrid gathers a broader pool (no cosine cutoff, larger K) so BM25 can
    // surface keyword matches the vector pass alone would miss.
    const passK = hybrid ? HYBRID_CANDIDATE_K : 4;
    const agentThreshold = hybrid ? 0 : GLOBAL_THRESHOLD;
    const localThreshold = hybrid ? 0 : LOCAL_THRESHOLD;

    // ── Pass 1: Agent memories — facts about the user, global across all projects ──
    const agentResults = searchMemories(queryEmbedding, passK, agentThreshold, {
      types: ["fact"],
      embeddingModel,
      indexCacheEnabled: !!config?.memory?.indexCache?.enabled,
      timingConfig: config,
    });

    // ── Pass 2: Project memories — patterns and project info for the active project ──
    // Only injected when a projectKey is known; keeps other projects from bleeding in.
    const projectResults = projectKey
      ? searchMemories(queryEmbedding, passK, agentThreshold, {
          projectKey,
          types: ["project", "pattern"],
          embeddingModel,
          indexCacheEnabled: !!config?.memory?.indexCache?.enabled,
          timingConfig: config,
        })
      : [];

    // ── Pass 3: Conversation summaries — scoped to this thread ──
    const summaryResults = conversationId
      ? searchMemories(queryEmbedding, hybrid ? passK : 2, localThreshold, {
          conversationId,
          scopeToConversation: true,
          types: ["summary"],
          embeddingModel,
          indexCacheEnabled: !!config?.memory?.indexCache?.enabled,
          timingConfig: config,
        })
      : [];

    // Merge all three passes, deduplicate by id; pinned global facts always qualify
    const seen = new Set();
    const candidates = [];
    for (const r of _pinnedGlobalFactCandidates(embeddingModel)) {
      if (!seen.has(r.id)) {
        seen.add(r.id);
        candidates.push(r);
      }
    }
    for (const r of [...agentResults, ...projectResults, ...summaryResults]) {
      if (!seen.has(r.id)) {
        seen.add(r.id);
        candidates.push(r);
      }
    }
    candidateCount = candidates.length;
    if (candidates.length === 0) return empty;

    // Rank: default = cosine score; hybrid = Reciprocal Rank Fusion of the
    // semantic (cosine) ranking and a BM25 keyword ranking over the same pool.
    let merged;
    if (hybrid) {
      const cosineRanked = [...candidates]
        .sort((a, b) => b.score - a.score)
        .map((m) => m.id);
      const bm25 = bm25Scores(
        queryText,
        candidates.map((m) => ({ id: m.id, content: m.content })),
      );
      const bm25Ranked = [...candidates]
        .sort((a, b) => (bm25.get(b.id) || 0) - (bm25.get(a.id) || 0))
        .map((m) => m.id);
      const fused = rrfFuse([cosineRanked, bm25Ranked]);
      merged = candidates
        .map((m) => ({ ...m, score: fused.get(m.id) || 0 }))
        .sort((a, b) => b.score - a.score);
    } else {
      merged = candidates.sort((a, b) => b.score - a.score);
    }

    if (merged.length === 0) return empty;

    // Format as prompt section — label each line by tier so the model understands provenance
    let items = [];
    let totalLen = 0;
    for (const r of merged) {
      const label =
        r.type === "fact"
          ? "agent"
          : r.type === "summary"
            ? "this conversation"
            : r.projectKey || "project";
      const line = `- [${label}] ${r.content}`;
      if (totalLen + line.length > maxChars) break;
      items.push(line);
      totalLen += line.length;
    }
    injectedCount = items.length;

    const prompt =
      items.length > 0
        ? `\n\n---\nMEMORY CONTEXT:\n[agent] = about you (recalled always) | [project] = active project | [this conversation] = from earlier in this chat\n${items.join("\n")}\n---`
        : "";

    const memories = merged.slice(0, items.length).map((m) => ({
      id: m.id,
      type: m.type,
      content: m.content,
      projectKey: m.projectKey || null,
    }));

    // nearMisses: ranked candidates that were retrieved but did NOT fit the
    // prompt budget. Optional debugging aid surfaced over SSE (MEMORYFIX P2).
    const NEAR_MISS_LIMIT = 5;
    const nearMisses = merged
      .slice(items.length, items.length + NEAR_MISS_LIMIT)
      .map((m) => ({
        id: m.id,
        type: m.type,
        content: m.content,
        projectKey: m.projectKey || null,
        score: typeof m.score === "number" ? Number(m.score.toFixed(4)) : null,
      }));

    return { prompt, memories, nearMisses };
  } catch (err) {
    const msg = err && err.message != null ? String(err.message) : "";
    if (!(err && err.name === "AbortError") && !/\baborted\b/i.test(msg)) {
      console.warn("[Memory] buildMemoryContext failed:", msg || err);
    }
    return empty;
  } finally {
    _logTiming(config, "buildMemoryContext", startedMs, {
      hybrid,
      queryChars: queryLength,
      candidates: candidateCount,
      injected: injectedCount,
      indexCache: !!config?.memory?.indexCache?.enabled,
    });
  }
}

module.exports = {
  initMemory,
  compactMemories,
  addMemory,
  getMemories,
  getMemory,
  updateMemory,
  deleteMemory,
  deleteMemoriesBySource,
  searchMemories,
  deriveProjectKey,
  getStats,
  analyzeMemoryQualityDryRun,
  startMemoryPruneCheckScheduler,
  cosineSimilarity,
  extractAndStore,
  buildMemoryContext,
  ensureAgentAutonomyMemory,
  flushMemoryToDisk,
  reembedAllMemories,
  buildMultiTurnQuery,
  bm25Scores,
  rrfFuse,
  extractionHasNewMessages,
  markExtractionWatermark,
  DEFAULT_EMBEDDING_MODEL,
  resolveEmbeddingModel,
  // Exported for unit tests (project-identity dedup-on-write).
  _deduplicateAndAdd,
  _projectIdentityName,
};

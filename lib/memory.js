...
const DEFAULT_PRUNE_CHECK_INTERVAL_DAYS = 30;
...
function _ensureIndexCache(options = {}) {
  ...
  for (const memory of _memories) {
    ...
    _pushToIndex(byTypeSource, _typeSourceKey(memory.type, memory.source), memory);
  }
...
function _getIndexedPool(options = {}) {
  ...
  if (scopeToConversation && conversationId) {
    ...
    for (const type of types) {
      const bucket = cache.byTypeSource.get(_typeSourceKey(type, conversationId));
...
function _scoreLowSignalCandidate(memory, opts, nowMs) {
  ...
  const ageDays = Math.max(0, Math.floor((nowMs - Math.max(createdMs, updatedMs)) / MS_PER_DAY));
  ...
  if (contentLength < opts.minContentLength) {
    const contentPenalty = Math.min(
      1,
      (opts.minContentLength - contentLength) / Math.max(1, opts.minContentLength),
    );
  ...
  if (ageDays > opts.staleDays) {
    score += Math.min(0.2, (ageDays - opts.staleDays) / Math.max(1, opts.staleDays) * 0.2);
    reasons.push("stale");
  }
  ...
  if (
    /^(ok|thanks|noted|got it|done|cool|yes|no|maybe|sure)[.!]?$/i.test(
      content,
    )
  ) {
    score += 0.35;
...
function analyzeMemoryQualityDryRun(opts = {}) {
  ...
  const staleDays = Math.floor(
    _toFiniteNumber(opts.staleDays, DEFAULT_PRUNE_CHECK_THRESHOLDS.staleDays, 1, 3650),
  );
  ...
  for (const memory of snapshot) {
    ...
    const updatedMs = Number(new Date(memory.updatedAt || memory.createdAt || 0));
    const ageDays = Math.max(0, Math.floor((nowMs - (Number.isFinite(updatedMs) ? updatedMs : nowMs)) / MS_PER_DAY));
    ...
    if (!nearDuplicateBuckets.has(nearKey)) nearDuplicateBuckets.set(nearKey, []);
  ...
  for (const bucket of nearDuplicateBuckets.values()) {
    ...
    for (let i = 0; i < sortedBucket.length; i++) {
      for (let j = i + 1; j < sortedBucket.length; j++) {
        ...
        const similarity = Number((jaccard * 0.7 + lengthRatio * 0.3).toFixed(4));
  ...
  lowSignalCandidates.sort((a, b) => b.score - a.score || a.id.localeCompare(b.id));

  const suggestedNearDuplicateSimilarity = Number(
    Math.max(nearDuplicateSimilarity, _percentile(nearDuplicatePairs.map((p) => p.similarity), 0.75)).toFixed(4),
  );
  const suggestedLowSignalScoreThreshold = Number(
    Math.max(lowSignalScoreThreshold, _percentile(lowSignalScores, 0.8)).toFixed(4),
  );
...
function _loadFromDisk() {
  ...
  try {
    ...
    _logTiming(null, "loadMemoryFromDisk", startedMs, { loaded: _memories.length });
  } catch {
    _memories = [];
    _markMemoryCollectionChanged();
    _logTiming(null, "loadMemoryFromDisk", startedMs, { loaded: 0 });
...
async function reembedAllMemories(
  ...
  if (reembedded > 0) _markMemoryCollectionChanged();

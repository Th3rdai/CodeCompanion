...
const { chatStructured, embed } = require('./ollama-client');
...
function addMemory({ type, content, source, embedding, embeddingModel, confidence }) {
  if (!_memoryDir) throw new Error('Memory not initialized. Call initMemory(dataRoot) first.');
...
function updateMemory(id, updates) {
  ...
  const allowedFields = ['type', 'content', 'embedding', 'embeddingModel', 'confidence', 'active'];
...
function searchMemories(queryEmbedding, topK = 5, threshold = 0.3) {
  if (!_memoryDir) throw new Error('Memory not initialized. Call initMemory(dataRoot) first.');
  if (!queryEmbedding || !Array.isArray(queryEmbedding)) return [];

  const scored = _memories
    .filter(m => m.active && m.embedding && Array.isArray(m.embedding))
    .map(m => ({
      ...m,
...
const EXTRACTION_SCHEMA = {
  type: 'object',
  properties: {
    facts: { type: 'array', items: { type: 'string' }, description: 'Factual statements about the user' },
    project: {
      type: 'object',
      properties: {
        name: { type: 'string' },
    ...
    patterns: { type: 'array', items: { type: 'string' }, description: 'Code patterns or recurring issues' },
...
async function extractAndStore(ollamaUrl, chatModel, embeddingModel, conversation) {
...
Return JSON only. Be concise. Skip generic facts like "user writes code".`;
  ...
  if (Array.isArray(extracted.facts)) {
    for (const fact of extracted.facts) {
      ...
      try {
        const emb = await embed(ollamaUrl, fact, embeddingModel);
        await _deduplicateAndAdd('fact', fact, conversationId, emb, embeddingModel, 0.7, maxMemories);
      } catch (err) {
  ...
  if (Array.isArray(extracted.patterns)) {
    for (const pattern of extracted.patterns) {
      if (!pattern || typeof pattern !== 'string' || pattern.trim().length < 5) continue;
      try {
        const emb = await embed(ollamaUrl, pattern, embeddingModel);
        await _deduplicateAndAdd('pattern', pattern, conversationId, emb, embeddingModel, 0.6, maxMemories);
      } catch (err) {
  ...
  if (extracted.project && extracted.project.name) {
    ...
    try {
      const emb = await embed(ollamaUrl, projContent, embeddingModel);
      await _deduplicateAndAdd('project', projContent, conversationId, emb, embeddingModel, 0.8, maxMemories);
    } catch (err) {
  ...
  // Process summary
  if (extracted.summary && typeof extracted.summary === 'string' && extracted.summary.trim().length > 5) {
    try {
      const emb = await embed(ollamaUrl, extracted.summary, embeddingModel);
      addMemory({
        type: 'summary',
...
function _deduplicateAndAdd(type, content, source, embedding, embeddingModel, confidence, maxMemories) {
  // Check for duplicates among same type
  const sameType = _memories.filter(m => m.active && m.type === type && m.embedding);
...
async function buildMemoryContext(ollamaUrl, embeddingModel, messages, config) {
  const empty = { prompt: '', memories: [] };

  // If 0 active memories, return immediately
  const active = _memories.filter(m => m.active && m.embedding);
  ...
  try {
    ...
    let queryEmbedding;
    try {
      queryEmbedding = await embed(ollamaUrl, lastUserMsg.content, embeddingModel);
    } finally {

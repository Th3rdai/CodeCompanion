...
function _migrateGlobalScope() {
  ...
  if (changed > 0) {
    console.log(`[Memory] Migrated ${changed} memory fields to global/project scope.`);
    _persistToDisk();
  }
}
...
function _deduplicateAndAdd(
  ...
  const sameType = _memories.filter(
    (m) => m.type === type && m.embedding && m.projectKey === (projectKey || null),
  );
  ...
  addMemory({ type, content, source, projectKey, embedding, embeddingModel, confidence });
  _autoPrune(maxMemories);
}
...
async function buildMemoryContext(
  ...
  try {
    ...
    for (const r of merged) {
      const label =
        r.type === "fact" ? "agent"
        : r.type === "summary" ? "this conversation"

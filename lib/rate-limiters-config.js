...
function registerRateLimiters(app) {
  ...
  mount("/api/dictate-transcribe", {
    name: "dictate-transcribe",
    max: envNum("RATE_LIMIT_MAX_DICTATE", 24),
    windowMs: RATE_LIMIT_WINDOW_MS,
    methods: ["POST"],
  });
  mount("/api/pentest", {
  ...
  mount("/api/setup-assistant", {
    name: "setup-assistant",
    max: envNum("RATE_LIMIT_MAX_SETUP_ASSISTANT", 12),
    windowMs: RATE_LIMIT_WINDOW_MS,
    methods: ["POST"],
  });
  mount("/api/memory", {

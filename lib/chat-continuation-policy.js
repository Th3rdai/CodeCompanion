...
function userRequestedSustainedWork(messages = []) {
  ...
  return /\b(until done|until complete|don't stop|do not stop|keep going|finish all|complete all|full implementation|all sections|entire feature|whole project|implement everything|write all|all files|autonomously|autonomous|on your own|without asking|do not ask|don't ask|no permission needed|you decide|figure it out|admin permissions?|full access)\b/i.test(
    text,
  );
}

function buildToolResultFollowUpMessage(
  ...
  if (actionableIntent) {
    return (
      `Tool results:\n${externalizedToolResults}\n\n` +
      `⚡ CONTINUE WORK — ${AGENT_AUTONOMY_PROMPT_SNIPPET} If the user's original request is not fully complete, emit the next TOOL_CALL now (minimal prose). ` +

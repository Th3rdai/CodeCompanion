...
class GsdBridge {
  ...
  exec(command, args = []) {
    if (!GsdBridge.isInstalled()) {
      return {
        error: "GSD tools not installed. Run auto-install or install manually.",
      };
    }
  ...
  readPlan(phaseDir, planNum) {
    ...
    try {
      const files = fs
        .readdirSync(fullDir)
        .filter((f) => f.includes(`-${padded}-PLAN.md`));
  ...
  readSummary(phaseDir, planNum) {
    ...
    try {
      const files = fs
        .readdirSync(fullDir)
        .filter((f) => f.includes(`-${padded}-SUMMARY.md`));
      if (files.length === 0) return null;
      return fs.readFileSync(path.join(fullDir, files[0]), "utf-8");
    } catch {
      return null;
  ...
  static ensureInstalled() {
    ...
    try {
      ...
      execFileSync(
        "git",
        ["clone", "https://github.com/gsd-build/get-shit-done.git", GSD_DIR],
        {
          timeout: 60000,
          stdio: "pipe",
        },

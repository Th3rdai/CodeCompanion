...
async function reviewCode(ollamaUrl, model, code) {
  try {
    ...
    return {
      type: "chat-fallback",
      stream: chatStream(ollamaUrl, model, fallbackMessages),
    };
  }
}
```
...
const FindingSchema = z.object({
  ...
  analogy: z
    .string()
    .optional()
    .describe("Everyday analogy for critical/high severity only"),
});
...
const ReportCardSchema = z.object({
  ...
  topPriority: z
    .object({
      ...
      explanation: z
        .string()
        .describe("Why this is the #1 thing to fix, plain English"),
    })

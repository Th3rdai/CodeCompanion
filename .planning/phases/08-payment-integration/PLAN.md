...
2. **Distribution channel detection** (prerequisite for all store integrations)
   ...
   // Mac App Store build:
   env: { CC_DISTRIBUTION: 'appstore-mac' }
   // Direct build:
   env: { CC_DISTRIBUTION: 'direct' }
   ```
...
3. **Electron main process** (`electron/main.js`)
   ...
   if (CC_DISTRIBUTION === 'appstore-mac') {
         ...
         switch (tx.transactionState) {
           ...
           case 'restored':
             if (tx.productIdentifier === 'cc_pro_lifetime') {
               ...
               }).catch(err => console.error('[Main] App Store activation error:', err));
               // Notify renderer
               mainWindow?.webContents.send('purchase-complete', { success: true, tier: 'pro' });
             }
     ...
     ipcMain.handle('restore-purchases', async () => {
       return { success: false, error: 'Restore is only available for App Store purchases' };
     });
   }
   ```
...
7. **Update UpgradePrompt for distribution channel awareness**
   ...
   {distribution === 'appstore-mac' ? (
     ...
     <p>Get a license key at <span>th3rdai.com</span></p>
   )}
   ```

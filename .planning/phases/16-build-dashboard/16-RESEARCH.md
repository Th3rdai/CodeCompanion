...
app.post('/api/build/projects/:id/research', async (req, res) => {
  ...
  const messages = [
    { role: 'system', content: buildResearchPrompt(state, roadmap, phaseNumber) },
...
app.post('/api/build/projects/:id/next-action', async (req, res) => {
  ...
  const messages = [
    { role: 'system', content: `You are a friendly project coach. Given the project state, suggest the single most important next action. Be concise (2-3 sentences). Use encouraging, non-technical language.` },
...
app.use('/api/build/projects', createRateLimiter({
  name: 'build-registry',

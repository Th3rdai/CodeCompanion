...
export default function DashboardPanel({
  ...
  return (
    <section
      ...
      aria-label="Dashboard and reports"
    >
        ...
        <div className="flex flex-wrap items-center gap-2 justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Pick the widgets you want to see and export reports whenever you
              need them.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={onRefresh}
              className="text-xs px-2.5 py-1.5 rounded-lg border border-slate-600 text-slate-300 hover:bg-slate-700/40"
            >
              Refresh
            </button>
            <button
              onClick={() => onExport("md")}
              className="text-xs px-2.5 py-1.5 rounded-lg border border-indigo-500/30 text-indigo-300 hover:bg-indigo-600/10"
            >
              Export MD
            </button>
            <button
      ...
      {visibleWidgets.includes("performance") && (
        <div className="glass rounded-xl border border-slate-700/30 p-4">
          ...
          {systemMetricsLoading ? (
            ...
            <p className="text-xs text-slate-500">
              Performance metrics aren't available just yet — check back soon!
            </p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="rounded-lg border border-slate-700/40 p-3">
                ...
                <div className="text-xl font-semibold text-indigo-300">
                  {systemMetrics.cpu?.usagePercent ?? 0}%
                </div>
                <div className="text-[11px] text-slate-500 mt-1">
                  Uptime: {systemMetrics.uptimeSec ?? 0}s
                </div>
              </div>
              <div className="rounded-lg border border-slate-700/40 p-3">
                ...
                <div className="text-xl font-semibold text-slate-100">
                  {(systemMetrics.memory?.rssMb ?? 0).toFixed(1)} /{" "}
                  {(systemMetrics.memory?.heapUsedMb ?? 0).toFixed(1)} MB
                </div>
                <div className="text-[11px] text-slate-500 mt-1">
                  Heap total:{" "}
                  {(systemMetrics.memory?.heapTotalMb ?? 0).toFixed(1)} MB
                </div>
              </div>
              <div className="rounded-lg border border-slate-700/40 p-3">
                <div className="text-[11px] text-slate-500">
                  Network Throughput (1m)
                </div>
                <div className="text-sm text-slate-200 mt-1">
                  In:{" "}
                  <span className="text-indigo-300">
                    {(systemMetrics.network?.inboundKbps1m ?? 0).toFixed(2)}{" "}
                    kbps
                  </span>
                </div>
                <div className="text-sm text-slate-200">
                  Out:{" "}
                  <span className="text-indigo-300">
                    {(systemMetrics.network?.outboundKbps1m ?? 0).toFixed(2)}{" "}
                    kbps
                  </span>
                </div>
                <div className="text-[11px] text-slate-500 mt-1">
                  Req/min: {systemMetrics.requests?.perMinute ?? 0} | Avg
                  latency: {systemMetrics.requests?.avgLatencyMs ?? 0} ms
                </div>
              </div>
            </div>
          )}
      ...
      {visibleWidgets.includes("activity") && (
        <div className="glass rounded-xl border border-slate-700/30 p-4">
          ...
          {activityRows.length === 0 ? (
            <p className="text-xs text-slate-500">
              No activity in the last 7 days — let's change that!
            </p>
          ) : (

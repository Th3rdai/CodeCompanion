...
const STEPS = [
  ...
  },
  {
    ...
    content: (
      <>
        <p className="text-slate-300 mb-3">
          Code Companion uses <strong className="text-indigo-300">Ollama</strong> to run AI models
        ...
        <div className="glass rounded-lg p-3 text-xs text-slate-400 space-y-1.5">
          <p><strong className="text-slate-300">Quick setup:</strong></p>
          <ol className="list-decimal list-inside space-y-1">
            <li>Install Ollama from <code className="bg-slate-700/50 px-1.5 py-0.5 rounded text-indigo-300">ollama.com</code></li>
            <li>Pull a model: <code className="bg-slate-700/50 px-1.5 py-0.5 rounded text-indigo-300">ollama pull llama3.2</code></li>
    ...
    content: (
      <>
        <div className="space-y-3">
          {[
            { icon: '🏠', title: 'Runs locally', desc: 'AI models run on your machine through Ollama. No cloud APIs, no external servers.' },
            { icon: '🔒', title: 'No data collection', desc: 'We never track, log, or transmit your code or conversations anywhere.' },
            { icon: '💾', title: 'Local storage only', desc: 'Conversation history is saved as JSON files on your disk. Delete them anytime.' },
            { icon: '🚫', title: 'No API keys needed', desc: 'No accounts, no sign-ups, no subscriptions. Just install and go.' },
          ].map(item => (
  ...
  return (
    ...
    >
      <div className="glass-heavy rounded-2xl w-full max-w-lg neon-border overflow-hidden">
        ...
        {/* Content */}
        <div className={`p-6 transition-opacity duration-200 ${fading ? 'opacity-0' : 'opacity-100'}`}>
          {/* Step icon and title */}
          <div className="text-center mb-5">
            <div className="text-4xl mb-3">{current.icon}</div>
            <h2 className="text-xl font-bold text-slate-100 neon-text mb-1">{current.title}</h2>
          ...
          <div className="flex items-center justify-center gap-2 mb-5">
            {STEPS.map((_, i) => (
              <button
                key={i}
                onClick={() => { setSlideDir(i > step ? 'right' : 'left'); setFading(true); setTimeout(() => { setStep(i); setFading(false); }, 200); }}

...
import BuildHeader from "../build-mode/BuildHeader";
import BuildSimpleView from "../build-mode/BuildSimpleView";
import BuildAdvancedView from "../build-mode/BuildAdvancedView";
  ...
  async function loadPhaseDetail(projectId, phaseNum) {
    ...
    try {
      const res = await apiFetch(
        `/api/build/projects/${projectId}/phase/${phaseNum}`,
      );
  ...
  async function handleImportProject() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        onToast?.(
          data.scaffolded
            ? "Project imported — .planning/ created automatically"
            : "Project imported",
        );
        setShowImport(false);
        setImportPath("");
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="btn-neon text-sm px-4 py-2 rounded-lg font-medium"
            >
              + New Project
            </button>
          </div>
        ...
        {showImport && (
          <div className="glass-neon rounded-xl p-4 space-y-3 mb-4">
            ...
            <p className="text-xs text-slate-400">
              Works with Create projects, GitHub clones, or any codebase.
              Planning structure will be added automatically if needed.
            </p>
        ...
        {projects.length === 0 && !showImport ? (
          <div className="glass rounded-xl p-8 text-center space-y-3">
            ...
            <p className="text-xs text-slate-500">
              Create a new project, or import an existing one (Create projects,
              GitHub clones, any folder)
            </p>
            <button
              onClick={onNewProject}
              className="btn-neon text-sm px-6 py-2.5 rounded-lg font-medium mt-2"
            >
    ...
    return (
      <div className="flex-1 overflow-y-auto scrollbar-thin p-4 sm:p-6 space-y-4">
        ...
        <div className="glass-neon rounded-xl p-4 space-y-3">
          <p className="text-sm font-medium text-slate-200">
            Execute in Claude Code or Cursor:
          </p>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => loadProjectData(activeProject)}
        ...
        <div className="space-y-3">
          {(phaseDetail.plans || []).map((plan) => (
            <div key={plan.number} className="glass rounded-xl p-4 space-y-2">
              <div className="flex items-center gap-2">
                <div
                  className={`w-2.5 h-2.5 rounded-full ${plan.hasSummary ? "bg-emerald-400" : plan.hasPlan ? "bg-amber-400" : "bg-slate-500"}`}
                />
              ...
              {plan.plan && (
                <details className="mt-2">
                  <summary className="text-xs text-slate-400 cursor-pointer hover:text-indigo-300">
                    View Plan
                  </summary>

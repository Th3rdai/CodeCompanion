...
const btnClass = 'text-xs px-2.5 py-1.5 rounded-lg text-slate-400 hover:text-indigo-300 hover:bg-indigo-500/10 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/40';
...
export default function InputToolbar({
  ...
  function handleDownloadMarkdown() {
    const last = [...messages].reverse().find(m => m.role === 'assistant');
    if (!last?.content) { onToast?.('No AI response to download'); return; }
      ...
      {onFileUpload && !hidden.has('upload') && (
        <>
          <input ref={fileRef} type="file" multiple className="hidden"
            accept={fileAccept || '.js,.jsx,.ts,.tsx,.py,.json,.md,.txt,.html,.css,.yaml,.yml,.sh,.sql,.go,.rs,.java,.c,.cpp,.h,.toml,.xml,.csv'}
            onChange={onFileUpload} />
          <button onClick={() => fileRef.current?.click()} title="Upload files" className={btnClass}>
            📎 Upload
          </button>
        </>
      )}
      {!hidden.has('paste') && (
        <button onClick={handlePaste} title="Paste text from clipboard" className={btnClass}>
          📋 Paste
        </button>
      )}
      {!hidden.has('copy') && (
        <button onClick={handleCopyLastResponse} title="Copy last AI response" className={btnClass}>
          📑 Copy Response
        </button>
      )}
      {!hidden.has('markdown') && (
        <button onClick={handleDownloadMarkdown} title="Download response as Markdown" className={btnClass}>
          📝 Markdown
        </button>
      )}
      {!hidden.has('export') && (

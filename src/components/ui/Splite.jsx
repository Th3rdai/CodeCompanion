 ...
 */
export default function Splite({ className = '', color = '#8b5cf6', width = '100%', height = 2, speed = 3 }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const p of particles) {
        const grad = ctx.createLinearGradient(p.x - p.size, cy, p.x + p.size, cy);
        grad.addColorStop(0, 'transparent');
        grad.addColorStop(0.5, color + Math.round(p.opacity * 255).toString(16).padStart(2, '0'));
        ...
        // Core bright center
        const coreGrad = ctx.createRadialGradient(p.x, cy, 0, p.x, cy, p.size * 0.3);

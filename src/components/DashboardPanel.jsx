    ...
    <section className="flex-1 overflow-y-auto scrollbar-thin px-4 py-4 space-y-4" aria-label="Dashboard and reports">
        ...
        <div className="flex flex-wrap items-center gap-2 justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Customize visible widgets and export PM-friendly reports.
            </p>
          </div>
      ...
      {visibleWidgets.includes('performance') && (
        <div className="glass rounded-xl border border-slate-700/30 p-4">
          ...
          {systemMetricsLoading ? (
            ...
            <p className="text-xs text-slate-500">Performance metrics unavailable right now.</p>
          ) : (
      ...
      {visibleWidgets.includes('modes') && (
        <div className="glass rounded-xl border border-slate-700/30 p-4">
          ...
          {modeCounts.length === 0 ? <p className="text-xs text-slate-500">No conversation data yet.</p> : <BarList items={modeCounts} />}
        </div>
      )}
      ...
      {visibleWidgets.includes('models') && (
        <div className="glass rounded-xl border border-slate-700/30 p-4">
          ...
          {modelCounts.length === 0 ? <p className="text-xs text-slate-500">No model data yet.</p> : <BarList items={modelCounts} />}
        </div>
      )}

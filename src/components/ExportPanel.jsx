...
const FORMAT_GROUPS = [
  ...
  {
    label: "Presentations",
    formats: [{ ext: ".pptx", label: "PowerPoint", icon: "📙" }],
  },
];
...
export default function ExportPanel({ messages, mode, showToast }) {
  ...
  async function handleExport() {
    ...
    if (useZip) {
      ...
      showToast(
        `${formats.length} file${formats.length > 1 ? "s" : ""} exported`,
      );
    }
                  ...
                  <div className="grid grid-cols-2 gap-1">
                    {group.formats.map((f) => (
                      ...
                      >
                        ...
                        />
                        <span
                          className={`w-3.5 h-3.5 rounded border flex-shrink-0 flex items-center justify-center text-[8px] ${
                            selected.has(f.ext)

...
import { ChevronDown, ChevronUp, Eye, Target, LayoutList, Zap, CheckSquare, FileCheck, BookOpen, Repeat, Compass, Wrench, GitBranch, Shield } from 'lucide-react';

// ── Icon registry ────────────────────────────────────
const ICONS = { Eye, Target, LayoutList, Zap, CheckSquare, FileCheck, BookOpen, Repeat, Compass, Wrench, GitBranch, Shield };
    ...
    <div className="glass rounded-xl border border-slate-700/30 overflow-hidden transition-all hover:border-slate-600/50">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-slate-100">{categoryConfig?.label || categoryKey}</h3>
          <p className="text-xs text-slate-400 mt-0.5 truncate">{categoryData?.summary || ''}</p>
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-2 border-t border-slate-700/20 pt-3 fade-in">
          {suggestions.length > 0 ? (
            ...
            <p className="text-xs text-slate-500 italic">No specific suggestions for this category.</p>
          )}

          {onRevise && (
            <button
              onClick={() => onRevise(categoryKey, categoryConfig?.label || categoryKey)}
    ...
    <div className="space-y-4 fade-in max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {categories.map(cat => {
          ...
          return (
            <div key={cat.key} className={`glass rounded-xl border ${colors.border} p-3 text-center`}>
              <div className="flex items-center justify-center">
                <IconComponent className={`w-4 h-4 ${colors.text}`} />
              </div>
              <div className={`text-2xl font-bold ${colors.text} mt-1`}>{cd?.grade || '-'}</div>

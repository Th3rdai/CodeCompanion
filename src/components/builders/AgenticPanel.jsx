...
const AGENTIC_CONFIG = {
  ...
  fields: [
    {
      ...
      label: "Agent Name",
      type: "text",
      placeholder: "e.g., code-reviewer, data-analyst, deployment-bot",
      required: true,
    },
    {
      name: "purpose",
      ...
      type: "textarea",
      placeholder:
        'What does this agent do? What problem does it solve?\n\ne.g., "Automatically reviews pull requests for security issues, suggests fixes, and generates compliance reports."',
      required: true,
    },
    {
      name: "tools",
      label: "Tools (one per line: name — description)",
      type: "textarea",
      placeholder:
    ...
    },
    {
      ...
      type: "textarea",
      placeholder:
        'How should this agent behave? What rules should it follow?\n\ne.g., "Always read the full file before suggesting changes. Never modify test files without asking. Explain every change in plain English."',
      required: true,
      large: true,
    },
    {
  ...
  buildContent: (formData) => {
    ...
    if (formData.tools?.trim()) {
      ...
      for (const tool of toolLines) {
        ...
        if (parts.length >= 2) {
          lines.push(
            `- **${parts[0].trim()}**: ${parts.slice(1).join(sep).trim()}`,
          );
        } else {
  ...
  parseLoaded: (content) => {
    ...
    if (fmMatch) {
      ...
      if (descMatch)
        result.purpose = descMatch[1].trim().replace(/^["']|["']$/g, "");
    }
    ...
    for (let i = 1; i < sections.length; i++) {
      ...
      if (header.includes("purpose")) result.purpose = sectionBody;
        ...
        result.tools = sectionBody
          .split("\n")
          .filter((l) => l.trim())
          .map((l) =>
            l.replace(/^-\s*\*\*(.+?)\*\*:\s*/, "$1 — ").replace(/^-\s*/, ""),
          )
          .join("\n");
      } else if (header.includes("instruction")) {
        result.instructions = sectionBody;
        hasInstructionSection = true;
      } else if (header.includes("workflow")) result.workflow = sectionBody;
      else if (header.includes("safety") || header.includes("guardrail"))

...
import { GRADE_COLORS, SEVERITY_COLORS } from "./report-card-tokens";

const CATEGORY_LABELS = {
  bugs: {
    label: "Bugs",
    icon: <Bug className="w-4 h-4" />,
    desc: "Logic errors, crashes, and broken behavior",
  },
  security: {
    label: "Security",
    icon: <Lock className="w-4 h-4" />,
    desc: "Vulnerabilities and safety risks",
  },
  readability: {
    label: "Readability",
    icon: <BookOpen className="w-4 h-4" />,
    desc: "Clarity, naming, and maintainability",
  },
  completeness: {
    label: "Completeness",
    icon: <CheckCircle className="w-4 h-4" />,
    desc: "Missing features, edge cases, and error handling",
  },
};
        ...
        <div className="space-y-2 pt-1 fade-in">
          ...
          {(() => {
            const prompt =
              finding.fixPrompt ||
              (finding.explanation
                ? `Please fix this issue in my code: ${finding.title}. ${finding.explanation}`
                : null);
            return prompt ? (
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <CopyFixButton
                    text={prompt}
                    toastMessage="Copied! Paste into your AI tool"
                  />
                </div>
      ...
      <div className="p-4 space-y-3">
        ...
        {onDeepDive && (
          <button
            onClick={() =>
              onDeepDive(
                { title: meta.label, explanation: category.summary },
                categoryKey,
              )
            }
      ...
      <div className="glass rounded-2xl border border-slate-700/30 p-6">
        <div className="flex flex-wrap items-center gap-6">
          ...
          <div className="min-w-[12rem] max-w-full flex-1">
            ...
            {filename && (
              <p className="text-sm text-slate-400 mt-0.5 font-mono truncate">
                {filename}
              </p>
            )}
            ...
            <div className="relative">
              ...
              {showExportMenu && (
                <div className="absolute right-0 top-full mt-1 glass rounded-lg border border-slate-600 shadow-xl z-20 min-w-[140px] py-1">
                  <button
                    onClick={() => {
                      handleExportMarkdown(data, filename);
                    ...
                    onDeepDive(
                      {
                        ...
                        explanation: topPriority.explanation,
                        severity: "high",
                      },
                      topPriority.category,
                    )

...
> **Huge trees:** If the configured project folder is very large, unscoped `GET /api/files/tree` may return megabytes. Add `folder=` (URL-encoded path) to scope the tree, or write to a file before parsing.

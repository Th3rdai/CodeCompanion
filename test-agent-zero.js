...
async function testAgentZero() {
  try {
    ...
    const agentZeroConfig = (config.mcpClients || []).find(c => c.id === 'agent-zero');
    if (!agentZeroConfig) {
    ...
    const result = await mcpClient.callTool('agent-zero', 'send_message', {
      message: 'Hello! I am Code Companion. Can you briefly tell me what capabilities you have for agent-to-agent collaboration?'
    });
...
testAgentZero().then(() => process.exit(0)).catch(err => {
  console.error(err);
  process.exit(1);
});

...
| `MAC_CODESIGN_IDENTITY`                     | **Developer ID Application** certificate: full Keychain name or common name only (e.g. `Developer ID Application: Name (TEAMID)` or `Name (TEAMID)`). `electron-builder.config.js` strips the `Developer ID Application:` prefix for electron-builder 26+. Required when `MAC_DISTRIBUTION_SIGN=1`. |

...
- If **`conversationId`** is missing or invalid, **no** memory block is added (avoids global/cross-chat leakage).
- Memories without a **`source`** (legacy or manual) are **not** injected into scoped chat until tied to a conversation.
- The server **does not** inject another conversation’s summary into the system prompt (each thread relies on its own `messages` + scoped memory).

...
| `npm run test:integration` | Node **`node:test`** — spawns **`server.js`** on test ports (**`FORCE_HTTP`**, **`CC_DATA_DIR`**). Covers **`api-with-images.test.js`** (chat/review/pentest + images) and **`review-folder.test.js`** (**`/api/review/folder`**). Optional **`CC_TEST_REVIEW_FOLDER_MODEL`** if the default Review auto-model (see **`lib/auto-model.js`**, currently **`qwen3-32k`**) is not installed locally. |
